// Duckie Days v24.184 — Duckie Dash layout polish: lower Ready window + cost under preview.
// Freeze-safe: no MutationObserver; updates occur only at lifecycle/click/timer points.
(function(){
  'use strict';
  window.DUCKIE_DASH_SCREEN_SURFACE='24.184-lower-ready-cost-below-preview';
  ...
  style.textContent=`
    ...
    #dashScreen .dash-v184-start-note{display:block!important;margin:0;color:#7e6571;font-size:.82rem;line-height:1.3;}
    ...
    #dashScreen .dash-v184-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;}
    #dashScreen .dash-v184-stat{min-height:58px;padding:8px 10px;border:2px solid rgba(140,98,93,.13);border-radius:14px;background:#fffdfb;display:grid;place-items:center;text-align:center;align-content:center;gap:2px;}
    #dashScreen .dash-v184-stat span{font-size:.67rem;letter-spacing:.08em;text-transform:uppercase;color:#8c6e79;}
    #dashScreen .dash-v184-stat strong{font-size:.98rem;color:#60464f;}
    ...
    #dashScreen .dash-playbox{gap:10px;justify-items:stretch;}
    #dashScreen .dash-track{width:100%;max-width:none;min-height:0;}
    #dashScreen .dash-bg{filter:none!important;opacity:1!important;}
    #dashScreen .dash-overlay{background:transparent!important;padding:30px 14px 16px!important;display:grid!important;align-items:end!important;justify-items:center!important;align-content:end!important;}
    #dashScreen .dash-overlay-card{width:min(76%,308px);max-width:308px;gap:8px;padding:12px 14px 11px;background:rgba(255,250,246,.96);box-shadow:0 6px 0 rgba(116,78,76,.16);}
    ...
    #dashScreen .dash-overlay-cost{display:none!important;}
    #dashScreen .dash-cost-below-preview{display:flex;align-items:center;justify-content:center;gap:6px;white-space:nowrap;width:max-content;max-width:100%;margin:0 auto 2px;padding:6px 12px;border-radius:999px;background:rgba(255,250,246,.97);border:1px solid rgba(140,98,93,.12);font-size:.76rem;font-weight:900;color:#71545f;box-shadow:0 2px 0 rgba(116,78,76,.08);}
    #dashScreen .dash-cost-below-preview img{width:18px;height:18px;object-fit:contain;image-rendering:pixelated;}
  ...
  function getBestCoins(){
    return Math.max(0,Math.floor(Number(localStorage.getItem(BEST_COINS_KEY))||0));
  }
  ...
  function updateBestCoinsDisplay(){
    ...
    const out=screen.querySelector('#dashBestCoinsV184');
  ...
  function ensureStartNote(screen){
    ...
    if(!titleCol) return;
    titleCol.querySelectorAll('small:not(.dash-v184-start-note)').forEach(el=>el.remove());
    let note=titleCol.querySelector('.dash-v184-start-note');
    if(!note){
      note=document.createElement('small');
      note.className='dash-v184-start-note';
  ...
  function ensureSummary(screen,shell,stageSection){
    let summary=screen.querySelector('#dashSummaryV184');
    if(!summary){
      summary=document.createElement('div');
      summary.id='dashSummaryV184';
      summary.className='dash-v184-summary';
      summary.innerHTML=`
        <div class="dash-v184-stat"><span>High Score</span><strong id="dashBestCoinsV184">0 Coins</strong></div>
        <div class="dash-v184-stat"><span>Time</span><strong>30 Seconds</strong></div>`;
    }
  ...
  function renderOcPicker(screen){
    ...
    const label=screen.querySelector('#dashRunOcNameV184');
    if(label) label.textContent=OC_NAMES[dashRunOc]||'Peep';
    window.DUCKIE_DASH_RUN_OC=dashRunOc;

    const picker=screen.querySelector('#dashOcPickerV184');
    if(!picker) return;
    picker.innerHTML='';
    ...
    if(!selectors){
      ...
      selectors.innerHTML=`
        ...
        <button id="dashOcSelector" class="dash-selector-card" type="button" aria-expanded="false">
          <span>Selected OC</span><strong id="dashRunOcNameV184">Peep</strong>
        </button>
        <div id="dashOcPickerV184" class="dash-oc-picker" role="menu" aria-label="Choose OC for this Duckie Dash run"></div>`;
    }else{
      const oldName=selectors.querySelector('#dashRunOcNameV183');
      if(oldName) oldName.id='dashRunOcNameV184';
      const oldPicker=selectors.querySelector('#dashOcPickerV183');
      if(oldPicker) oldPicker.id='dashOcPickerV184';
      ...
      if(oldOc && oldOc.tagName!=='BUTTON'){
        ...
        replacement.innerHTML='<span>Selected OC</span><strong id="dashRunOcNameV184">Peep</strong>';
        oldOc.replaceWith(replacement);
      }
      if(!selectors.querySelector('#dashOcPickerV184')){
        const picker=document.createElement('div');
        picker.id='dashOcPickerV184';
  ...
  function ensureOverlayAndCost(screen,shell,playbox){
    ...
    // Remove/retire old inside-overlay cost pill and place it below the preview window instead.
    overlay.querySelector('.dash-overlay-cost')?.remove();
    let cost=screen.querySelector('#dashCostBelowPreviewV184');
    if(!cost){
      cost=document.createElement('div');
      cost.id='dashCostBelowPreviewV184';
      cost.className='dash-cost-below-preview';
      cost.innerHTML='Cost: 3 Tokens <img src="assets/dash/Jump-Token.png" alt="">';
    }
    if(playbox && cost.parentElement!==shell){
      shell.insertBefore(cost, playbox.nextSibling);
    }else if(playbox && playbox.nextElementSibling!==cost){
      shell.insertBefore(cost, playbox.nextSibling);
    }
  }
  ...
  function wireSelectors(screen){
    ...
    if(cartButton && garageWrap && cartButton.dataset.v184Bound!=='1'){
      cartButton.dataset.v184Bound='1';
      const sync=()=>{
      ...
      cartButton.addEventListener('click',()=>{
        ...
        screen.querySelector('#dashOcPickerV184')?.classList.remove('open');
        screen.querySelector('#dashOcSelector')?.classList.remove('is-open');
        screen.querySelector('#dashOcSelector')?.setAttribute('aria-expanded','false');
        sync();
      });
    ...
    const picker=screen.querySelector('#dashOcPickerV184');
    if(ocButton && picker && ocButton.dataset.v184Bound!=='1'){
      ocButton.dataset.v184Bound='1';
  ...
  function applyDashLayout(){
    ...
    // Stable compact order: title -> course -> summary -> selectors -> optional cart garage -> stage window -> cost.
    ...
    ensureOverlayAndCost(screen,shell,playbox);
  ...
  if(typeof originalShowScreen==='function' && !originalShowScreen.__dashV184Wrapped){
    ...
    wrapped.__dashV184Wrapped=true;

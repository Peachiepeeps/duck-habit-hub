...
const ui = {
  ...
  runConfirm: document.querySelector("#runConfirm"),
  runConfirmText: document.querySelector("#runConfirmText"),
  cancelRunConfirm: document.querySelector("#cancelRunConfirm"),
  confirmRunButton: document.querySelector("#confirmRunButton"),
...
let pendingRunRank = null;

function openRunConfirm(rank = selectedRank) {
  pendingRunRank = Math.max(1, Math.min(questSave.unlockedRank, Number(rank) || selectedRank));
  ui.runConfirmText.textContent = `Start Rank ${pendingRunRank}?`;
  ui.runConfirm.classList.remove("hidden");
  ui.runConfirm.setAttribute("aria-hidden", "false");
}

function closeRunConfirm() {
  pendingRunRank = null;
  ui.runConfirm.classList.add("hidden");
  ui.runConfirm.setAttribute("aria-hidden", "true");
}

function confirmRunStart() {
  if (pendingRunRank == null) return;
  selectedRank = pendingRunRank;
  closeRunConfirm();
  beginRun();
}
...
document.querySelector("#startRun").addEventListener("click",()=>openRunConfirm(selectedRank));
...
document.querySelector("#runAgain").addEventListener("click",()=>openRunConfirm(currentRun?.rank||selectedRank));
document.querySelector("#backToQuest").addEventListener("click",returnHome);
ui.cancelRunConfirm.addEventListener("click",closeRunConfirm);
ui.confirmRunButton.addEventListener("click",confirmRunStart);

...
function applyMushroomVariant(template,rank){
  ...
  return {...template,name:v.name,idle:v.idle,hurt:v.hurt,hp:Math.round(template.hp*v.hpMultiplier),attack:Math.round(template.attack*v.attackMultiplier),exp:Math.round(template.exp*v.expMultiplier),coinMin:Math.round(template.coinMin*v.coinMultiplier),coinMax:Math.round(template.coinMax*v.coinMultiplier),mushroomVariant:v.id,healPercent:Number(v.healPercent)||0,maxHeals:Number(v.maxHeals)||0,healChance:Number(v.healChance)||0};
}
...
function startEnemy(enemyId) {
  ...
  const template=enemyId==="mushroom-cat" ? applyMushroomVariant(baseTemplate,currentRun.rank) : baseTemplate;
  currentEnemy=enemyScaled(template,currentRun.rank);
  currentEnemy.id=enemyId;
  currentEnemy.healsUsed=0;
  ui.enemyCombatant.classList.remove("hidden");
  ui.enemySprite.classList.remove("boss-fighter","gold-boss-fighter");
...
async function enemyTurn() {
  ...
  const canGreenHeal=currentEnemy.mushroomVariant==="green" && currentEnemy.healPercent>0 && currentEnemy.hpNow<currentEnemy.maxHp && Number(currentEnemy.healsUsed||0)<Number(currentEnemy.maxHeals||0) && currentEnemy.hpNow<=currentEnemy.maxHp*.72 && Math.random()<Number(currentEnemy.healChance||0);
  if(canGreenHeal){
    const amount=Math.max(1,Math.round(currentEnemy.maxHp*currentEnemy.healPercent));
    const before=currentEnemy.hpNow; currentEnemy.hpNow=Math.min(currentEnemy.maxHp,currentEnemy.hpNow+amount); currentEnemy.healsUsed=Number(currentEnemy.healsUsed||0)+1;
    const gained=currentEnemy.hpNow-before; setMessage(`${currentEnemy.name} restores some of its HP!`); ui.enemySprite.classList.add("attack-pop"); showFloat(`+${gained}`,"heal","enemy"); renderEnemyHp(); await sleep(520); ui.enemySprite.classList.remove("attack-pop"); setMessage("Peep is ready!"); return;
  }

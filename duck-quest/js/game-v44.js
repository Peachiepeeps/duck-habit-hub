...
const BUDDY_ASSIGN_CHARACTER_NAMES = Object.freeze({peep:"Peep",miko:"Miko",io:"Io"});
...
function buddyEquippedCount(key,excludeCharacterId="",excludeSlotIndex=-1){
  ...
  ensureBuddySave();
  let count=0;
  for(const characterId of ["peep","miko","io"]){
...
function beginEndlessRun(startFloor=1){
  ...
  currentRun={
    ...
    hp:stats.maxHp,maxHp:stats.maxHp,coinsEarned:0,expEarned:0,itemsEarned:[],iconBackgroundsEarned:[],wallpapersEarned:[],levelBackgroundsEarned:[],levelsGained:[],bossWon:false
  };
  persistAll();
  showScreen("battle");
  startEncounter();
...
function renderResult(won) {
  ...
  if(!(currentRun?.itemsEarned||[]).length && !(currentRun?.iconBackgroundsEarned||[]).length && !(currentRun?.wallpapersEarned||[]).length){

...
function weightedOceanVariant(rank, ids) {
  ...
  let weights;
  if(r<=16) weights=[100,0,0,0,0];
  else if(r<=30) weights=[60,40,0,0,0];
  else if(r<=48) weights=[20,50,30,0,0];
  else if(r<=70) weights=[0,25,35,40,0];
  else if(r<=88) weights=[0,0,25,50,25];
...
function applySeaStarVariant(t,r){return applyProgressVariant(t,SEA_STAR_VARIANTS,["base","blue","green","pink","purple"],r,"seaStarVariant",100);}
...
function applyStarMouseVariant(t,r){return applyProgressVariant(t,STAR_MOUSE_VARIANTS,["base","blue","mint","pink","purple"],r,"starMouseVariant",150);}
...
function applyPuffFairyVariant(t,r){return applyProgressVariant(t,PUFF_FAIRY_VARIANTS,["base","pink","purple","gold","dark"],r,"puffFairyVariant",150);}
...
function applyTulipaVariant(t,r){return applyProgressVariant(t,TULIPA_VARIANTS,["base","yellow","orange","purple","dark"],r,"tulipaVariant",150);}
...
function applySnoudVariant(t,r){return applyProgressVariant(t,SNOUD_VARIANTS,["base","green","grey","purple","gold"],r,"snoudVariant",150);}
...
function applyCloudBunnyVariant(t,r){return applyProgressVariant(t,CLOUD_BUNNY_VARIANTS,["base","grey","blue","pink","black"],r,"cloudBunnyVariant",150);}
...
function applyLunarMothVariant(t,r){return applyProgressVariant(t,LUNAR_MOTH_VARIANTS,["base","blue","mint","pink","golden"],r,"lunarMothVariant",150);}
...
function applyAriesVariant(t,r){return applyProgressVariant(t,ARIES_VARIANTS,["base","blue","pink","purple","gold","galaxy"],r,"ariesVariant",150);}
...
const ENDLESS_BOSSES = Object.freeze(["mushroom-cat","tree-squirrel","vampire-squid","jellybun","aries","cherub-duck"]);
...
function enemyVariantId(enemy) {
  ...
  return String(
    ...
    enemy.catterpillarVariant || enemy.seaStarVariant || enemy.starMouseVariant || enemy.puffFairyVariant || enemy.tulipaVariant || enemy.snoudVariant || enemy.cloudBunnyVariant || enemy.lunarMothVariant || enemy.ariesVariant || enemy.cherubDuckVariant || "base"
  );
}
...
function normalizeCharacterQuestProgress(raw, legacy={}){
  ...
  const meadowUnlocked=clampInt(q.areas?.meadow?.unlockedRank ?? legacyAreas.meadow?.unlockedRank ?? legacy.unlockedRank,1,40,1);
  const meadowLast=clampInt(q.areas?.meadow?.lastRank ?? legacyAreas.meadow?.lastRank ?? legacy.lastRank,1,meadowUnlocked,1);
  const oceanUnlocked=clampInt(q.areas?.ocean?.unlockedRank ?? legacyAreas.ocean?.unlockedRank,1,100,1);
  const oceanLast=clampInt(q.areas?.ocean?.lastRank ?? legacyAreas.ocean?.lastRank,1,oceanUnlocked,1);
  const cloudUnlocked=clampInt(q.areas?.cloud?.unlockedRank ?? legacyAreas.cloud?.unlockedRank,1,150,1);
  const cloudLast=clampInt(q.areas?.cloud?.lastRank ?? legacyAreas.cloud?.lastRank,1,cloudUnlocked,1);
...
function hasUniversalClosetReward(itemId){
  const unlocked=hubSave.characterUnlockedItems;
  return Boolean(itemId && unlocked && QUEST_CHARACTER_IDS.every(id=>Array.isArray(unlocked[id]) && unlocked[id].includes(itemId)));
}

function unlockUniversalClosetReward(itemId){
  if(!itemId) return false;
  if(!hubSave.characterUnlockedItems || typeof hubSave.characterUnlockedItems!=="object") hubSave.characterUnlockedItems={};
  let changed=false;
  for(const id of QUEST_CHARACTER_IDS){
    const list=Array.isArray(hubSave.characterUnlockedItems[id]) ? hubSave.characterUnlockedItems[id] : [];
    if(!list.includes(itemId)){ list.push(itemId); changed=true; }
    hubSave.characterUnlockedItems[id]=[...new Set(list)];
  }
  hubSave.unlockedItems=[...new Set([...(Array.isArray(hubSave.unlockedItems)?hubSave.unlockedItems:[]),itemId])];
  return changed;
}
...
function warmQuestMenuAssets(){
  const run=()=>{
    ...
    AREA_CONFIG && Object.values(AREA_CONFIG).forEach(area=>(area.backgrounds||[]).forEach(warmQuestImage));
    ENDLESS_BACKGROUNDS.forEach(warmQuestImage);
...
function rankWord(rank) {
  ...
  const n=Math.max(1,Math.min(150,Math.round(Number(rank)||1)));
...
function rankDescription(rank,areaId=selectedArea) {
  if(areaId==="cloud"){
    ...
    if(rank<=30) return `Cloud Garden Rank ${rank} · New celestial colors are drifting in.`;
    if(rank<=70) return `Cloud Garden Rank ${rank} · The garden is growing stronger.`;
    if(rank<=110) return `Cloud Garden Rank ${rank} · Elite sky creatures are appearing.`;
    if(rank<150) return `Cloud Garden Rank ${rank} · The Star Sanctuary is close.`;
    return "Cloud Garden Rank 150 · Clear the sanctuary to earn the Halo for every OC!";
  }
  if(areaId==="ocean"){
    if(rank<=1) return "Ocean Rank 1 · The shore is calm... for now.";
    if(rank<=20) return `Ocean Rank ${rank} · New colors are beginning to appear.`;
    if(rank<=48) return `Ocean Rank ${rank} · Special enemy moves are joining the tide.`;
    if(rank<=70) return `Ocean Rank ${rank} · Deep-water enemies are much tougher.`;
    if(rank<=88) return `Ocean Rank ${rank} · Elite colors are beginning to surface.`;
    return rank>=100 ? "Ocean Rank 100 · Clear the route to earn Sunglasses for every OC!" : `Ocean Rank ${rank} · The deepest water is extremely dangerous.`;
  }
  ...
  return rank>=40 ? "Rank 40 · Clear the meadow to earn the Daisy Crown for every OC!" : `Rank ${rank} · High-risk, high-reward territory.`;
}
...
function endlessEffectiveRank(floor){
  return Math.max(1,Math.min(150,1+Math.floor((Math.max(1,floor)-1)/2)));
}
...
function endRun(won) {
  ...
  if(won){
    ...
    if(areaReward && currentRun.rank>=maxRank && !hasUniversalClosetReward(areaReward.id)){
      if(unlockUniversalClosetReward(areaReward.id)){
        if(!Array.isArray(currentRun.closetRewardsEarned)) currentRun.closetRewardsEarned=[];
        currentRun.closetRewardsEarned.push({name:`${areaReward.name} · Unlocked for every OC!`,image:areaReward.image});
      }
    }
  }

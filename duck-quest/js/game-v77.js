...
function updateEncounterHeader(encounter=currentEncounterData()){
  ...
  if(encounter.type==="double-enemy"){
    const total=Math.max(2,Math.floor(Number(encounter.enemyIds?.length)||2));
    const wave=Math.min(total,Math.max(1,Math.floor(Number(encounter.waveIndex)||0)+1));
    ui.encounterLabel.textContent += ` · DOUBLE ${wave}/${total}`;
...
function startEncounter() {
  ...
  if(encounter.type==="double-enemy"){
    if(!Array.isArray(encounter.enemyIds) || !encounter.enemyIds.length) encounter.enemyIds=pickEncounterEnemies(isEndless?ENDLESS_NORMAL_ENEMIES:["bee","cat-slime"],2);
    if(!Number.isFinite(encounter.waveIndex)) encounter.waveIndex=0;
    if(!Array.isArray(encounter.defeatedEnemies)) encounter.defeatedEnemies=[];
    const introWave=Math.max(1,Math.floor(Number(encounter.waveIndex)||0)+1);
    setMessage(introWave===1 ? "Double Trouble! Two enemies are waiting this encounter." : "The next enemy jumps in!");
    startEnemy(encounter.enemyIds[Math.max(0,Math.floor(Number(encounter.waveIndex)||0))]);
    return;
  }
...
async function enemyDefeated(options={}) {
  ...
  const isDouble=encounter?.type==="double-enemy";
  if(isDouble){
    if(!Array.isArray(encounter.defeatedEnemies)) encounter.defeatedEnemies=[];
    encounter.defeatedEnemies.push(defeated);
    const total=Math.max(2,Math.floor(Number(encounter.enemyIds?.length)||2));
    const currentWave=Math.max(0,Math.floor(Number(encounter.waveIndex)||0));
    if(currentWave<total-1){
      encounter.waveIndex=currentWave+1;
      actionLocked=false;
      updateEncounterHeader(encounter);
      setMessage(`${defeated.name}${defeated.shiny?" ✨":""} was defeated! Another enemy jumps in!`);
      await sleep(360);
      startEnemy(encounter.enemyIds[encounter.waveIndex]);
      return;
    }
  }

...
function aliveDoubleEnemies(){
  return doubleBattleEnemies.filter(enemy=>enemy && enemy.hpNow>0 && !enemy._defeated);
}
...
function closeTargetPicker(){
  ...
  if(!actionLocked && currentEnemy) ui.commandGrid?.classList.remove("hidden");
}
...
function openDoubleTargetPicker(mode="attack"){
  const targets=aliveDoubleEnemies();
  if(targets.length<=1){
    if(targets[0]) selectCurrentEnemy(targets[0]);
    openCommandWindow(mode==="attack"?"attack":mode);
    return;
  }
  pendingTargetMode=mode;
  closeCommandWindow();
  ui.commandGrid?.classList.add("hidden");
  if(ui.targetPickerTitle) ui.targetPickerTitle.textContent=mode==="attack"?"Choose who to attack":"Choose a target";
  if(ui.targetPickerGrid){
    ...
    for(const enemy of targets){
      ...
      button.addEventListener("click",()=>{
        ...
        if(nextMode==="attack") openCommandWindow("attack");
      });
...
function startEncounter() {
  ...
  if(encounter.type==="lucky-picnic"){
    ...
    ui.commandGrid.classList.add("hidden");
    closeCommandWindow();
    showChest({kind:"event", eventType:"lucky-picnic", captionText:"Lucky Picnic! Your buddy found a cozy snack spot.", buttonLabel:"Enjoy Picnic"});
    setMessage("A Lucky Picnic appeared! Time for a cozy reward break.");
    return;
  }
  ...
  if(encounter.type==="wishing-fountain"){
    ...
    ui.commandGrid.classList.add("hidden");
    closeCommandWindow();
    showChest({kind:"event", eventType:"wishing-fountain", captionText:"Wishing Fountain! Toss in a wish and see what happens.", buttonLabel:"Make a Wish"});
    setMessage("You found a Wishing Fountain! Maybe luck is on your side.");
    return;
  }
...
async function openPendingChest() {
  ...
  if(pendingChest.eventType){
    ...
    setMessage(pendingChest.eventType==="lucky-picnic"
      ? "You and your buddy enjoyed a Lucky Picnic!"
      : "The Wishing Fountain granted a little bit of luck!");
  ...
  ui.chestCaption.textContent=pendingChest.hiddenTreasure
    ...
    : pendingChest.eventType
      ? `${situationTitle(pendingChest.eventType)}! · ${textParts.join(" · ")}`
  ...
  if(!pendingChest.hiddenTreasure){
    if(pendingChest.eventType){
      setMessage(`${situationTitle(pendingChest.eventType)} rewards collected!`);
...
function generateRewards(chest) {
  ...
  if(chest.eventType==="lucky-picnic"){
    coins=randInt(10,18)+Math.max(1,Math.round(rank*1.1));
    exp=randInt(7,12)+Math.max(1,Math.round(rank*.75));
    addRewardItem(items,weightedRewardItemByIds(["bread-loaf","apple","flower","thread","yarn"]),1);
    if(Math.random()<.35) addRewardItem(items,weightedRewardItemByIds(["bread-loaf","apple","flower"]),1);
    return finalizeCharmRewards({
      coins,exp,happiness:chestHappinessReward(chest),items,
      iconBackground:Math.random()<.10 ? pickChestIconBackground({kind:"normal"}) : null,
      iconBorderStyle:null,
      iconBorderColor:Math.random()<.12 ? pickChestIconBorderColor({kind:"normal"}) : null,
      wallpaper:null
    },chest);
  }

  if(chest.eventType==="wishing-fountain"){
    coins=randInt(16,28)+Math.max(2,Math.round(rank*1.4));
    exp=randInt(9,15)+Math.max(1,Math.round(rank*.9));
    addRewardItem(items,weightedRewardItemByIds(["glitter","pink-paint","mint-paint","aqua-paint","buddy-pon","super-buddy-pon"]),1);
    if(Math.random()<.28) addRewardItem(items,weightedRewardItemByIds(["glitter","pink-paint","mint-paint","aqua-paint","gold-paint"]),1);
    return finalizeCharmRewards({
      coins,exp,happiness:chestHappinessReward(chest),items,
      iconBackground:Math.random()<.16 ? pickChestIconBackground({kind:"rare"}) : null,
      iconBorderStyle:Math.random()<.08 ? pickChestIconBorderStyle({kind:"rare"}) : null,
      iconBorderColor:Math.random()<.22 ? pickChestIconBorderColor({kind:"rare"}) : null,
      wallpaper:Math.random()<.08 ? pickChestWallpaper({kind:"rare"}) : null
    },chest);
  }
...
ui.attackMenuButton.addEventListener("click",()=>{
  if(isDoubleBattleActive() && aliveDoubleEnemies().length>1) openDoubleTargetPicker("attack");
  else openCommandWindow("attack");
});

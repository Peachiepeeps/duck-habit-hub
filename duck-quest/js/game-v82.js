...
(function(){
    ...
    if(battle && !document.querySelector('#dashScreen')){
      battle.insertAdjacentHTML('beforebegin',`
        ...
        <section id="hatcheryScreen" class="screen hidden">
          <div class="dq-feature-shell">
            <div class="dq-feature-top"><div><span class="mini-label">HATCHING AREA</span><h2>Buddy Egg Hatchery</h2><small>Keep as many eggs as you find, but incubate only three at once.</small></div><button id="hatchBack" class="pixel-button" type="button">← Quest Menu</button></div>
            <div class="dq-mini-grid"><div><span>Common Eggs</span><strong id="commonEggQty">0</strong></div><div><span>Rare Eggs</span><strong id="rareEggQty">0</strong></div><div><span>Ready</span><strong id="hatchReadyQty">0</strong></div></div>
            <div class="hatch-bg"><div><span class="mini-label">EGG INVENTORY</span><div id="eggInventoryGrid" class="egg-grid"></div></div><div><span class="mini-label">INCUBATORS</span><div id="incubatorGrid" class="incubator-grid"></div></div><div><span class="mini-label">LATEST HATCH</span><div id="hatchLog" class="hatch-log">No eggs have hatched yet.</div></div></div>
          </div>
        </section>
  ...
  function startEgg(type){const def=eggDefinition(type),slot=emptyIncubator();if(slot<0){setMessage('All three incubators are busy!');return;}if(!useInventory(`${type}-egg`,1)){setMessage(`You do not have a ${def.name}.`);return;}const now=Date.now();questSave.eggs.incubators[slot]={type,startAt:now,endAt:now+def.ms};persistAll();renderHatchery();refreshFeatureBadges();}
  function baseEggCandidates(type){
    const normals=BUDDY_CATALOG.filter(e=>!e.shiny&&e.enemyId!=='mimic');
    if(type==='common')return normals.filter(e=>!e.boss);
    const bosses=normals.filter(e=>e.boss),nonboss=normals.filter(e=>!e.boss);
    return Math.random()<.65?bosses:nonboss;
  }
  ...
  async function hatchSlot(index,img){const slot=questSave.eggs.incubators[index];if(!slot||Date.now()<slot.endAt)return;if(img){img.classList.add('egg-shake');await sleep(680);}const entry=rollEggBuddy(slot.type);if(!entry)return;const owned=grantCatalogBuddy(entry);const egg=eggDefinition(slot.type);questSave.eggs.incubators[index]=null;questSave.eggs.lastHatch={name:entry.name,image:entry.image,shiny:Boolean(entry.shiny),boss:Boolean(entry.boss),eggName:egg.name,quantity:owned.quantity};persistAll();renderHatchery();refreshFeatureBadges();renderBuddyHomeCount();}
  function renderHatchery(){ensureExpansionSave();const inv=document.querySelector('#eggInventoryGrid');if(inv){inv.innerHTML='';['common','rare'].forEach(type=>{const d=eggDefinition(type),qty=specialQty(`${type}-egg`);const card=document.createElement('div');card.className='egg-card';card.innerHTML=`<img src="${d.image}" alt="${d.name}"><strong>${d.name}</strong><small>Owned ×${qty}<br>${type==='common'?'1 hour · No bosses · Normal Shiny chance':'24 hours · Bosses favored · 1 / 100 Shiny chance'}</small>`;const b=document.createElement('button');b.type='button';b.className='pixel-button';b.textContent=qty?`Incubate`:'None Owned';b.disabled=!qty||emptyIncubator()<0;b.addEventListener('click',()=>startEgg(type));card.appendChild(b);inv.appendChild(card);});}
    const box=document.querySelector('#incubatorGrid');if(box){box.innerHTML='';questSave.eggs.incubators.forEach((slot,i)=>{const card=document.createElement('div');card.className='incubator-card';if(!slot){card.innerHTML=`<img src="assets/eggs/Common-egg.png" alt=""><strong>Incubator ${i+1}</strong><small>Empty</small>`;}else{const d=eggDefinition(slot.type),ready=Date.now()>=slot.endAt;card.innerHTML=`<img src="${d.image}" alt="${d.name}"><strong>Incubator ${i+1}</strong><small>${ready?'Ready to hatch!':fmtRemaining(slot.endAt-Date.now())}</small>`;const b=document.createElement('button');b.type='button';b.className=`pixel-button${ready?' primary':''}`;b.textContent=ready?'Hatch!':'Incubating';b.disabled=!ready;b.addEventListener('click',()=>hatchSlot(i,card.querySelector('img')));card.appendChild(b);}box.appendChild(card);});}
    const log=document.querySelector('#hatchLog');if(log){const h=questSave.eggs.lastHatch;log.innerHTML=h?`<div style="display:flex;align-items:center;gap:9px"><img src="${h.image}" alt="" style="width:54px;height:54px;object-fit:contain;image-rendering:pixelated"><div><strong>${h.name}${h.shiny?' ✨':''}</strong><br><small>${h.eggName} hatched${h.boss?' a Boss Buddy':''}! Owned ×${h.quantity}</small></div></div>`:'No eggs have hatched yet.';}
    refreshFeatureBadges();
  }
  ...
  function showMerchantEvent(){currentEnemy=null;ui.enemyCombatant.classList.add('hidden');ui.enemyCombatant2?.classList.add('hidden');ui.commandGrid.classList.add('hidden');closeCommandWindow();pendingChest={kind:'event',eventType:'mysterious-merchant',wares:merchantStock()};showChest({kind:'event',eventType:'mysterious-merchant',captionText:'Mysterious Merchant! Gentleman Duck has a tiny roadside shop.'});ui.chestSprite.src='../assets/ducks/Standard-duck.webp';ui.chestSprite.classList.add('event-scene-art','merchant-duck');ui.openChest.classList.add('hidden');setMessage('Gentleman Duck offers two completely random items today.');renderMerchantButtons();}

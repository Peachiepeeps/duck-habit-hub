...
(function(){
  ...
  const SIBLING_SPRITES={
    miko:{
      ...
      angry:'../assets/miko/Miko-angry.PNG'
    },
    miho:{
  ...
  ENEMIES['sibling-miko']={name:'Miko',hp:38,attack:7,exp:46,coinMin:15,coinMax:24,idle:[SIBLING_SPRITES.miko.neutral,SIBLING_SPRITES.miko.neutral],hurt:SIBLING_SPRITES.miko.angry,speed:520,uncatchable:true,siblingCharacterId:'miko'};
  ENEMIES['sibling-miho']={name:'Miho',hp:40,attack:8,exp:46,coinMin:15,coinMax:24,idle:[SIBLING_SPRITES.miho.neutral,SIBLING_SPRITES.miho.neutral],hurt:SIBLING_SPRITES.miho.fluster,speed:520,uncatchable:true,siblingCharacterId:'miho'};
      ...
      battle.insertAdjacentHTML('beforebegin',`
        <section id="dashScreen" class="screen hidden">
          <div class="dq-feature-shell">
            <div class="dq-feature-top"><div><span class="mini-label">DUCKIE DASH</span><h2>Duckie Dash</h2><small>Tap the course to jump. Survive 30 seconds to reach the boss-stage finish!</small></div><button id="dashBack" class="pixel-button" type="button">← Quest Menu</button></div>
  ...
  // ----- Duckie Dash -----
  const dash={running:false,raf:0,last:0,elapsed:0,hearts:3,coins:0,tiny:0,jumpY:0,jumpV:0,obstacles:[],pickups:[],obstacleClock:0,coinClock:0,segment:-1,invuln:0};
  ...
  function dashJump(){if(!dash.running)return;if(dash.jumpY<=1)dash.jumpV=510;}
  function spawnDashThing(kind){const track=dashTrack();if(!track)return;const cfg=DASH_STAGE[questSave.dash.selectedStage];const el=document.createElement('img');el.className=`dash-thing ${kind==='obstacle'?'dash-obstacle':kind==='tiny'?'dash-tiny-duck':'dash-coin'}`;el.src=kind==='obstacle'?cfg.obstacle:kind==='tiny'?'../assets/ducks/Tiny-duck.webp':'../assets/ui/pink-coin.webp';el.alt='';track.appendChild(el);const raise=kind==='coin'?(Math.random()<.35?62:20):kind==='tiny'?45:8;const obj={kind,x:track.clientWidth+40,raise,el,hit:false};(kind==='obstacle'?dash.obstacles:dash.pickups).push(obj);}
  ...
  function dashFrame(ts){if(!dash.running)return;if(!dash.last)dash.last=ts;const dt=Math.min(.04,(ts-dash.last)/1000);dash.last=ts;dash.elapsed+=dt;dash.invuln=Math.max(0,dash.invuln-dt);dash.obstacleClock+=dt;dash.coinClock+=dt;changeDashSegment();if(dash.elapsed>=30){finishDash(true);return;}if(dash.obstacleClock>1.75){dash.obstacleClock=0;spawnDashThing('obstacle')}if(dash.coinClock>.9){dash.coinClock=0;spawnDashThing('coin')}
    dash.jumpV-=1450*dt;dash.jumpY=Math.max(0,dash.jumpY+dash.jumpV*dt);if(dash.jumpY<=0&&dash.jumpV<0){dash.jumpY=0;dash.jumpV=0}const runner=document.querySelector('#dashRunner');if(runner)runner.style.transform=`translateY(${-dash.jumpY}px)`;const track=dashTrack();if(!track){finishDash(false);return;}const W=track.clientWidth,H=track.clientHeight,playerX=W*.12,playerW=Math.min(W*.20,92),playerH=playerW*.75,ground=H*.10,playerY=H-ground-playerH-dash.jumpY;const speed=W*.43;
    dash.obstacles=dash.obstacles.filter(o=>{o.x-=speed*dt;o.el.style.left=`${o.x}px`;o.el.style.bottom=`${o.raise}px`;const ow=Math.min(W*.13,62),oh=ow;if(!o.hit&&dash.invuln<=0&&overlap(playerX,playerY,playerW*.72,playerH*.75,o.x,H-ground-oh-o.raise,ow*.72,oh*.72)){o.hit=true;dash.hearts--;dash.invuln=1.0;runner?.classList.add('hit');setTimeout(()=>runner?.classList.remove('hit'),350);dashHud();if(dash.hearts<=0){finishDash(false);return false}}if(o.x<-80){o.el.remove();return false}return true});
    dash.pickups=dash.pickups.filter(o=>{o.x-=speed*dt;o.el.style.left=`${o.x}px`;o.el.style.bottom=`${o.raise}px`;const iw=o.kind==='tiny'?Math.min(W*.10,44):Math.min(W*.08,34),ih=iw;if(overlap(playerX,playerY,playerW*.72,playerH*.75,o.x,H-ground-ih-o.raise,iw,ih)){if(o.kind==='tiny'){dash.tiny++;recordTinyDuck()}else dash.coins++;o.el.remove();dashHud();return false}if(o.x<-60){o.el.remove();return false}return true});dashHud();dash.raf=requestAnimationFrame(dashFrame)}
  function startDash(){if(dash.running)return;ensureExpansionSave();if(questSave.dash.freeRuns>0)questSave.dash.freeRuns--;else if(!useInventory('jump-token',DASH_COST)){setMessage('Duckie Dash costs 3 Jump Tokens.');refreshFeatureBadges();return;}clearDashObjects();Object.assign(dash,{running:true,last:0,elapsed:0,hearts:3,coins:0,tiny:0,jumpY:0,jumpV:0,obstacleClock:0,coinClock:0,segment:-1,invuln:0});const ov=document.querySelector('#dashOverlay');ov?.classList.add('hidden');persistAll();refreshFeatureBadges();resetDashPreview();dash.raf=requestAnimationFrame(dashFrame)}
  ...
  function showMerchantEvent(){currentEnemy=null;ui.enemyCombatant.classList.add('hidden');ui.enemyCombatant2?.classList.add('hidden');ui.commandGrid.classList.add('hidden');closeCommandWindow();pendingChest={kind:'event',eventType:'mysterious-merchant',wares:merchantStock()};showChest({kind:'event',eventType:'mysterious-merchant',captionText:'Mysterious Merchant! Gentleman Duck has a tiny roadside shop.'});ui.chestSprite.src='../assets/ducks/Top-hat-duck.webp';ui.chestSprite.alt='Gentleman Duck';ui.chestSprite.classList.add('event-scene-art','merchant-duck');ui.openChest.classList.add('hidden');setMessage('Gentleman Duck offers two completely random items today.');renderMerchantButtons();}
  function renderMerchantButtons(){const holder=ui.eventChoiceActions;if(!holder||!pendingChest?.wares)return;holder.innerHTML='';const wrap=document.createElement('div');wrap.className='merchant-wares';pendingChest.wares.forEach(ware=>{const b=document.createElement('button');b.type='button';b.className='pixel-button merchant-ware';b.disabled=Boolean(ware.sold);b.innerHTML=`<img src="${ware.image}" alt=""><span><strong>${ware.name}</strong><small>${ware.detail}</small></span><span class="price">${ware.sold?'Sold':'◉ '+ware.price}</span>`;b.addEventListener('click',()=>{if(ware.sold)return;if(Number(hubSave.coins||0)<ware.price){setMessage('Not enough Pink Coins for that item.');return;}hubSave.coins-=ware.price;if(ware.kind==='charm')charmState().owned[ware.id]=true;else addInventory(ware.id,1);ware.sold=true;persistAll();renderMeta();renderMerchantButtons();refreshFeatureBadges();setMessage(`Bought ${ware.name}!`);});wrap.appendChild(b)});const leave=document.createElement('button');leave.type='button';leave.className='pixel-button';leave.textContent='Leave Merchant';leave.addEventListener('click',()=>finishNonBattleEvent('Gentleman Duck tips his hat and packs up his tiny shop.'));wrap.appendChild(leave);holder.appendChild(wrap);holder.classList.remove('hidden');ui.battleUi?.classList.add('event-mode');}

  // ----- Sibling Spat dialogue + battle -----
  let siblingDialogueIndex=0,siblingDialogueState=null;
  function siblingSprite(who,expression){return SIBLING_SPRITES[who]?.[expression]||SIBLING_SPRITES[who]?.neutral||''}
  function showSiblingEvent(){const script=SIBLING_SCRIPTS[activeCharacterId];if(!script)return;document.querySelector('.peep-combatant')?.classList.add('hidden');currentEnemy=null;ui.enemyCombatant.classList.add('hidden');ui.enemyCombatant2?.classList.add('hidden');ui.buddyCombatant?.classList.add('hidden');ui.commandGrid.classList.add('hidden');closeCommandWindow();ui.chestLayer.classList.add('hidden');const scene=document.querySelector('#siblingScene'),left=document.querySelector('#siblingLeft'),right=document.querySelector('#siblingRight');scene?.classList.remove('hidden');scene?.setAttribute('aria-hidden','false');const leftWho=activeCharacterId,rightWho=activeCharacterId==='miko'?'miho':'miko';siblingDialogueState={script,leftWho,rightWho,leftExpression:'neutral',rightExpression:'neutral'};if(left)left.src=siblingSprite(leftWho,'neutral');if(right)right.src=siblingSprite(rightWho,'neutral');siblingDialogueIndex=0;renderSiblingLine();}
  ...
  const oldShowScreen=showScreen;showScreen=function(which){oldShowScreen(which);document.querySelector('#dashScreen')?.classList.toggle('hidden',which!=='dash');document.querySelector('#hatcheryScreen')?.classList.toggle('hidden',which!=='hatchery');if(which!=='dash'&&dash.running){dash.running=false;cancelAnimationFrame(dash.raf);clearDashObjects()}if(which==='dash'){renderDashSelectors();resetDashPreview()}if(which==='hatchery')renderHatchery()};

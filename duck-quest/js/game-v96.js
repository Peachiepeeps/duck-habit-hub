...
(function(){
  ...
  function spawnDashThing(kind){const track=dashTrack();if(!track)return;const cfg=DASH_STAGE[questSave.dash.selectedStage];const el=document.createElement('img');el.className=`dash-thing ${kind==='obstacle'?'dash-obstacle':kind==='tiny'?'dash-tiny-duck':'dash-coin'}`;el.src=kind==='obstacle'?cfg.obstacle:kind==='tiny'?'../assets/ducks/Tiny-duck.webp':'../assets/ui/pink-coin.webp';el.alt='';track.appendChild(el);const raise=kind==='coin'?(Math.random()<.35?82:38):kind==='tiny'?56:22;const obj={kind,x:track.clientWidth+40,raise,el,hit:false};(kind==='obstacle'?dash.obstacles:dash.pickups).push(obj);}

...
function isLightQuestBorderColor(hex){
  return questBorderRelativeLuminance(hex) >= 0.72;
}

function makeQuestBorderAssistColor(hex){
  const {r,g,b}=questBorderHexToRgb(hex);
  return questBorderRgbToHex(r*0.68,g*0.68,b*0.68);
}
    ...
    source.onload=()=>{
      try{
        ...
        if(wantsLightStitchOutline){
          // v24.157: make the light Stitched outline visibly readable at phone size.
          // The source art is 512px, so a 1px source offset was barely visible after scaling.
          // This uses a symmetric 4px source-space contour: crisp outline, no drop shadow.
          ...
          // Thicken the actual selected stitch color too, so it stays dominant over the outline.
          const stitchOffsets=[
            [-2,0],[2,0],[0,-2],[0,2],
            [-1,-1],[1,-1],[-1,1],[1,1]
          ];
          for(const [dx,dy] of stitchOffsets){
...
const PEEP_SKILLS = [
  ...
  },
  {
    ...
    description: "Peep has had enough. Hits all enemies in Double Trouble."
  },
  {
    id: "activate-zoomies",
...
const IO_SKILLS = [
  ...
  {
    id:"star-shower", name:"Star Shower", unlock:40, type:"multi-hit", multiplier:1.55, hits:4, cooldown:2, hitsAll:true,
    sprite:"assets/characters/io/base/sparkle-shot.webp",
    description:"Four falling stars for 1.55× total damage. Hits all enemies in Double Trouble."
  },
  {
...
const MIHO_SKILLS = [
  ...
  {
    id:"magic", name:"Magic", unlock:10, type:"damage-burn", multiplier:1.0, burnChance:1, burnDamagePercent:0.08, burnTurns:99, hitsAll:true,
    sprite:"assets/characters/miho/base/spell.webp",
    description:"A strong magic spell! Hits all enemies in Double Trouble and burns them, causing HP loss at the end of their turns."
  },
  {
...
const ANNIKA_SKILLS = [
  {
    id:"scold", name:"Scold", unlock:1, type:"damage", multiplier:1.0, hitsAll:true,
    sprite:"assets/characters/annika/base/scold.webp",
    description:"A sharp scolding that deals normal damage. In Double Trouble, Annika scolds everybody."
  },
  {
...
const CATFISH_VARIANTS=Object.freeze({
  ...
  black:{id:"black",name:"Tuxedo Catfish",hp:1.7,atk:1.3,exp:1.5,coin:1.6,elite:true,idle:["assets/enemies/catfish/base/Black-idle-1.webp","assets/enemies/catfish/base/Black-idle-2.webp"],hurt:"assets/enemies/catfish/base/Black-hurt.webp"}
});
...
const AMETHYST_MIMIC_RATE = 1 / 40; // 2.5% per purple Mystery Chest; intentionally rarer
...
function currentEncounterData(){
  return currentRun?.mode==="endless"
    ? currentRun?.endlessEncounter || null
    : currentRun?.plan?.[currentRun?.index || 0] || null;
}

function pickEncounterEnemy(enemyPool){
  return enemyPool[randInt(0,enemyPool.length-1)];
}

function pickEncounterEnemies(enemyPool,count=2){
  const source=Array.isArray(enemyPool)?enemyPool.filter(Boolean):[];
  if(!source.length) return [];
  const picks=[];
  const available=[...new Set(source)];
  while(picks.length<count){
    const pool=(available.length>=count)?available.filter(id=>!picks.includes(id)):source;
    picks.push(pool[randInt(0,pool.length-1)]);
  }
  return picks;
}

function pickSituationType(mode="stage"){
  return weightedPick(mode==="endless"
    ? [{value:"lucky-picnic",weight:5},{value:"wishing-fountain",weight:4}]
    : [{value:"lucky-picnic",weight:6},{value:"wishing-fountain",weight:4}]
  );
}

function buildEncounterFromPool(enemyPool,mode="stage"){
  ...
  if(mode==="endless"){
    ...
    if(roll<.25) return {type:"double-enemy",enemyIds:pickEncounterEnemies(ENDLESS_NORMAL_ENEMIES,2),waveIndex:0,defeatedEnemies:[]};
  ...
  if(roll<.28) return {type:"double-enemy",enemyIds:pickEncounterEnemies(enemyPool,2),waveIndex:0,defeatedEnemies:[]};
  if(roll<.36) return {type:pickSituationType("stage")};
  return {type:"enemy",enemyId:pickEncounterEnemy(enemyPool)};
}

function situationTitle(type){
  if(type==="lucky-picnic") return "Lucky Picnic";
  if(type==="wishing-fountain") return "Wishing Fountain";
  return "Special Event";
}

function updateEncounterHeader(encounter=currentEncounterData()){
  if(!ui.encounterLabel) return;
  ...
  if(!encounter) return;
  if(encounter.type==="double-enemy"){
    ui.encounterLabel.textContent += ` · DOUBLE BATTLE`;
  } else if(encounter.type==="lucky-picnic"){
    ui.encounterLabel.textContent += ' · PICNIC';
  } else if(encounter.type==="wishing-fountain"){
    ui.encounterLabel.textContent += ' · FOUNTAIN';
  }
}
...
const SHINY_VARIANTS = Object.freeze({
  ...
  "sea-turtle": {
    id:"shiny", name:"Shiny Sea Turtle",
    idle:["assets/enemies/sea-turtle/base/Sea-turtle-shiny-idle-1.png","assets/enemies/sea-turtle/base/Sea-turtle-shiny-idle-2.png"],
    hurt:"assets/enemies/sea-turtle/base/Sea-turtle-shiny-hurt.png"
  },
  "catfish": {
...
const ui = {
  ...
  enemyCombatant2: document.querySelector("#enemyCombatant2"),
  enemyName2: document.querySelector("#enemyName2"),
  enemyRank2: document.querySelector("#enemyRank2"),
  enemyHpFill2: document.querySelector("#enemyHpFill2"),
  enemyHpText2: document.querySelector("#enemyHpText2"),
  enemySprite2: document.querySelector("#enemySprite2"),
  peepDamageText: document.querySelector("#peepDamageText"),
  enemyDamageText: document.querySelector("#enemyDamageText"),
  enemyDamageText2: document.querySelector("#enemyDamageText2"),
  ...
  eventChoiceActions: document.querySelector("#eventChoiceActions"),
  battleUi: document.querySelector("#battleScreen .battle-ui"),
  commandStage: document.querySelector("#commandStage"),
  targetPicker: document.querySelector("#targetPicker"),
  targetPickerTitle: document.querySelector("#targetPickerTitle"),
  targetPickerGrid: document.querySelector("#targetPickerGrid"),
...
function setPostFloorLayout(active){
  ui.battleUi?.classList.toggle("reward-mode", Boolean(active));
  ui.commandStage?.classList.toggle("hidden", Boolean(active));
}
...
let doubleBattleEnemies = [];
let pendingTargetMode = "";
let pendingTargetSkill = null;
let pendingCapturePonId = "";
let idleTimer = null;
let secondaryEnemyIdleTimer = null;
...
const ENEMY_EFFECT_KEYS=[
  "enemyAttackDownTurns","enemyAttackMultiplier",
  "enemyDefenseDownTurns","enemyDefenseMultiplier",
  "enemyAccuracyDownTurns","enemyMissChance",
  "enemyBurnTurns","enemyBurnDamagePercent",
  "enemyNextAttackMultiplier","enemyStunned","enemyStunTurns"
];

function enemyEffectDefaults(){
  return {
...
function ensureEnemyEffectSnapshot(enemy){
  if(!enemy) return;
  if(!enemy._battleEffects || typeof enemy._battleEffects!=="object") enemy._battleEffects=enemyEffectDefaults();
  const defaults=enemyEffectDefaults();
  for(const key of ENEMY_EFFECT_KEYS){
    if(enemy._battleEffects[key]===undefined) enemy._battleEffects[key]=defaults[key];
  }
}

function saveCurrentEnemyEffects(enemy=currentEnemy){
  if(!enemy || !skillState) return;
  ensureEnemyEffectSnapshot(enemy);
  for(const key of ENEMY_EFFECT_KEYS) enemy._battleEffects[key]=skillState[key];
}

function loadEnemyEffects(enemy){
  if(!enemy || !skillState) return;
  ensureEnemyEffectSnapshot(enemy);
  for(const key of ENEMY_EFFECT_KEYS) skillState[key]=enemy._battleEffects[key];
}

function enemySlot(enemy=currentEnemy){
  return Number(enemy?._doubleSlot)===1?1:0;
}

function enemyUiFor(enemyOrSlot=currentEnemy){
  const slot=typeof enemyOrSlot==="number"?enemyOrSlot:enemySlot(enemyOrSlot);
  if(slot===1){
    return {
      slot:1,combatant:ui.enemyCombatant2,name:ui.enemyName2,rank:ui.enemyRank2,
      hpFill:ui.enemyHpFill2,hpText:ui.enemyHpText2,sprite:ui.enemySprite2,damageText:ui.enemyDamageText2
    };
  }
  return {
    slot:0,combatant:ui.enemyCombatant,name:ui.enemyName,rank:ui.enemyRank,
    hpFill:ui.enemyHpFill,hpText:ui.enemyHpText,sprite:ui.enemySprite,damageText:ui.enemyDamageText
  };
}

function activeEnemyUi(){ return enemyUiFor(currentEnemy); }

function isDoubleBattleActive(){
  return Boolean(currentRun && currentEncounterData()?.type==="double-enemy" && doubleBattleEnemies.length>1);
}

function aliveDoubleEnemies(){
  return doubleBattleEnemies
    .filter(enemy=>enemy && enemy.hpNow>0 && !enemy._defeated)
    .sort((a,b)=>enemySlot(a)-enemySlot(b));
}

function selectCurrentEnemy(enemy){
  if(!enemy) return;
  if(currentEnemy && currentEnemy!==enemy) saveCurrentEnemyEffects(currentEnemy);
  currentEnemy=enemy;
  loadEnemyEffects(enemy);
  if(isDoubleBattleActive()){
    for(const candidate of doubleBattleEnemies){
      enemyUiFor(candidate).combatant?.classList.toggle("target-selected",candidate===enemy && candidate.hpNow>0);
    }
  }
  renderEnemyHp(enemy);
}

function resetDoubleBattleUi(){
  doubleBattleEnemies=[];
  pendingTargetMode="";
  ui.battlefield?.classList.remove("double-battle");
  ui.enemyCombatant?.classList.remove("target-selected");
  ui.enemyCombatant2?.classList.remove("target-selected");

  // Fully clear the secondary slot so a defeated Double Trouble sprite/name
  // can never flash into the next normal or boss encounter.
  if(ui.enemyCombatant2) ui.enemyCombatant2.classList.add("hidden");
  if(ui.enemyName2) ui.enemyName2.textContent="";
  if(ui.enemyRank2) ui.enemyRank2.textContent="";
  if(ui.enemyHpFill2) ui.enemyHpFill2.style.width="0%";
  if(ui.enemyHpText2) ui.enemyHpText2.textContent="";
  if(ui.enemyDamageText2){
    ui.enemyDamageText2.textContent="";
    ui.enemyDamageText2.classList.add("hidden");
  }
  if(ui.enemySprite2){
    ui.enemySprite2.removeAttribute("src");
    ui.enemySprite2.alt="";
    ui.enemySprite2.className="pixel-sprite fighter enemy-fighter";
  }
  ui.enemyCombatant2?.querySelectorAll(".shiny-arrival-burst").forEach(node=>node.remove());

  closeTargetPicker();
  clearInterval(secondaryEnemyIdleTimer);
  secondaryEnemyIdleTimer=null;
}

function renderEnemyNameFor(enemy,slot=enemySlot(enemy)){
  const refs=enemyUiFor(slot);
  if(!refs.name) return;
  refs.name.textContent="";
  refs.name.append(document.createTextNode(String(enemy?.name || "Enemy")));
  if(enemy?.shiny){
    const sparkle=document.createElement("span");
    sparkle.className="enemy-shiny-sparkle";
    sparkle.textContent=" ✨";
    sparkle.setAttribute("aria-label","Shiny");
    refs.name.append(sparkle);
  }
  if(enemy && buddyOwnedQuantity(buddyKeyForEnemy(enemy))>0){
    ...
    refs.name.append(heart);
  }
}

function renderEnemyCombatant(enemy,slot=enemySlot(enemy)){
  if(!enemy) return;
  enemy._doubleSlot=slot;
  const refs=enemyUiFor(slot);
  // Populate every label/image before showing this combatant. This prevents
  // the HTML placeholder or previous enemy art from flashing on screen.
  refs.combatant?.classList.add("hidden");
  refs.sprite?.removeAttribute("src");
  refs.sprite?.classList.remove("boss-fighter","gold-boss-fighter","queen-bee-fighter","strawberry-slime-fighter","rainbow-flower-fighter","ocean-elite-fighter","ocean-boss-fighter","cream-fox-fighter");refs.sprite?.classList.toggle("sibling-opponent-v230",Boolean(enemy?.siblingCharacterId));ui.battlefield?.classList.toggle("sibling-battle-v230",Boolean(enemy?.siblingCharacterId));
  ...
  if(refs.sprite){
    refs.sprite.classList.toggle("boss-fighter",Boolean(enemy.boss));
    refs.sprite.classList.toggle("gold-boss-fighter",enemy.mushroomVariant==="gold");
    refs.sprite.classList.toggle("queen-bee-fighter",enemy.beeVariant==="queen");
    refs.sprite.classList.toggle("strawberry-slime-fighter",enemy.catSlimeVariant==="strawberry");
    refs.sprite.classList.toggle("rainbow-flower-fighter",enemy.flowerVariant==="rainbow");
    refs.sprite.classList.toggle("ocean-elite-fighter",Boolean(enemy.eliteVariant));
    refs.sprite.classList.toggle("ocean-boss-fighter",enemy.id==="vampire-squid");
    refs.sprite.classList.toggle("cream-fox-fighter",enemy.id==="cream-fox");
    refs.sprite.src=enemy.idle?.[0] || enemy.image || "";
    refs.sprite.alt=enemy.name || "Enemy";
  }
  renderEnemyHp(enemy);
  refs.combatant?.classList.remove("hidden");
}

function startSecondaryEnemyIdle(enemy){
  clearInterval(secondaryEnemyIdleTimer);
  secondaryEnemyIdleTimer=null;
  if(!enemy || enemy.hpNow<=0) return;
  let index=0;
  const refs=enemyUiFor(enemy);
  const frames=Array.isArray(enemy.idle)&&enemy.idle.length?enemy.idle:[enemy.image].filter(Boolean);
  if(!frames.length || !refs.sprite) return;
  refs.sprite.src=frames[0];
  if(frames.length<2) return;
  secondaryEnemyIdleTimer=setInterval(()=>{
    if(actionLocked || enemy.hpNow<=0 || refs.combatant?.classList.contains("hidden")) return;
    index=(index+1)%frames.length;
    refs.sprite.src=frames[index];
  },enemy.speed || 380);
}

function skillNeedsEnemyTarget(skill){
  return Boolean(skill && ["damage","damage-burn","multi-hit","safe-chip","stun"].includes(skill.type));
}

function closeTargetPicker(){
  const returnMode=pendingTargetMode;
  pendingTargetMode="";
  pendingTargetSkill=null;
  pendingCapturePonId="";
  ui.targetPicker?.classList.add("hidden");
  ui.targetPickerGrid && (ui.targetPickerGrid.innerHTML="");
  if(!actionLocked && currentEnemy){
    if(returnMode==="skill") openCommandWindow("attack");
    else if(returnMode==="capture") openCommandWindow("item");
    else ui.commandGrid?.classList.remove("hidden");
  }
}

function doubleTargetBaseName(enemy){
  if(!enemy) return "Enemy";
  // The base Sea Turtle art is green, so say that explicitly in target choice.
  if(enemy.id==="sea-turtle" && enemy.oceanVariant==="green") return "Green Sea Turtle";
  return String(enemy.name || "Enemy");
}

function doubleTargetLabel(enemy,targets){
  const base=doubleTargetBaseName(enemy);
  const sameName=(targets||[]).filter(candidate=>doubleTargetBaseName(candidate)===base);
  if(sameName.length>1){
    return `${base} — ${enemySlot(enemy)===0?"Left":"Right"}`;
  }
  return base;
}

function openDoubleTargetPicker(mode="skill",payload=null){
  const targets=aliveDoubleEnemies();
  if(targets.length<=1){
    if(targets[0]) selectCurrentEnemy(targets[0]);
    if(mode==="skill" && payload){ useSkill(payload); return; }
    if(mode==="capture" && payload){ attemptBattleCapture(payload,targets[0]||currentEnemy); return; }
    return;
  }

  pendingTargetMode=mode;
  pendingTargetSkill=mode==="skill"?payload:null;
  pendingCapturePonId=mode==="capture"?String(payload||""):"";
  closeCommandWindow();
  ui.commandGrid?.classList.add("hidden");

  if(ui.targetPickerTitle){
    if(mode==="capture") ui.targetPickerTitle.textContent="Choose who to befriend";
    else if(mode==="skill" && payload) ui.targetPickerTitle.textContent=`Choose ${payload.name || "move"} target`;
    else ui.targetPickerTitle.textContent="Choose a target";
  }

  if(ui.targetPickerGrid){
    ui.targetPickerGrid.innerHTML="";
    for(const enemy of targets){
      const button=document.createElement("button");
      button.type="button";
      button.className=`pixel-button target-enemy-button${enemy.shiny?" shiny-target":""}`;
      const art=document.createElement("img");
      art.src=enemy.idle?.[0] || enemy.image || "";
      art.alt="";
      const name=document.createElement("strong");
      name.textContent=`${doubleTargetLabel(enemy,targets)}${enemy.shiny?" ✨":""}`;
      const hp=document.createElement("small");
      if(mode==="capture"){
        const pon=BUDDY_PONS.find(item=>item.id===pendingCapturePonId);
        const rate=pon?buddyPonCatchRate(pon,enemy):0;
        button.disabled=rate<=0;
        hp.textContent=rate>0
          ? `${Math.max(0,Math.round(enemy.hpNow))} / ${enemy.maxHp} HP · ${Math.round(rate*100)}% catch`
          : `${Math.max(0,Math.round(enemy.hpNow))} / ${enemy.maxHp} HP · Can't catch`;
      } else {
        hp.textContent=`${Math.max(0,Math.round(enemy.hpNow))} / ${enemy.maxHp} HP`;
      }
      button.append(art,name,hp);
      button.addEventListener("click",()=>{
        const nextMode=pendingTargetMode;
        const skill=pendingTargetSkill;
        const ponId=pendingCapturePonId;
        selectCurrentEnemy(enemy);
        pendingTargetMode="";
        pendingTargetSkill=null;
        pendingCapturePonId="";
        ui.targetPicker?.classList.add("hidden");
        ...
        if(nextMode==="skill" && skill){ useSkill(skill); return; }
        if(nextMode==="capture" && ponId){ attemptBattleCapture(ponId,enemy); return; }
      });
      ui.targetPickerGrid.appendChild(button);
    }
  }
  ui.targetPicker?.classList.remove("hidden");
}


const EVENT_CHOICE_SETS={
  "wishing-fountain":[
    {id:"fortune",label:"Wish for Fortune",detail:"Ask for a bigger pile of Pink Coins."},
    {id:"treasure",label:"Wish for Treasure",detail:"Ask for items and a better chance at collectible rewards."},
    {id:"friendship",label:"Wish for Friendship",detail:"Ask for extra Happiness and EXP with your OC."}
  ],
  "lucky-picnic":[
    {id:"share-snacks",label:"Share the Snacks",detail:"A cozy choice with the biggest Happiness reward."},
    {id:"check-basket",label:"Check the Basket",detail:"Look for extra items and Pink Coins inside."},
    {id:"relax",label:"Relax Together",detail:"Take a break for extra EXP and a little Happiness."}
  ]
};

function eventChoiceLabel(type,id){
  return EVENT_CHOICE_SETS[type]?.find(choice=>choice.id===id)?.label || situationTitle(type);
}

function hideEventChoices(){
  ui.eventChoiceActions?.classList.add("hidden");
  if(ui.eventChoiceActions) ui.eventChoiceActions.innerHTML="";
  ui.battleUi?.classList.remove("event-mode");
}

function showEventChoices(type){
  const choices=EVENT_CHOICE_SETS[type] || [];
  if(!ui.eventChoiceActions || !choices.length) return;
  ui.eventChoiceActions.innerHTML="";
  for(const choice of choices){
    const button=document.createElement("button");
    button.type="button";
    button.className="pixel-button event-choice-button";
    button.innerHTML=`<strong>${choice.label}</strong><small>${choice.detail}</small>`;
    button.addEventListener("click",async()=>{
      if(actionLocked || !pendingChest || pendingChest.eventType!==type) return;
      pendingChest.eventChoice=choice.id;
      hideEventChoices();
      await openPendingChest();
    });
    ui.eventChoiceActions.appendChild(button);
  }
  ui.eventChoiceActions.classList.remove("hidden");
...
function chestHappinessReward(chest){
  if(chest?.eventType==="lucky-picnic") return 4;
  if(chest?.eventType==="wishing-fountain") return 2;
  if(chest?.kind==="hidden-treasure") return 5;
  if(chest?.kind==="boss") return 3;
  if(chest?.kind==="double-trouble") return 2;
...
function makeEndlessEncounter(floor){
  ...
  return buildEncounterFromPool(ENDLESS_NORMAL_ENEMIES,"endless");
}
...
function startEncounter() {
  ...
  if(encounter.type==="lucky-picnic"){
    currentEnemy=null;
    ui.enemyCombatant.classList.add("hidden");
    ui.enemyCombatant2?.classList.add("hidden");
    ui.commandGrid.classList.add("hidden");
    closeCommandWindow();
    showChest({kind:"event", eventType:"lucky-picnic", captionText:"Lucky Picnic! Your buddy found a cozy snack spot."});
    ui.openChest.classList.add("hidden");
    setMessage("You found a Lucky Picnic! What should you do?");
    showEventChoices("lucky-picnic");
    return;
  }

  if(encounter.type==="wishing-fountain"){
    currentEnemy=null;
    ui.enemyCombatant.classList.add("hidden");
    ui.enemyCombatant2?.classList.add("hidden");
    ui.commandGrid.classList.add("hidden");
    closeCommandWindow();
    showChest({kind:"event", eventType:"wishing-fountain", captionText:"Wishing Fountain! Toss in a wish and see what happens."});
    ui.openChest.classList.add("hidden");
    setMessage("You found a Wishing Fountain! What would you like to wish for?");
    showEventChoices("wishing-fountain");
    return;
  }

  if(encounter.type==="double-enemy"){
    if(!Array.isArray(encounter.enemyIds) || encounter.enemyIds.length<2) encounter.enemyIds=pickEncounterEnemies(isEndless?ENDLESS_NORMAL_ENEMIES:["bee","cat-slime"],2);
    encounter.defeatedEnemies=[];
    startDoubleBattle(encounter.enemyIds);
    return;
  }
...
function renderEnemyName(enemy) {
  renderEnemyNameFor(enemy,enemySlot(enemy));
}
...
function startEnemy(enemyId, options={}) {
  ...
  loadEnemyEffects(currentEnemy);
  warmCurrentBattleAssets(currentEnemy);
  renderEnemyCombatant(currentEnemy,currentEnemy._doubleSlot);
  if(!options.silentSetup){
    startEnemyIdle(); renderBattleBuddy(); renderSkills(); renderBattleItems();
    if(currentEnemy.shiny) requestAnimationFrame(()=>playShinyArrivalSparkle(enemyUiFor(currentEnemy).combatant));
...
function startDoubleBattle(enemyIds=[]){
  resetDoubleBattleUi();
  currentEnemy=null;
  // Clear both visible slots before generating the pair.
  for(const slot of [0,1]){
    const refs=enemyUiFor(slot);
    refs.combatant?.classList.add("hidden");
    if(refs.name) refs.name.textContent="";
    if(refs.rank) refs.rank.textContent="";
    if(refs.hpFill) refs.hpFill.style.width="0%";
    if(refs.hpText) refs.hpText.textContent="";
    refs.sprite?.removeAttribute("src");
  }
  const ids=Array.isArray(enemyIds)?enemyIds.filter(Boolean).slice(0,2):[];
  if(ids.length<2){
    startEnemy(ids[0]||"bee");
    return;
  }

  ui.battlefield?.classList.add("double-battle");
  const spawned=[];
  ids.forEach((enemyId,slot)=>{
    startEnemy(enemyId,{slot,suppressIntro:true,silentSetup:true});
    currentEnemy._doubleSlot=slot;
    ensureEnemyEffectSnapshot(currentEnemy);
    saveCurrentEnemyEffects(currentEnemy);
    spawned.push(currentEnemy);
  });
  doubleBattleEnemies=spawned;
  for(const enemy of doubleBattleEnemies) renderEnemyCombatant(enemy,enemy._doubleSlot);
  selectCurrentEnemy(doubleBattleEnemies[0]);
  startEnemyIdle();
  startSecondaryEnemyIdle(doubleBattleEnemies[1]);
  renderBattleBuddy(); renderSkills(); renderBattleItems(); renderCommandButtons();
  ui.commandGrid?.classList.remove("hidden");
  closeCommandWindow();
  for(const enemy of doubleBattleEnemies){
    if(enemy.shiny) requestAnimationFrame(()=>playShinyArrivalSparkle(enemyUiFor(enemy).combatant));
  }
  const openingBuddy=activeBattleBuddyRecord();
  if(openingBuddy?.shiny) requestAnimationFrame(()=>playShinyArrivalSparkle(ui.buddyCombatant));
  setMessage(`Double Trouble! ${doubleBattleEnemies[0].name}${doubleBattleEnemies[0].shiny?" ✨":""} and ${doubleBattleEnemies[1].name}${doubleBattleEnemies[1].shiny?" ✨":""} appeared together!`);
}
...
async function useBuddySkill(){
  ...
  if(currentEnemy && currentEnemy.hpNow<=0){
    const encounterDone=await enemyDefeated();
...
function renderEnemyHp(enemy=currentEnemy) {
  if(!enemy) return;
  const refs=enemyUiFor(enemy);
  const pct=Math.max(0,Math.min(100,(enemy.hpNow/enemy.maxHp)*100));
  paintHpBar(refs.hpFill,pct,true);
  if(refs.hpText) refs.hpText.textContent=`${Math.max(0,Math.round(enemy.hpNow))} / ${enemy.maxHp} HP`;
}
...
function startEnemyIdle() {
  ...
  const tracked=currentEnemy;
  const refs=enemyUiFor(tracked);
  refs.sprite.src=tracked.idle[0];
  refs.sprite.alt=tracked.name;
  idleTimer=setInterval(()=>{
    if(actionLocked || !tracked || tracked.hpNow<=0 || refs.combatant?.classList.contains("hidden")) return;
    enemyIdleIndex=(enemyIdleIndex+1)%tracked.idle.length;
    refs.sprite.src=tracked.idle[enemyIdleIndex];
  },tracked.speed || 380);
}

function clearAnimations() {
  clearInterval(idleTimer); idleTimer=null;
  clearInterval(secondaryEnemyIdleTimer); secondaryEnemyIdleTimer=null;
...
function renderSkills() {
  ...
  sortedActiveSkills()
    ...
    .forEach(skill=>{
      ...
      if(skill.hitsAll && isDoubleBattleActive() && aliveDoubleEnemies().length>1 && !unavailable){
        detail += " · Hits BOTH enemies";
        button.innerHTML=`<strong>${skillDisplayName(skill)}</strong><span${detail.startsWith("Cooldown")?' class="cooldown"':""}>${detail}</span>`;
      }
      ...
      if(!unavailable) {
        button.addEventListener("click",()=>{
          closeCommandWindow();
          if(isDoubleBattleActive() && aliveDoubleEnemies().length>1 && skillNeedsEnemyTarget(skill) && !skill.hitsAll){
            openDoubleTargetPicker("skill",skill);
            return;
          }
          useSkill(skill);
        });
...
function renderBattleItems() {
  ...
  BUDDY_PONS.forEach(pon => {
    ...
    const captureTargets=isDoubleBattleActive()?aliveDoubleEnemies():[currentEnemy].filter(Boolean);
    const rates=captureTargets.map(enemy=>buddyPonCatchRate(pon,enemy));
    ...
    const lowHpBonus=!isDoubleBattleActive() && buddyLowHpCatchBonus(currentEnemy)>0;
    const rateLabel=isDoubleBattleActive() && captureTargets.length>1
      ? "Choose which enemy to catch"
      : rate>0 ? `${Math.round(rate*100)}% catch${lowHpBonus?" · Low HP bonus!":""}` : "Can't catch this boss";
    button.innerHTML=`
    ...
    `;
    button.addEventListener("click",()=>{
      if(isDoubleBattleActive() && aliveDoubleEnemies().length>1){
        openDoubleTargetPicker("capture",pon.id);
      } else {
        attemptBattleCapture(pon.id);
      }
    });
    ui.itemButtons.appendChild(button);
  });
...
async function applyEnemyBurnTick() {
  ...
  if(dealt>0) {
    ...
    const refs=activeEnemyUi();
    if(refs.sprite) refs.sprite.classList.add("hurt-pop");
    showFloat(`-${dealt}`,"damage","enemy");
    renderEnemyHp();
    await sleep(420);
    if(refs.sprite) refs.sprite.classList.remove("hurt-pop");
  }
...
async function useAllEnemySkill(skill){
  const targets=aliveDoubleEnemies();
  if(targets.length<2){
    if(targets[0]) selectCurrentEnemy(targets[0]);
    return useSkill(skill);
  }
  if(actionLocked || !currentRun) return;
  ...
  const remaining=aliveDoubleEnemies();
  if(remaining.length) selectCurrentEnemy(remaining[0]);
  await sleep(260);
  await enemyTurnSequence();
  actionLocked=false;
  renderCommandButtons();
...
async function useSkill(skill) {
  if(skill?.hitsAll && isDoubleBattleActive() && aliveDoubleEnemies().length>1){
    await useAllEnemySkill(skill);
    return;
  }
  if(actionLocked || !currentEnemy) return;
  actionLocked=true;
...
async function hurtEnemyNonLethal(dmg=1) {
  ...
  const refs=activeEnemyUi();
  if(refs.sprite){
    refs.sprite.src=currentEnemy.hurt;
    refs.sprite.classList.add("hurt-pop");
  }
  if(dealt>0) showFloat(`-${dealt}`,"damage","enemy");
  renderEnemyHp();
  await sleep(360);
  if(refs.sprite) refs.sprite.classList.remove("hurt-pop");
  if(currentEnemy.hpNow>0 && refs.sprite) refs.sprite.src=currentEnemy.idle[0];
}
...
async function hurtEnemy(dmg) {
  ...
  const refs=activeEnemyUi();
  if(refs.sprite){refs.sprite.src=currentEnemy.hurt; refs.sprite.classList.add("hurt-pop");}
  showFloat(`-${finalDmg}`,"damage","enemy"); renderEnemyHp(); await sleep(360);
  if(refs.sprite) refs.sprite.classList.remove("hurt-pop");
  if(currentEnemy.hpNow>0 && refs.sprite) refs.sprite.src=currentEnemy.idle[enemyIdleIndex%currentEnemy.idle.length];
  return finalDmg;
}
...
async function performEnemyAttack(multiplier=1,label="",lifeDrainHeal=0){
  ...
  const refs=activeEnemyUi();
  refs.sprite?.classList.add("attack-pop"); await sleep(300);
  ...
  await sleep(420);removeHeroVisualClass("hurt-pop");refs.sprite?.classList.remove("attack-pop");
  restoreHeroIdleFrame();
...
async function enemyTurn() {
  ...
  if(currentEnemy.specialType==="harden-shell" && !currentEnemy.shellHitsRemaining &&
    ...
    const refs=activeEnemyUi(); refs.sprite?.classList.add("attack-pop");await sleep(650);refs.sprite?.classList.remove("attack-pop");if(await finishEnemyBuddyTurn()) return; return;
  }
...
async function enemyTurnSequence(){
  if(!currentEnemy || !currentRun) return;
  if(!isDoubleBattleActive()){
    await enemyTurn();
    return;
  }
  ...
  saveCurrentEnemyEffects();
  const turnOrder=[...aliveDoubleEnemies()];
  for(const enemy of turnOrder){
    if(!currentRun || currentRun.hp<=0) break;
    if(!enemy || enemy.hpNow<=0 || enemy._defeated) continue;
    selectCurrentEnemy(enemy);
    setMessage(`${enemy.name}${enemy.shiny?" ✨":""}'s turn!`);
    await sleep(260);
    await enemyTurn();
    if(currentEnemy===enemy) saveCurrentEnemyEffects(enemy);
  }

  if(currentRun?.hp>0){
    const remaining=aliveDoubleEnemies();
    if(remaining.length){
      selectCurrentEnemy(remaining[0]);
      startEnemyIdle();
      if(remaining[1]) startSecondaryEnemyIdle(remaining[1]);
      setMessage(`${heroDisplayName()} is ready! Choose who to attack.`);
    }
  }
}
...
async function attemptBattleCapture(ponId,targetOverride=null) {
  if(actionLocked || !currentEnemy || !currentRun) return;
  const pon=BUDDY_PONS.find(item=>item.id===ponId);
  if(!pon) return;
  const target=targetOverride || currentEnemy;
  if(target) selectCurrentEnemy(target);
  ...
  if(success) {
    ...
    await sleep(520);
    const encounterDone=await enemyDefeated({captured:true});
    if(encounterDone) return;
...
async function enemyDefeated(options={}) {
  ...
  clearInterval(idleTimer);
  const refs=enemyUiFor(defeated);
  defeated._defeated=true;
  defeated.hpNow=0;
  saveCurrentEnemyEffects(defeated);
  ui.commandGrid.classList.add("hidden");
  closeCommandWindow();
  refs.combatant?.classList.add("hidden");
  setMessage(options.captured
    ? `${defeated.name}${defeated.shiny?" ✨":""} was befriended!`
    : `${defeated.name}${defeated.shiny?" ✨":""} was defeated!`);

  await sleep(options.captured?360:420);

  const encounter=currentEncounterData();
  const isDouble=encounter?.type==="double-enemy" && doubleBattleEnemies.length>1;
  if(isDouble){
    if(!Array.isArray(encounter.defeatedEnemies)) encounter.defeatedEnemies=[];
    if(!encounter.defeatedEnemies.some(entry=>entry?._doubleSlot===defeated._doubleSlot)) encounter.defeatedEnemies.push({...defeated});
    const alive=aliveDoubleEnemies();
    if(alive.length){
      selectCurrentEnemy(alive[0]);
      clearInterval(secondaryEnemyIdleTimer); secondaryEnemyIdleTimer=null;
      startEnemyIdle();
      ui.commandGrid.classList.remove("hidden");
      renderCommandButtons(); renderSkills(); renderBattleItems();
      setMessage(`${defeated.name}${options.captured?" was befriended":" is down"}! ${alive.map(enemy=>enemy.name).join(" remains! ")}${alive.length===1?" remains!":""}`);
      return false;
    }
  }
  ...
  pendingChest={
    kind:isBoss?"boss":isDouble?"double-trouble":"normal",
    revealMimic:false,
    enemy:defeated,
    enemies:isDouble?[...(encounter?.defeatedEnemies||[])] : [defeated],
    doubleTrouble:Boolean(isDouble)
  };

  currentEnemy=defeated;
  actionLocked=false;
  showChest(pendingChest);
  return true;
}
...
function showChest(data) {
  pendingChest=data;
  if(!data?.eventType) hideEventChoices();
  ui.buddyCombatant?.classList.add("hidden");
  ui.chestSprite.src=data.eventType==="lucky-picnic"
    ? "assets/events/Picnic.png"
    : data.eventType==="wishing-fountain"
      ? "assets/events/Fountain.png"
      : data.mystery
        ? "assets/shinies/amethyst-mimic-closed.webp"
        : "assets/items/chests/treasure/closed.webp";
  ui.chestSprite.classList.remove("opening","mystery-chest-sprite","event-scene-art","merchant-duck");
  ...
  ui.openChest.textContent=data.buttonLabel || "Open";
  ui.chestLayer.classList.remove("hidden");
  ui.chestCaption.textContent=data.captionText || (data.mystery
    ? "A strange purple chest appeared..."
    :data.kind==="rare"
      ?"A rare treasure chest appeared!"
      :data.kind==="double-trouble"
        ?"Double Trouble cleared!"
        :data.revealMimic
...
function weightedRewardItemByIds(ids){
  const pool=REWARD_ITEMS.filter(item=>ids.includes(item.id));
  if(!pool.length) return weightedItemWithCharm();
...
async function openPendingChest() {
  ...
  ui.chestCaption.textContent=pendingChest.hiddenTreasure
    ...
    : pendingChest.eventType
      ? `${eventChoiceLabel(pendingChest.eventType,pendingChest.eventChoice)} · ${textParts.join(" · ")}`
      : pendingChest.kind==="double-trouble"
        ? `Double Trouble cleared! · ${textParts.join(" · ")}`
        : textParts.join(" · ");
  ui.openChest.classList.add("hidden");
  if(!pendingChest.hiddenTreasure){
    if(pendingChest.eventType){
      setMessage(`${eventChoiceLabel(pendingChest.eventType,pendingChest.eventChoice)} rewards collected!`);
    } else if(pendingChest.kind==="double-trouble"){
      setMessage("Double Trouble rewards collected!");
    } else {
...
function generateRewards(chest) {
  ...
  if(chest.eventType==="lucky-picnic"){
    const choice=chest.eventChoice || "share-snacks";
    if(choice==="check-basket"){
      coins=randInt(22,36)+Math.max(2,Math.round(rank*1.35));
      exp=randInt(5,9)+Math.max(1,Math.round(rank*.55));
      addRewardItem(items,weightedRewardItemByIds(["bread-loaf","apple","flower","thread","yarn","glitter"]),1);
      addRewardItem(items,weightedRewardItemByIds(["bread-loaf","apple","flower","thread","yarn"]),1);
      return finalizeCharmRewards({coins,exp,happiness:2,items,iconBackground:null,iconBorderStyle:null,iconBorderColor:null,wallpaper:null},chest);
    }
    if(choice==="relax"){
      coins=randInt(8,14)+Math.max(1,Math.round(rank*.65));
      exp=randInt(18,28)+Math.max(2,Math.round(rank*1.35));
      addRewardItem(items,weightedRewardItemByIds(["bread-loaf","apple","flower"]),1);
      return finalizeCharmRewards({coins,exp,happiness:3,items,iconBackground:null,iconBorderStyle:null,iconBorderColor:null,wallpaper:null},chest);
    }
    coins=randInt(10,18)+Math.max(1,Math.round(rank*.85));
    exp=randInt(8,13)+Math.max(1,Math.round(rank*.65));
    addRewardItem(items,weightedRewardItemByIds(["bread-loaf","apple","flower"]),1);
    return finalizeCharmRewards({coins,exp,happiness:5,items,iconBackground:null,iconBorderStyle:null,iconBorderColor:Math.random()<.08?pickChestIconBorderColor({kind:"normal"}):null,wallpaper:null},chest);
  }

  if(chest.eventType==="wishing-fountain"){
    const choice=chest.eventChoice || "fortune";
    if(choice==="treasure"){
      coins=randInt(10,18)+Math.max(1,Math.round(rank*.75));
      exp=randInt(7,11)+Math.max(1,Math.round(rank*.55));
      addRewardItem(items,weightedRewardItemByIds(["glitter","pink-paint","mint-paint","aqua-paint","gold-paint","buddy-pon","super-buddy-pon"]),1);
      if(Math.random()<.55) addRewardItem(items,weightedItemWithCharm(),1);
      return finalizeCharmRewards({
        coins,exp,happiness:2,items,
        iconBackground:Math.random()<.24 ? pickChestIconBackground({kind:"rare"}) : null,
        iconBorderStyle:Math.random()<.12 ? pickChestIconBorderStyle({kind:"rare"}) : null,
        iconBorderColor:Math.random()<.28 ? pickChestIconBorderColor({kind:"rare"}) : null,
        wallpaper:Math.random()<.12 ? pickChestWallpaper({kind:"rare"}) : null
      },chest);
    }
    if(choice==="friendship"){
      coins=randInt(6,12)+Math.max(1,Math.round(rank*.55));
      exp=randInt(20,30)+Math.max(2,Math.round(rank*1.35));
      addRewardItem(items,weightedRewardItemByIds(["flower","apple","glitter"]),1);
      return finalizeCharmRewards({coins,exp,happiness:6,items,iconBackground:null,iconBorderStyle:null,iconBorderColor:null,wallpaper:null},chest);
    }
    coins=randInt(35,55)+Math.max(3,Math.round(rank*2.0));
    exp=randInt(6,10)+Math.max(1,Math.round(rank*.5));
    addRewardItem(items,weightedRewardItemByIds(["pink-paint","mint-paint","aqua-paint","glitter"]),1);
    return finalizeCharmRewards({coins,exp,happiness:2,items,iconBackground:Math.random()<.08?pickChestIconBackground({kind:"normal"}):null,iconBorderStyle:null,iconBorderColor:null,wallpaper:null},chest);
  }
  ...
  if(chest.kind==="double-trouble"){
    const foes=Array.isArray(chest.enemies) && chest.enemies.length ? chest.enemies : (chest.enemy?[chest.enemy]:[]);
    coins=foes.reduce((sum,foe)=>sum+randInt(Math.max(1,Number(foe.coinMinNow)||1),Math.max(1,Number(foe.coinMaxNow)||1)),0)+12;
    exp=foes.reduce((sum,foe)=>sum+Math.max(1,Number(foe.expNow)||1),0)+6;
    itemRolls=2;
    itemChance=1;
  } else if(chest.kind==="rare") {
  ...
  if(chest.enemy?.id==="mimic") {
    coins+=randInt(20,32);
    itemRolls=Math.max(1,itemRolls)+1;
    itemChance=1;
  }
  ...
  for(let i=0;i<itemRolls;i++) {
    if(Math.random()<=itemChance) {
      ...
      if((chest.kind==="boss" || chest.kind==="double-trouble") && items.some(x=>x.id===item.id)) {
  ...
  const rewardChestKind=chest.kind==="double-trouble" ? "rare" : chest.kind;
  const iconBackground=pickChestIconBackground({kind:rewardChestKind,enemy:chest.enemy});
  const iconBorderStyle=pickChestIconBorderStyle({kind:rewardChestKind,enemy:chest.enemy});
  const iconBorderColor=pickChestIconBorderColor({kind:rewardChestKind,enemy:chest.enemy});
  const wallpaper=pickChestWallpaper({kind:rewardChestKind,enemy:chest.enemy});
...
function applyRewards(rewards) {
  ...
  questSave.totalBattlesWon=(Number(questSave.totalBattlesWon)||0)+(pendingChest?.doubleTrouble ? Math.max(1,Math.floor(Number(pendingChest?.enemies?.length)||2)) : (pendingChest?.enemy?1:0));
...
function showFloat(text,type="damage",target="enemy") {
  const el = target === "peep" ? ui.peepDamageText : activeEnemyUi().damageText;
  if (!el) return;
...
function returnHome() {
  clearAnimations(); resetDoubleBattleUi(); closeBuddyDetail(); currentRun=null;
...
ui.attackMenuButton.addEventListener("click",()=>{
  openCommandWindow("attack");
});
ui.closeTargetPicker?.addEventListener("click",closeTargetPicker);
...
(function(){
  const DASH_RUN_OPTIONS={30:{duration:30,cost:3},60:{duration:60,cost:5}};
  const DASH_GROUND_RATIO=.05;
  const DASH_RUNNER_RAISE=4;
  const DASH_BASE_SPEED=.54;
  ...
  const DASH_CARTS=[
    ...
    {id:'pink',name:'Pink Cart',image:'assets/dash/carts/Duck-car-pink.png',kind:'shop',itemId:'dash-cart-pink'},
    {id:'mint',name:'Mint Cart',image:'assets/dash/carts/Duck-car-mint.png',kind:'shop',itemId:'dash-cart-mint'},
    {id:'purple',name:'Purple Cart',image:'assets/dash/carts/Duck-car-purple.png',kind:'shop',itemId:'dash-cart-purple'},
    {id:'black',name:'Black Cart',image:'assets/dash/carts/Duck-car-black.png',kind:'shop',itemId:'dash-cart-black'},
    {id:'peep',name:'Peep Cart',image:'assets/dash/carts/Duck-car-peep.png',kind:'happiness',characterId:'peep'},
    {id:'miko',name:'Miko Cart',image:'assets/dash/carts/Duck-car-miko.png',kind:'happiness',characterId:'miko'},
    {id:'io',name:'Io Cart',image:'assets/dash/carts/Duck-car-io.png',kind:'happiness',characterId:'io'},
    {id:'miho',name:'Miho Cart',image:'assets/dash/carts/Duck-car-miho.png',kind:'happiness',characterId:'miho'},
  ...
  const EVENT_ART={
    ...
    'wishing-fountain':'assets/events/Fountain.png'
  };
  const SIBLING_SCRIPTS={
    miho:[
      {speaker:'Miho',who:'miho',expression:'neutral',text:'Ugh. What the hell are you doing here?'},
      {speaker:'Miko',who:'miko',expression:'angry',text:"I don’t need YOUR permission to be here, Miho. I’m just looking for one of my loves."},
      {speaker:'Miho',who:'miho',expression:'neutral',text:"Then scram. I don’t want to deal with your bullshit."},
      {speaker:'Miko',who:'miko',expression:'angry',text:'Eh?! No, YOU move! I was here first!'},
      {speaker:'Miho',who:'miho',expression:'threaten',text:'You are so damn annoying! Get out of my way!'},
      {speaker:'Miko',who:'miko',expression:'angry',text:'No!! You get out of MY way!!'}
    ],
    miko:[
      {speaker:'Miko',who:'miko',expression:'mock',text:'Oho? Is that Miho I see? Hiding from your boys?'},
      {speaker:'Miho',who:'miho',expression:'fluster',text:"Shut the fuck up, Miko. I’m hiding from Dad."},
      {speaker:'Miko',who:'miko',expression:'angry',text:"He’s here?! Oh hell no."},
      {speaker:'Miho',who:'miho',expression:'neutral',text:'Maybe I should throw you to him so he’ll leave me alone.'},
      {speaker:'Miko',who:'miko',expression:'angry',text:'Over my dead body!! I’m going to throw YOU!'},
      {speaker:'Miho',who:'miho',expression:'threaten',text:"Fuck you! I’ve dealt with him enough!"},
      {speaker:'Miko',who:'miko',expression:'smug',text:'Too bad!!'}
    ]
  };
  const SIBLING_SPRITES={
    miko:{
      neutral:'assets/characters/miko/base/idle-1.webp',
      mock:'assets/characters/miko/base/mock.webp',
      smug:'assets/characters/miko/base/smug.webp',
      angry:'assets/characters/miko/base/Miko-angry.png'
    },
    miho:{
      neutral:'assets/characters/miho/base/idle-1.webp',
      fluster:'assets/characters/miho/base/fluster.webp',
      threaten:'assets/characters/miho/base/threaten.webp'
    }
  };

  // The opposite sister is a real battle opponent, not a cutscene-only dummy.
  ENEMIES['sibling-miko']={name:'Miko',hp:38,attack:7,exp:46,coinMin:15,coinMax:24,idle:['assets/characters/miko/base/idle-1.webp','assets/characters/miko/base/idle-2.webp'],hurt:SIBLING_SPRITES.miko.angry,speed:520,uncatchable:true,siblingCharacterId:'miko'};
  ENEMIES['sibling-miho']={name:'Miho',hp:40,attack:8,exp:46,coinMin:15,coinMax:24,idle:['assets/characters/miho/base/idle-1.webp','assets/characters/miho/base/idle-2.webp'],hurt:SIBLING_SPRITES.miho.fluster,speed:520,uncatchable:true,siblingCharacterId:'miho'};

  const extraCss=document.createElement('style');
  extraCss.textContent=`
    .dq-expand-card{margin-top:16px;padding:15px;border:3px solid var(--border,#8c625d);border-radius:20px;background:rgba(255,250,246,.94);display:grid;gap:11px;box-shadow:0 7px 0 rgba(140,98,93,.12)}
    .dq-expand-head{display:flex;gap:10px;align-items:flex-start;justify-content:space-between}.dq-expand-head h2{margin:2px 0 0}.dq-expand-head small{display:block;color:#725d68}.dq-token-pill{display:inline-flex;align-items:center;gap:6px;border:2px solid rgba(140,98,93,.2);border-radius:999px;padding:6px 9px;background:#fff}.dq-token-pill img{width:24px;height:24px;object-fit:contain;image-rendering:pixelated}
    .dq-mini-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.dq-mini-grid>div{padding:9px;border:2px solid rgba(140,98,93,.14);border-radius:14px;background:#fff;display:grid;gap:2px}.dq-mini-grid span{font-size:.68rem;letter-spacing:.08em;text-transform:uppercase;color:#8b7380}.dq-mini-grid strong{color:#5f4552}.dq-expand-actions{display:flex;gap:8px;flex-wrap:wrap}.dq-expand-actions .pixel-button{flex:1 1 150px}
    #dashScreen,#hatcheryScreen{padding:14px;display:grid;align-content:start;gap:12px}.dq-feature-shell{display:grid;gap:14px;background:rgba(255,250,246,.96);border:3px solid var(--border,#8c625d);border-radius:22px;padding:15px}.dq-feature-top{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap}.dq-feature-top h2{margin:2px 0}.dq-feature-top small{color:#725d68}
    .dash-stage-buttons,.dash-garage{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.dash-stage-button.selected,.dash-cart-card.selected{box-shadow:inset 0 0 0 3px rgba(233,145,178,.35);background:#fff0f7}.dash-stage-button{min-height:52px}
    .dash-playbox{display:grid;justify-items:center;gap:9px}.dash-hud{width:min(100%,520px);display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.dash-hud>span{font-size:.73rem;text-align:center;padding:6px 4px;border:2px solid rgba(140,98,93,.15);border-radius:11px;background:#fff}.dash-track{position:relative;width:min(100%,520px);aspect-ratio:16/9;overflow:hidden;border:4px solid var(--border,#8c625d);border-radius:18px;background:#cde8f4;touch-action:manipulation;user-select:none}.dash-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;image-rendering:pixelated}.dash-runner{position:absolute;width:20%;max-width:92px;height:auto;left:12%;bottom:calc(5% + 4px);z-index:5;object-fit:contain;image-rendering:pixelated;transition:filter .12s}.dash-runner.hit{filter:brightness(1.65)}.dash-thing{position:absolute;z-index:4;object-fit:contain;image-rendering:pixelated;pointer-events:none}.dash-obstacle{width:15.5%;max-width:74px}.dash-coin{width:9.8%;max-width:42px}.dash-tiny-duck{width:10.8%;max-width:48px}.dash-powerup-v217{width:10.8%;max-width:48px;filter:drop-shadow(0 2px 0 rgba(255,255,255,.7))}.dash-bg-scroll-v217{position:absolute;inset:0;z-index:0;display:flex;width:max-content;height:100%;will-change:transform;pointer-events:none}.dash-bg-scroll-v217 .dash-bg{position:relative!important;inset:auto!important;flex:0 0 auto!important;width:auto!important;max-width:none!important;height:100%!important;object-fit:fill!important}.dash-bubble-v217{position:absolute!important;z-index:9!important;width:142%!important;height:142%!important;left:-21%!important;top:-21%!important;object-fit:contain!important;pointer-events:none!important;opacity:0!important;transform:scale(.82)!important;transition:opacity .14s ease,transform .14s ease!important;filter:drop-shadow(0 0 8px rgba(180,239,255,.8))!important}.dash-bubble-v217.active{opacity:1!important;transform:scale(1)!important}.dash-power-pop-v217{position:absolute;left:17%;bottom:36%;z-index:12;padding:5px 8px;border:2px solid #fff;border-radius:999px;background:rgba(255,248,224,.95);color:#8b5a6d;font-size:.65rem;font-weight:900;animation:dashPowerPopV217 .9s ease-out forwards;pointer-events:none}@keyframes dashPowerPopV217{to{opacity:0;transform:translateY(-28px)}}.dash-overlay{position:absolute;inset:0;z-index:10;display:grid;place-items:center;background:rgba(255,249,253,.78);padding:15px}.dash-overlay.hidden{display:none}.dash-overlay-card{max-width:280px;display:grid;gap:9px;text-align:center;padding:14px;border:3px solid var(--border,#8c625d);border-radius:17px;background:#fff}.dash-tap-note{font-size:.76rem;color:#735e69;text-align:center}.dash-garage-wrap{display:grid;gap:8px}.dash-cart-card{border:2px solid rgba(140,98,93,.15);border-radius:15px;padding:9px;background:#fff;display:grid;grid-template-columns:60px 1fr;gap:8px;align-items:center;text-align:left}.dash-cart-card img{width:58px;height:58px;object-fit:contain;image-rendering:pixelated}.dash-cart-card small{display:block;color:#796471}.dash-cart-card.locked{opacity:.62}
    .hatch-bg{background-image:linear-gradient(rgba(255,250,246,.3),rgba(255,250,246,.3)),url('assets/eggs/Egg-hatching-background.png');background-size:cover;background-position:center;border:3px solid var(--border,#8c625d);border-radius:18px;padding:13px;display:grid;gap:13px;min-height:360px}.egg-grid,.incubator-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.egg-card,.incubator-card{background:rgba(255,255,255,.9);border:2px solid rgba(140,98,93,.17);border-radius:14px;padding:9px;text-align:center;display:grid;gap:6px;align-content:start}.egg-card img,.incubator-card img{width:64px;height:64px;object-fit:contain;image-rendering:pixelated;margin:auto}.egg-card small,.incubator-card small{color:#765f6b}.egg-shake{animation:eggShake .65s ease-in-out}@keyframes eggShake{0%,100%{transform:rotate(0)}20%{transform:rotate(-10deg) translateY(-2px)}40%{transform:rotate(10deg)}60%{transform:rotate(-8deg) translateY(-3px)}80%{transform:rotate(8deg)}}.hatch-log{padding:10px;border:2px dashed rgba(140,98,93,.25);border-radius:13px;background:rgba(255,255,255,.9);min-height:58px}
    .event-scene-art{width:min(210px,52vw)!important;height:min(210px,52vw)!important;object-fit:contain!important;image-rendering:pixelated}.merchant-duck{width:min(170px,46vw)!important;height:min(170px,46vw)!important}.merchant-wares{display:grid;gap:8px}.merchant-ware{display:grid;grid-template-columns:48px 1fr auto;gap:8px;align-items:center;text-align:left}.merchant-ware img{width:46px;height:46px;object-fit:contain;image-rendering:pixelated}.merchant-ware small{display:block;color:#755f6a}.merchant-ware .price{white-space:nowrap;font-weight:700}.chest-layer.merchant-active-v197{display:flex!important;flex-direction:column!important;justify-content:center!important;align-items:center!important;gap:8px!important;padding:10px!important;overflow:auto!important;background:rgba(74,52,48,.34)!important}.chest-layer.merchant-active-v197 .merchant-duck{width:min(112px,30vw)!important;height:min(112px,30vw)!important;flex:0 0 auto!important}.merchant-shop-v197{width:min(96%,460px)!important;display:grid!important;gap:7px!important;padding:9px!important;border:3px solid var(--border,#8c625d)!important;border-radius:14px!important;background:rgba(255,250,246,.98)!important;box-shadow:0 8px 24px rgba(74,52,48,.22)!important}.merchant-shop-v197 .merchant-ware{width:100%!important;min-height:54px!important;padding:7px 9px!important;grid-template-columns:42px minmax(0,1fr) auto!important;gap:7px!important}.merchant-shop-v197 .merchant-ware img{width:40px!important;height:40px!important}.merchant-shop-v197 .merchant-ware strong{font-size:.72rem!important}.merchant-shop-v197 .merchant-ware small{font-size:.53rem!important;line-height:1.2!important}.merchant-shop-v197 .price{font-size:.62rem!important;grid-column:auto!important}.merchant-leave-v197{width:100%!important;min-height:42px!important}
    .sibling-scene{position:absolute;inset:0;z-index:13;display:flex;align-items:flex-end;justify-content:space-between;padding:7% 6% 5%;pointer-events:none}.sibling-scene.hidden{display:none}.sibling-scene img{width:36%;height:62%;object-fit:contain;object-position:center bottom;image-rendering:pixelated;filter:drop-shadow(0 4px 0 rgba(0,0,0,.12))}.battlefield .peep-combatant.sibling-spat-hidden-v224{display:none!important}.sibling-scene.sibling-spat-v224 img{transform-origin:center bottom;will-change:transform}.sibling-scene.sibling-spat-v224 .sibling-left-v224{animation:siblingLeftIdleV224 1.05s ease-in-out infinite}.sibling-scene.sibling-spat-v224 .sibling-right-v224{animation:siblingRightIdleV224 1.05s ease-in-out -.525s infinite}@keyframes siblingLeftIdleV224{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}@keyframes siblingRightIdleV224{0%,100%{transform:scaleX(-1) translateY(0)}50%{transform:scaleX(-1) translateY(-4px)}}.sibling-dialogue-actions{display:flex;justify-content:flex-end}.sibling-dialogue-actions .pixel-button{min-width:120px}
    ...
    @media(max-width:560px){.dq-mini-grid,.egg-grid,.incubator-grid{grid-template-columns:1fr}.dash-hud{grid-template-columns:repeat(2,minmax(0,1fr))}.dash-cart-card{grid-template-columns:52px 1fr}.dash-cart-card img{width:50px;height:50px}.merchant-ware{grid-template-columns:42px 1fr}.merchant-ware .price{grid-column:2}.sibling-scene img{width:42%;height:58%}}

    /* v24.167 — Hatching Area as a dedicated portrait location. */
    #hatcheryScreen{padding:0!important;min-height:100dvh;background:linear-gradient(180deg,#d9eef9,#f9edf4);overflow:auto;}
    .hatch-world-shell{min-height:100dvh;display:grid;place-items:center;padding:10px 10px calc(18px + env(safe-area-inset-bottom));}
    .hatch-world{position:relative;width:min(100%,420px);aspect-ratio:9/16;overflow:hidden;border:4px solid var(--border,#8c625d);border-radius:24px;background:#9ee78f;box-shadow:0 16px 42px rgba(77,67,83,.22);isolation:isolate;}
    .hatch-world-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;image-rendering:pixelated;z-index:0;}
    .hatch-world-topbar{position:absolute;left:0;right:0;top:0;z-index:12;display:grid;grid-template-columns:auto 1fr auto;align-items:start;gap:8px;padding:calc(9px + env(safe-area-inset-top)) 9px 0;pointer-events:none;}
    .hatch-world-topbar>*{pointer-events:auto;}
    .hatch-back-button,.hatch-inventory-button{min-height:36px;border:2px solid #d8647d;border-radius:10px;background:rgba(255,247,249,.94);color:#a7425b;font-weight:800;box-shadow:0 3px 0 rgba(167,66,91,.14);}
    .hatch-back-button{width:42px;font-size:1.1rem;}.hatch-inventory-button{padding:6px 9px;display:inline-flex;align-items:center;gap:6px;justify-self:end;}
    .hatch-inventory-button .count{display:inline-grid;place-items:center;min-width:22px;height:22px;padding:0 6px;border-radius:999px;background:#ed7892;color:white;font-size:.75rem;}
    .hatch-world-title{align-self:start;justify-self:center;margin-top:1px;padding:6px 10px;border:2px solid rgba(255,255,255,.9);border-radius:999px;background:rgba(255,250,252,.86);color:#8b4b5d;font-size:.72rem;font-weight:900;letter-spacing:.08em;text-transform:uppercase;box-shadow:0 3px 12px rgba(96,77,88,.12);white-space:nowrap;}
    .hatch-world-ready{position:absolute;left:50%;top:12%;transform:translateX(-50%);z-index:10;background:#fff6b9;color:#8b5b20;border:2px solid #e5c64f;border-radius:999px;padding:5px 10px;font-size:.72rem;font-weight:900;box-shadow:0 4px 12px rgba(105,84,45,.14);}.hatch-world-ready.hidden{display:none;}
    .hatch-nest-slots{position:absolute;inset:0;z-index:4;}.hatch-nest-slot{position:absolute;left:50%;width:44%;height:19%;transform:translate(-50%,-50%);border:0;background:transparent;padding:0;display:grid;place-items:center;cursor:pointer;touch-action:manipulation;}
    .hatch-nest-slot:nth-child(1){top:32.5%;}.hatch-nest-slot:nth-child(2){top:63.5%;}.hatch-nest-slot:nth-child(3){top:89%;}.hatch-nest-slot:focus-visible{outline:3px solid rgba(216,100,125,.9);outline-offset:3px;border-radius:50%;}
    .hatch-main-egg{position:absolute;width:72%;height:72%;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 5px 0 rgba(122,76,60,.16));transition:transform .16s ease;}.hatch-nest-slot:hover .hatch-main-egg,.hatch-nest-slot:active .hatch-main-egg{transform:translateY(-3px);}
    .hatch-empty-plus{display:grid;place-items:center;width:42px;height:42px;border:2px dashed rgba(173,84,103,.62);border-radius:50%;background:rgba(255,250,252,.78);color:#b6516b;font-size:1.45rem;font-weight:700;}
    .hatch-slot-status{position:absolute;top:83%;left:50%;transform:translateX(-50%);max-width:140%;white-space:nowrap;padding:4px 8px;border-radius:999px;border:2px solid rgba(255,255,255,.9);background:rgba(255,248,250,.88);color:#ad4963;font-size:.68rem;font-weight:900;box-shadow:0 3px 9px rgba(91,70,81,.12);}
    .hatch-nest-slot.ready .hatch-slot-status{background:#fff4a9;border-color:#ffe978;color:#8c5d1f;animation:hatchReadyPulse 1.25s ease-in-out infinite;}.hatch-nest-slot.ready .hatch-main-egg{animation:hatchReadyBob 1.35s ease-in-out infinite;}
    @keyframes hatchReadyPulse{50%{transform:translateX(-50%) scale(1.06);box-shadow:0 0 18px rgba(255,235,101,.85)}}@keyframes hatchReadyBob{50%{transform:translateY(-5px)}}
    .hatch-world-tip{margin:8px 0 0;text-align:center;color:#755f6c;font-size:.78rem;}
    .hatch-modal{position:fixed;inset:0;z-index:350;display:grid;place-items:center;padding:14px;background:rgba(54,42,51,.38);backdrop-filter:blur(3px);}.hatch-modal.hidden{display:none;}.hatch-modal-backdrop{position:absolute;inset:0;border:0;background:transparent;}
    .hatch-modal-card{position:relative;z-index:1;width:min(94vw,390px);max-height:min(86dvh,720px);overflow:auto;border:3px solid #cf667d;border-radius:22px;background:#fff8f4;box-shadow:0 18px 50px rgba(60,43,52,.26);padding:15px;display:grid;gap:12px;color:#684b58;}
    .hatch-modal-close{position:absolute;top:8px;right:8px;width:32px;height:32px;border:2px solid #dd748b;border-radius:50%;background:#fff;color:#bd4d66;font-weight:900;font-size:1rem;z-index:4;}.hatch-modal-heading{padding-right:34px;text-align:center;}.hatch-modal-heading h2{margin:2px 0 0;font-size:1.2rem;}.hatch-modal-heading small{color:#8b6b78;}
    .hatch-window{position:relative;width:min(100%,270px);aspect-ratio:4/3;margin:auto;overflow:hidden;border:3px solid #efb4c1;border-radius:18px;background:linear-gradient(180deg,#fff2ef,#ffe9e4);display:grid;place-items:center;isolation:isolate;}
    .hatch-window-nest{position:absolute;width:150%;height:auto;left:50%;top:50%;transform:translate(-50%,-49%);image-rendering:pixelated;z-index:1;pointer-events:none;}.hatch-window-egg{position:relative;z-index:3;width:43%;height:72%;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 5px 0 rgba(122,76,60,.14));transform-origin:50% 82%;}
    .hatch-window-glow{position:absolute;z-index:2;width:55%;aspect-ratio:1;border-radius:50%;background:radial-gradient(circle,rgba(255,252,157,.96) 0%,rgba(255,229,113,.72) 35%,rgba(255,203,231,.35) 62%,transparent 76%);opacity:0;transform:scale(.45);filter:blur(1px);pointer-events:none;}.hatch-window-glow.active{animation:hatchGlow 1.35s ease-in-out forwards;}.hatch-window-egg.hatching{animation:hatchBounce 1.35s cubic-bezier(.22,.9,.3,1) forwards;}
    @keyframes hatchBounce{0%{transform:translateY(0) rotate(0)}14%{transform:translateY(-12px) rotate(-5deg)}27%{transform:translateY(0) rotate(5deg)}42%{transform:translateY(-18px) rotate(-7deg)}57%{transform:translateY(0) rotate(6deg)}73%{transform:translateY(-24px) rotate(-8deg)}88%{transform:translateY(-4px) rotate(7deg)}100%{transform:translateY(-8px) rotate(0) scale(1.08)}}@keyframes hatchGlow{0%{opacity:0;transform:scale(.45)}35%{opacity:.55;transform:scale(.82)}75%{opacity:1;transform:scale(1.22)}100%{opacity:.95;transform:scale(1.12)}}
    .hatch-result{position:absolute;inset:0;z-index:5;display:grid;place-items:center;padding:12px;text-align:center;background:radial-gradient(circle at 50% 50%,rgba(255,255,214,.92),rgba(255,246,252,.88) 52%,rgba(255,238,246,.9));}.hatch-result.hidden{display:none;}.hatch-result img{width:52%;height:54%;object-fit:contain;image-rendering:pixelated;animation:hatchBuddyPop .55s cubic-bezier(.2,1.5,.45,1) both;filter:drop-shadow(0 6px 0 rgba(111,75,87,.12));}.hatch-result strong{display:block;margin-top:2px;color:#9b405a;font-size:1.05rem;}.hatch-result small{display:block;color:#806571;margin-top:2px;}
    .hatch-result.shiny::before,.hatch-result.shiny::after{content:'✦';position:absolute;color:#f0b93b;font-size:1.4rem;animation:hatchSparkle .9s ease-in-out infinite alternate;}.hatch-result.shiny::before{left:18%;top:20%;}.hatch-result.shiny::after{right:17%;top:29%;animation-delay:.2s;}@keyframes hatchBuddyPop{from{opacity:0;transform:scale(.55) translateY(12px)}to{opacity:1;transform:scale(1) translateY(0)}}@keyframes hatchSparkle{to{transform:scale(1.4) rotate(18deg);opacity:.45}}
    .hatch-detail-status{text-align:center;background:#fff;border:2px solid rgba(207,102,125,.18);border-radius:14px;padding:9px;font-weight:800;color:#8f5061;}.hatch-detail-actions{display:flex;gap:8px;flex-wrap:wrap;}.hatch-detail-actions .pixel-button{flex:1 1 130px;}
    .egg-inventory-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;}.egg-inventory-card{border:2px solid rgba(207,102,125,.2);border-radius:16px;background:#fff;padding:10px;display:grid;gap:7px;text-align:center;}.egg-inventory-card img{width:72px;height:72px;margin:auto;object-fit:contain;image-rendering:pixelated}.egg-inventory-card strong{color:#8e4b5d}.egg-inventory-card small{color:#826c76;line-height:1.35}.egg-inventory-card .egg-count{font-size:.9rem;font-weight:900;color:#ba526b}.egg-inventory-note{margin:0;text-align:center;color:#856b77;font-size:.75rem;line-height:1.4;}
    @media(max-width:460px){.hatch-world-shell{padding:0}.hatch-world{width:100%;max-width:none;border-left:0;border-right:0;border-radius:0;min-height:100dvh;aspect-ratio:auto}.hatch-world-bg{object-fit:cover}.hatch-world-tip{display:none}.hatch-slot-status{font-size:.64rem}.hatch-modal{padding:9px}.hatch-modal-card{width:min(96vw,390px)}}
  `;
  ...
  function injectFeatureMarkup(){
    ...
    if(battle && !document.querySelector('#dashScreen')){
      battle.insertAdjacentHTML('beforebegin',`
        <section id="dashScreen" class="screen hidden">
          <div class="dq-feature-shell">
            <div class="dq-feature-top"><div><span class="mini-label">DUCKIE DASH</span><h2>Duckie Dash</h2><small>Tap the course to jump. Keep going until the timer reaches zero!</small></div><button id="dashBack" class="pixel-button" type="button">← Quest Menu</button></div>
            <div class="dq-mini-grid"><div><span>Jump Tokens</span><strong id="dashTokenCount">0</strong></div><div><span>Free Runs</span><strong id="dashFreeCount">0</strong></div><div><span>Selected Cart</span><strong id="dashCartLabel">Default Cart</strong></div></div>
        ...
        <section id="hatcheryScreen" class="screen hidden">
          <div class="hatch-world-shell">
            <div class="hatch-world" aria-label="Hatching Area">
              <img class="hatch-world-bg" src="assets/eggs/Hatching-background-new.png" alt="">
              <div class="hatch-world-topbar">
                <button id="hatchBack" class="hatch-back-button" type="button" aria-label="Back to Duck Quest">←</button>
                <div class="hatch-world-title">Hatching Area</div>
              ...
              <div id="hatchNestSlots" class="hatch-nest-slots" aria-label="Incubating eggs"></div>
            </div>
            <p class="hatch-world-tip">Tap an empty nest to choose an egg, or tap an incubating egg to check on it. ♡</p>
          </div>
        ...
        <section id="siblingScene" class="sibling-scene hidden" aria-hidden="true"><img id="siblingLeft" src="" alt=""><img id="siblingRight" src="" alt=""></section>`);
    }
  }
  injectFeatureMarkup();
  const siblingSceneNode=document.querySelector('#siblingScene');
  if(siblingSceneNode && document.querySelector('#battlefield')) document.querySelector('#battlefield').appendChild(siblingSceneNode);

  function ensureExpansionSave(){
    if(!questSave.dash || typeof questSave.dash!=='object') questSave.dash={};
    questSave.dash.selectedCart=String(questSave.dash.selectedCart||'default');
    questSave.dash.selectedStage=DASH_STAGE[questSave.dash.selectedStage]?questSave.dash.selectedStage:'meadow';
    questSave.dash.highScore=Math.max(0,Math.floor(Number(questSave.dash.highScore)||0));
    questSave.dash.runs=Math.max(0,Math.floor(Number(questSave.dash.runs)||0));
    questSave.dash.freeRuns=Math.max(0,Math.floor(Number(questSave.dash.freeRuns)||0));
    if(!questSave.eggs || typeof questSave.eggs!=='object') questSave.eggs={};
    if(!Array.isArray(questSave.eggs.incubators)) questSave.eggs.incubators=[];
  ...
  function specialQty(id){return Math.max(0,Math.floor(Number(hubSave.inventory?.[id])||0));}
  function addInventory(id,qty=1){if(!hubSave.inventory||typeof hubSave.inventory!=='object')hubSave.inventory={};hubSave.inventory[id]=specialQty(id)+Math.max(1,Math.floor(Number(qty)||1));}
  function useInventory(id,qty=1){const n=Math.max(1,Math.floor(Number(qty)||1));if(specialQty(id)<n)return false;const left=specialQty(id)-n;if(left)hubSave.inventory[id]=left;else delete hubSave.inventory[id];return true;}
  function cartUnlocked(cart){
    if(cart.kind==='default') return true;
    if(cart.kind==='shop') return specialQty(cart.itemId)>0;
    return happinessLevelFromTotal(hubSave.characterProgress?.[cart.characterId]?.happinessTotal)>=100;
  }
  ...
  function renderDashSelectors(){
    ensureExpansionSave();
    const stageBox=document.querySelector('#dashStageButtons');if(stageBox){stageBox.innerHTML='';Object.entries(DASH_STAGE).forEach(([id,cfg])=>{const b=document.createElement('button');b.type='button';b.className=`pixel-button dash-stage-button${questSave.dash.selectedStage===id?' selected':''}`;b.textContent=cfg.name;b.addEventListener('click',()=>{questSave.dash.selectedStage=id;persistAll();renderDashSelectors();resetDashPreview();});stageBox.appendChild(b);});}
    const garage=document.querySelector('#dashGarage');if(garage){garage.innerHTML='';DASH_CARTS.forEach(cart=>{const unlocked=cartUnlocked(cart);const card=document.createElement('button');card.type='button';card.className=`dash-cart-card${unlocked?'':' locked'}${questSave.dash.selectedCart===cart.id?' selected':''}`;card.disabled=!unlocked;let note='';if(cart.kind==='shop')note=unlocked?'Owned':'Buy this color in the Hub Shop';else if(cart.kind==='happiness')note=unlocked?'Max Happiness reward unlocked!':`Reach Happiness Lv. 100 with ${QUEST_CHARACTER_NAMES[cart.characterId]}`;else note='Always available';card.innerHTML=`<img src="${cart.image}" alt=""><span><strong>${cart.name}</strong><small>${note}</small></span>`;card.addEventListener('click',()=>{questSave.dash.selectedCart=cart.id;persistAll();renderDashSelectors();refreshFeatureBadges();});garage.appendChild(card);});}
  ...
  function dashHud(){const set=(id,v)=>{const e=document.querySelector(id);if(e)e.textContent=String(v)};set('#dashCoins',dash.coins);set('#dashTiny',dash.tiny);set('#dashTime',Math.max(0,(Number(dash.duration)||30)-dash.elapsed).toFixed(1));set('#dashPower',dash.bubble>0?`Bubble ${dash.bubble.toFixed(1)}s`:dash.speedBoost>0?`Wing ${dash.speedBoost.toFixed(1)}s`:'—');document.querySelector('#dashBubble')?.classList.toggle('active',dash.bubble>0);}
  function resetDashPreview(){ensureExpansionSave();const cfg=DASH_STAGE[questSave.dash.selectedStage];const bg=document.querySelector('#dashBg'),clone=document.querySelector('#dashBgCloneV217'),strip=document.querySelector('#dashBgScrollV217');if(bg)bg.src=cfg.dashBackground;if(clone)clone.src=cfg.dashBackground;if(strip)strip.style.transform='translateX(0px)';dash.bgOffset=0;const runner=document.querySelector('#dashRunner');if(runner)runner.style.transform='translateY(0px)';document.querySelector('#dashBubble')?.classList.remove('active');syncDashDriverV208(false);dashHud();}
  function selectedDashRunV235(){ensureExpansionSave();const select=document.querySelector('#dashRunDurationV235');const duration=Number(select?.value||questSave.dash.runDuration||30)===60?60:30;questSave.dash.runDuration=duration;return DASH_RUN_OPTIONS[duration];}
  ...
  function addDashDuckCopyV232(saveData,duckId,amount=0){
  ...
  function showDashPowerPopV217(text){const track=dashTrack();if(!track)return;const pop=document.createElement('div');pop.className='dash-power-pop-v217';pop.textContent=text;track.appendChild(pop);setTimeout(()=>pop.remove(),950);}
  ...
  function showMerchantEvent(){
    ...
    ui.commandGrid.classList.add('hidden');
    closeCommandWindow();
    pendingChest={kind:'event',eventType:'mysterious-merchant',wares:merchantStock()};
    showChest({kind:'event',eventType:'mysterious-merchant',captionText:'Mysterious Merchant! Gentleman Duck has a tiny roadside shop.'});
    ...
    ui.openChest.classList.add('hidden');
    setMessage('Gentleman Duck offers two completely random items today.');
    renderMerchantButtons();
  }
  function renderMerchantButtons(){
    if(!pendingChest?.wares||!ui.chestLayer)return;
    ui.eventChoiceActions?.classList.add('hidden');
    if(ui.eventChoiceActions)ui.eventChoiceActions.innerHTML='';
    ui.battleUi?.classList.add('event-mode');
    ui.chestLayer.classList.add('merchant-active-v197');
    let shop=ui.chestLayer.querySelector('#merchantShopV197');
    if(!shop){shop=document.createElement('div');shop.id='merchantShopV197';shop.className='merchant-shop-v197';ui.chestLayer.appendChild(shop);}
    shop.innerHTML='';
    pendingChest.wares.forEach(ware=>{
      ...
      b.addEventListener('click',()=>{
        ...
        hubSave.coins-=ware.price;
        if(ware.kind==='charm')charmState().owned[ware.id]=true;else addInventory(ware.id,1);
        ware.sold=true;
        persistAll();renderMeta();renderMerchantButtons();refreshFeatureBadges();setMessage(`Bought ${ware.name}!`);
      });
      shop.appendChild(b);
    });
    const leave=document.createElement('button');
    leave.type='button';
    leave.className='pixel-button merchant-leave-v197';
    leave.textContent='No Thanks';
    leave.addEventListener('click',()=>finishNonBattleEvent('Gentleman Duck tips his hat and packs up his tiny shop.'));
    shop.appendChild(leave);
  }
  window.SIBLING_SPAT_BATTLE_FIX='24.230-inward-equal-height';
  // ----- Sibling Spat dialogue + battle -----
  let siblingDialogueIndex=0,siblingDialogueState=null,siblingIdleTimer=null,siblingIdleIndex=0;
  const SIBLING_IDLE_FRAMES={miko:['assets/characters/miko/base/idle-1.webp','assets/characters/miko/base/idle-2.webp'],miho:['assets/characters/miho/base/idle-1.webp','assets/characters/miho/base/idle-2.webp']};
  function siblingSprite(who,expression,frame=0){if(expression==='neutral'){const idle=SIBLING_IDLE_FRAMES[who];if(idle?.length)return idle[frame%idle.length]}return SIBLING_SPRITES[who]?.[expression]||SIBLING_SPRITES[who]?.neutral||''}
  function stopSiblingAnimation(){if(siblingIdleTimer){clearInterval(siblingIdleTimer);siblingIdleTimer=null}}
  function updateSiblingSceneSprites(){const state=siblingDialogueState;if(!state)return;const left=document.querySelector('#siblingLeft'),right=document.querySelector('#siblingRight');if(left)left.src=siblingSprite(state.leftWho,state.leftExpression,siblingIdleIndex);if(right)right.src=siblingSprite(state.rightWho,state.rightExpression,siblingIdleIndex)}
  function startSiblingAnimation(){stopSiblingAnimation();siblingIdleIndex=0;updateSiblingSceneSprites();siblingIdleTimer=setInterval(()=>{siblingIdleIndex=(siblingIdleIndex+1)%2;updateSiblingSceneSprites()},520)}
  function showSiblingEvent(){const script=SIBLING_SCRIPTS[activeCharacterId];if(!script)return;const hero=document.querySelector('.peep-combatant');hero?.classList.add('hidden','sibling-spat-hidden-v224');currentEnemy=null;ui.enemyCombatant.classList.add('hidden');ui.enemyCombatant2?.classList.add('hidden');ui.buddyCombatant?.classList.add('hidden');ui.commandGrid.classList.add('hidden');closeCommandWindow();ui.chestLayer.classList.add('hidden');const scene=document.querySelector('#siblingScene'),left=document.querySelector('#siblingLeft'),right=document.querySelector('#siblingRight');scene?.classList.add('sibling-spat-v224');scene?.classList.remove('hidden');scene?.setAttribute('aria-hidden','false');left?.classList.add('sibling-left-v224');right?.classList.add('sibling-right-v224');const leftWho=activeCharacterId,rightWho=activeCharacterId==='miko'?'miho':'miko';if(left)left.alt=leftWho==='miko'?'Miko':'Miho';if(right)right.alt=rightWho==='miko'?'Miko':'Miho';siblingDialogueState={script,leftWho,rightWho,leftExpression:'neutral',rightExpression:'neutral'};siblingDialogueIndex=0;startSiblingAnimation();renderSiblingLine();}
  function renderSiblingLine(){const state=siblingDialogueState;if(!state)return;const line=state.script[siblingDialogueIndex];if(!line)return;const side=line.who===state.leftWho?'left':'right';state[`${side}Expression`]=line.expression;const img=document.querySelector(side==='left'?'#siblingLeft':'#siblingRight');if(img)img.src=siblingSprite(line.who,line.expression);setMessage(`${line.speaker}: ${line.text}`);const holder=ui.eventChoiceActions;holder.innerHTML='';const b=document.createElement('button');b.type='button';b.className='pixel-button primary';const last=siblingDialogueIndex>=state.script.length-1;b.textContent=last?'Fight!':'Next';b.addEventListener('click',()=>{if(last)beginSiblingBattle();else{siblingDialogueIndex++;renderSiblingLine()}});const row=document.createElement('div');row.className='sibling-dialogue-actions';row.appendChild(b);holder.appendChild(row);holder.classList.remove('hidden');ui.battleUi?.classList.add('event-mode');}
  function beginSiblingBattle(){const opponent=activeCharacterId==='miko'?'sibling-miho':'sibling-miko';stopSiblingAnimation();document.querySelector('.peep-combatant')?.classList.remove('hidden','sibling-spat-hidden-v224');document.querySelector('#siblingScene')?.classList.add('hidden');document.querySelector('#siblingScene')?.setAttribute('aria-hidden','true');hideEventChoices();ui.enemyCombatant.classList.remove('hidden');ui.commandGrid.classList.remove('hidden');startEnemy(opponent,{suppressIntro:true});ui.battlefield?.classList.add('sibling-battle-v230');ui.enemySprite?.classList.add('sibling-opponent-v230');currentEnemy.uncatchable=true;currentEnemy.siblingCharacterId=opponent.endsWith('miko')?'miko':'miho';currentEnemy.siblingAttackBuffTurns=0;currentEnemy.siblingAttackMultiplier=1;currentEnemy.siblingBuffUsed=false;setMessage(`Sibling Spat! ${currentEnemy.name} is ready to fight!`);}

  const oldCatchRate=buddyPonCatchRate;buddyPonCatchRate=function(pon,enemy){if(enemy?.uncatchable)return 0;return oldCatchRate(pon,enemy)};
  const oldEnemyTurn=enemyTurn;enemyTurn=async function(){
    if(!currentEnemy?.siblingCharacterId)return oldEnemyTurn();
    if(currentEnemy.hpNow<=0)return;
    const id=currentEnemy.siblingCharacterId;const skills=(id==='miko'?MIKO_SKILLS:MIHO_SKILLS).filter(x=>activeHeroProgress().level>=x.unlock&&x.id!=='just-a-smack');let candidates=skills.filter(x=>['damage','damage-burn','multi-hit'].includes(x.type));
    if(id==='miko'&&!currentEnemy.siblingBuffUsed&&currentEnemy.hpNow<currentEnemy.maxHp*.55){const heal=skills.find(x=>x.id==='thoughts-of-lovers');if(heal)candidates=[...candidates,heal,heal]}
    const buff=skills.find(x=>x.type==='buff');if(buff&&currentEnemy.siblingAttackBuffTurns<=0)candidates.push(buff);
    const skill=candidates[Math.floor(Math.random()*candidates.length)]||skills[0];const refs=activeEnemyUi();if(skill?.sprite&&refs.sprite)refs.sprite.src=skill.sprite;
    if(skill?.type==='buff'){currentEnemy.siblingAttackMultiplier=1+Number(skill.attackBoost||.3);currentEnemy.siblingAttackBuffTurns=Number(skill.duration||3);currentEnemy.siblingBuffUsed=true;setMessage(`${currentEnemy.name} used ${skill.name}! Attack rose!`);refs.sprite?.classList.add('attack-pop');await sleep(650);refs.sprite?.classList.remove('attack-pop');}
    else if(skill?.type==='full-heal-buff'){currentEnemy.hpNow=currentEnemy.maxHp;currentEnemy.siblingAttackMultiplier=1+Number(skill.attackBoost||.5);currentEnemy.siblingAttackBuffTurns=Number(skill.duration||4);currentEnemy.siblingBuffUsed=true;renderEnemyHp();setMessage(`${currentEnemy.name} used ${skill.name}! Full heal + Attack boost!`);await sleep(750);}
    else {const mult=Math.max(.7,Number(skill?.multiplier)||1)*Math.max(1,Number(currentEnemy.siblingAttackMultiplier)||1);await performEnemyAttack(mult,`${currentEnemy.name} used ${skill?.name||'Attack'}!`);}
    currentEnemy.siblingAttackBuffTurns=Math.max(0,Number(currentEnemy.siblingAttackBuffTurns||0)-1);if(currentEnemy.siblingAttackBuffTurns<=0)currentEnemy.siblingAttackMultiplier=1;if(currentRun?.hp>0){if(refs.sprite)refs.sprite.src=currentEnemy.idle[0];setMessage(`${heroDisplayName()} is ready!`)}
  };

  // Add Merchant + character-only Sibling Spat to the existing special-event pool.
  pickSituationType=function(mode='stage'){const entries=mode==='endless'?[{value:'lucky-picnic',weight:5},{value:'wishing-fountain',weight:4},{value:'mysterious-merchant',weight:3}]:[{value:'lucky-picnic',weight:6},{value:'wishing-fountain',weight:4},{value:'mysterious-merchant',weight:3}];if(activeCharacterId==='miko'||activeCharacterId==='miho')entries.push({value:'sibling-spat',weight:1});return weightedPick(entries)};
  ...
  const oldShowEventChoices=showEventChoices;showEventChoices=function(type){oldShowEventChoices(type);if(EVENT_ART[type]){ui.chestSprite.src=EVENT_ART[type];ui.chestSprite.classList.add('event-scene-art')}};

  // Eggs + Jump Tokens can drop from every chest tier and special event.
  const oldGenerateRewards=generateRewards;generateRewards=function(chest){const r=oldGenerateRewards(chest);r.freeDashRuns=0;if(!Array.isArray(r.items))r.items=[];const add=(item,qty=1)=>addRewardItem(r.items,item,qty);
    if(chest?.eventType==='lucky-picnic'){if(Math.random()<.16)add(SPECIAL_REWARD_ITEMS.commonEgg);if(Math.random()<.22)add(SPECIAL_REWARD_ITEMS.token)}
    else if(chest?.eventType==='wishing-fountain'){if(Math.random()<.18)add(SPECIAL_REWARD_ITEMS.commonEgg);if(Math.random()<.05)add(SPECIAL_REWARD_ITEMS.rareEgg);if(Math.random()<.24)add(SPECIAL_REWARD_ITEMS.token);if(Math.random()<.05)r.freeDashRuns++}
    else if(chest?.kind==='hidden-treasure'){add(SPECIAL_REWARD_ITEMS.token,2);add(SPECIAL_REWARD_ITEMS.rareEgg);if(Math.random()<.18)r.freeDashRuns++}
    else if(chest?.kind==='double-trouble'){if(Math.random()<.42)add(SPECIAL_REWARD_ITEMS.token);if(Math.random()<.18)add(SPECIAL_REWARD_ITEMS.commonEgg);if(Math.random()<.04)add(SPECIAL_REWARD_ITEMS.rareEgg)}
    else if(chest?.kind==='rare'){if(Math.random()<.38)add(SPECIAL_REWARD_ITEMS.token);if(Math.random()<.20)add(SPECIAL_REWARD_ITEMS.commonEgg);if(Math.random()<.05)add(SPECIAL_REWARD_ITEMS.rareEgg);if(Math.random()<.04)r.freeDashRuns++}
    else if(chest?.kind==='boss'){if(Math.random()<.32)add(SPECIAL_REWARD_ITEMS.token);if(Math.random()<.18)add(SPECIAL_REWARD_ITEMS.commonEgg);if(Math.random()<.08)add(SPECIAL_REWARD_ITEMS.rareEgg);if(Math.random()<.05)r.freeDashRuns++}
    else {if(Math.random()<.08)add(SPECIAL_REWARD_ITEMS.token);if(Math.random()<.035)add(SPECIAL_REWARD_ITEMS.commonEgg);if(Math.random()<.006)add(SPECIAL_REWARD_ITEMS.rareEgg)}return r};
  const oldApplyRewards=applyRewards;applyRewards=function(r){oldApplyRewards(r);if(Number(r?.freeDashRuns)>0){ensureExpansionSave();questSave.dash.freeRuns+=Math.max(0,Math.floor(Number(r.freeDashRuns)||0));persistAll();refreshFeatureBadges()}};

  // Home/screen integration.
  const oldShowScreen=showScreen;showScreen=function(which){oldShowScreen(which);const dashScreen=document.querySelector('#dashScreen');dashScreen?.classList.toggle('hidden',which!=='dash');document.querySelector('#hatcheryScreen')?.classList.toggle('hidden',which!=='hatchery');if(which!=='dash'){if(dash.running){dash.running=false;cancelAnimationFrame(dash.raf);clearDashObjects()}dashScreen?.classList.remove('dash-run-stage-v193');document.body.classList.remove('dash-run-stage-v193-active')}if(which==='dash'){renderDashSelectors();resetDashPreview();if(!dash.running)renderDashReadyOverlay()}if(which==='hatchery')renderHatchery()};
  const oldRenderMeta=renderMeta;renderMeta=function(){oldRenderMeta();refreshFeatureBadges()};
  document.querySelector('#openDuckieDash')?.addEventListener('click',()=>showScreen('dash'));
  document.querySelector('#openHatchery')?.addEventListener('click',()=>showScreen('hatchery'));
  document.querySelector('#dashBack')?.addEventListener('click',()=>showScreen('home'));
  document.querySelector('#hatchBack')?.addEventListener('click',()=>showScreen('home'));
  document.querySelector('#hatchInventoryButton')?.addEventListener('click',()=>openEggInventory(null,false));
  document.querySelector('#closeHatchDetail')?.addEventListener('click',closeHatchDetail);
  document.querySelector('#hatchDetailBackdrop')?.addEventListener('click',closeHatchDetail);
...
// v24.169 — compact Duck Quest home + one-screen Hatching Area polish
(function(){
  const polish=document.createElement('style');
  polish.textContent=`
    /* Home organization — based on the player's compact mockup. */
    #homeScreen.dq-home-polished{gap:10px;}
    #homeScreen.dq-home-polished .hero-card{grid-template-columns:minmax(118px,30%) minmax(0,1fr);gap:12px;padding:12px;border-radius:18px;}
    #homeScreen.dq-home-polished .hero-sprite-column{gap:6px;}
    #homeScreen.dq-home-polished .hero-sprite-wrap{min-height:132px;border-radius:14px;}
    #homeScreen.dq-home-polished .menu-peep{width:108px;height:108px;}
    #homeScreen.dq-home-polished .hero-info{align-content:start;gap:10px;padding-top:4px;}
    #homeScreen.dq-home-polished .hero-name-row h2{font-size:1.22rem;}
    #homeScreen.dq-home-polished .hero-quick-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;order:4;}
    #homeScreen.dq-home-polished .hero-info-button{min-height:39px;padding:6px 7px;font-size:.64rem;border-radius:10px;}
    #homeScreen.dq-home-polished .meter-group{gap:4px;}
    #homeScreen.dq-home-polished .happiness-row{align-items:flex-end;}
    #homeScreen.dq-home-polished .hearts{font-size:1.08rem;gap:2px;flex-wrap:nowrap;}
    #homeScreen.dq-home-polished .icon-background-control>span{font-size:.57rem;}
    #homeScreen.dq-home-polished .icon-background-button{min-height:39px;padding:5px 7px;}
    #homeScreen.dq-home-polished .switch-quest-oc-button{min-height:39px;margin-top:1px;}
    .dq-hero-tools{grid-column:1/-1;display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.08fr);gap:10px;margin-top:1px;}
    .dq-hero-tools .charm-home-card{margin:0!important;min-height:58px;padding:0!important;border:0!important;background:transparent!important;box-shadow:none!important;display:block!important;}
    .dq-hero-tools .charm-home-copy,.dq-hero-tools .charm-home-slots{display:none!important;}
    .dq-hero-tools #openCharmScreen,.dq-hero-tools #openHatchery{width:100%;min-height:58px;border-radius:14px;font-size:.82rem;display:flex;align-items:center;justify-content:center;gap:8px;}
    .dq-hero-tools #openCharmScreen::before{content:'♡';font-size:1.15rem;color:#cf6788;}
    .dq-hero-tools #openHatchery{background:linear-gradient(180deg,#f4a4c0,#e87fa7);color:#fff;border-color:#9d5b70;text-shadow:1px 1px #99566d;}
    .dq-hero-tools #openHatchery .home-hatch-icon{width:30px;height:30px;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 2px 0 rgba(97,61,73,.16));}
    .dq-hero-tools #openHatchery .home-hatch-label{font-size:.88rem;letter-spacing:.01em;}
    .dq-hero-tools #hatchReadyBadge:not(:empty){display:inline-grid;place-items:center;min-width:21px;height:21px;padding:0 5px;border-radius:999px;background:#fff5aa;color:#98661f;text-shadow:none;font-size:.67rem;}

    #homeScreen.dq-home-polished .endless-home-card{padding:12px;border-radius:18px;gap:9px;background:linear-gradient(135deg,rgba(250,244,255,.98),rgba(245,236,255,.96));border-color:#8c78bc;}
    #homeScreen.dq-home-polished .endless-home-heading{align-items:center;}
    #homeScreen.dq-home-polished .endless-home-heading h2{font-size:1.16rem;margin:1px 0 1px;}
    #homeScreen.dq-home-polished .endless-home-heading small{font-size:.66rem;line-height:1.25;}
    #homeScreen.dq-home-polished .endless-symbol{width:42px;height:42px;font-size:1.25rem;display:grid;place-items:center;border:3px solid #8c78bc;border-radius:50%;background:rgba(255,255,255,.72);color:#745db0;}
    #homeScreen.dq-home-polished .endless-record-grid{gap:7px;}
    #homeScreen.dq-home-polished .endless-record-grid>div{padding:8px 9px;min-height:57px;border-radius:12px;background:rgba(255,255,255,.72);}
    #homeScreen.dq-home-polished .endless-record-grid span{font-size:.57rem;}
    #homeScreen.dq-home-polished .endless-record-grid strong{font-size:.82rem;}
    #homeScreen.dq-home-polished .endless-home-actions{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(0,1fr);gap:8px;}
    #homeScreen.dq-home-polished .endless-home-actions .pixel-button{min-height:45px;}
    #homeScreen.dq-home-polished .endless-note{display:none;}

    #homeScreen.dq-home-polished #duckieDashHomeCard{margin-top:0;padding:12px 13px;border-radius:18px;gap:8px;position:relative;overflow:hidden;}
    #homeScreen.dq-home-polished #duckieDashHomeCard::before{content:'♪';position:absolute;left:12px;bottom:12px;color:#ee9ab7;font-size:1.35rem;opacity:.5;pointer-events:none;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head{align-items:center;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head>div{padding-left:34px;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head h2{font-size:1.15rem;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head small{font-size:.67rem;line-height:1.25;max-width:440px;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-token-pill{flex:0 0 auto;padding:5px 9px;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-mini-grid{display:none;}
    .dash-home-cost{margin:0 0 0 auto;font-size:.72rem;font-weight:900;color:#6d4d59;display:flex;align-items:center;gap:5px;}
    .dash-home-cost img{width:20px;height:20px;object-fit:contain;image-rendering:pixelated;}
    #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-actions{justify-content:flex-end;}
    #homeScreen.dq-home-polished #duckieDashHomeCard #openDuckieDash{flex:0 1 58%;min-height:44px;}

    #homeScreen.dq-home-polished .area-select-card{padding:12px;border-radius:18px;}
    #homeScreen.dq-home-polished .area-select-title{margin-bottom:8px;}
    #homeScreen.dq-home-polished .area-select-card h2{font-size:1rem;}
    #homeScreen.dq-home-polished .area-picker{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;}
    #homeScreen.dq-home-polished .area-option{min-height:76px;padding:7px 8px;border-radius:13px;grid-template-columns:66px minmax(0,1fr);gap:8px;}
    #homeScreen.dq-home-polished .area-option-image{width:64px;height:58px;border-radius:9px;}
    #homeScreen.dq-home-polished .area-option strong{font-size:.76rem;}
    #homeScreen.dq-home-polished .area-option small{font-size:.55rem;}

    /* Hatching Area: all three nests fit in the available phone viewport. */
    body.hatchery-active{overflow:hidden;}
    #hatcheryScreen.hatchery-fit-screen{min-height:0!important;height:var(--hatch-screen-height,calc(100dvh - 64px))!important;overflow:hidden!important;padding:0!important;}
    #hatcheryScreen.hatchery-fit-screen .hatch-world-shell{height:100%;min-height:0!important;padding:0!important;display:grid;place-items:stretch;}
    #hatcheryScreen.hatchery-fit-screen .hatch-world{height:100%;min-height:0!important;width:100%;max-width:none;aspect-ratio:auto!important;border-radius:18px;}
    #hatcheryScreen.hatchery-fit-screen .hatch-world-title{font-size:.92rem;padding:7px 13px;letter-spacing:.065em;}
    #hatcheryScreen.hatchery-fit-screen .hatch-world-topbar{padding-top:9px;}
    #hatcheryScreen.hatchery-fit-screen .hatch-world-ready{top:13%;font-size:.68rem;padding:4px 8px;}
    #hatcheryScreen.hatchery-fit-screen .hatch-nest-slot{width:42%;height:16.5%;}
    #hatcheryScreen.hatchery-fit-screen .hatch-nest-slot:nth-child(1){top:35.5%;}
    #hatcheryScreen.hatchery-fit-screen .hatch-nest-slot:nth-child(2){top:61%;}
    #hatcheryScreen.hatchery-fit-screen .hatch-nest-slot:nth-child(3){top:83%;}
    #hatcheryScreen.hatchery-fit-screen .hatch-main-egg{width:68%;height:68%;}
    #hatcheryScreen.hatchery-fit-screen .hatch-empty-plus{width:38px;height:38px;font-size:1.25rem;}
    #hatcheryScreen.hatchery-fit-screen .hatch-slot-status{top:80%;font-size:.61rem;padding:3px 7px;}
    #hatcheryScreen.hatchery-fit-screen .hatch-world-tip{display:none!important;}

    @media(max-width:600px){
      #homeScreen.dq-home-polished .hero-card{grid-template-columns:31% minmax(0,1fr);gap:9px;padding:9px;}
      #homeScreen.dq-home-polished .hero-sprite-wrap{min-height:112px;}
      #homeScreen.dq-home-polished .menu-peep{width:94px;height:94px;}
      #homeScreen.dq-home-polished .hero-info{gap:7px;padding-top:1px;}
      #homeScreen.dq-home-polished .hero-name-row h2{font-size:1rem;}
      #homeScreen.dq-home-polished .hero-info-button{min-height:32px;padding:4px 4px;font-size:.52rem;}
      #homeScreen.dq-home-polished .hearts{font-size:.91rem;gap:1px;}
      #homeScreen.dq-home-polished .icon-background-button{font-size:.54rem;min-height:35px;}
      #homeScreen.dq-home-polished .switch-quest-oc-button{font-size:.54rem;min-height:35px;}
      .dq-hero-tools{gap:7px;}
      .dq-hero-tools #openCharmScreen,.dq-hero-tools #openHatchery{min-height:50px;font-size:.68rem;padding:6px 7px;}
      .dq-hero-tools #openHatchery .home-hatch-icon{width:25px;height:25px;}
      .dq-hero-tools #openHatchery .home-hatch-label{font-size:.72rem;}
      #homeScreen.dq-home-polished .endless-home-card{padding:9px;gap:7px;}
      #homeScreen.dq-home-polished .endless-home-heading h2{font-size:.95rem;}
      #homeScreen.dq-home-polished .endless-home-heading small{font-size:.54rem;}
      #homeScreen.dq-home-polished .endless-symbol{width:34px;height:34px;font-size:1rem;border-width:2px;}
      #homeScreen.dq-home-polished .endless-record-grid>div{padding:6px;min-height:48px;}
      #homeScreen.dq-home-polished .endless-record-grid span{font-size:.48rem;}
      #homeScreen.dq-home-polished .endless-record-grid strong{font-size:.68rem;}
      #homeScreen.dq-home-polished .endless-home-actions .pixel-button{min-height:40px;font-size:.64rem;padding:6px;}
      #homeScreen.dq-home-polished #duckieDashHomeCard{padding:9px 10px;gap:6px;}
      #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head>div{padding-left:27px;}
      #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head h2{font-size:.94rem;}
      #homeScreen.dq-home-polished #duckieDashHomeCard .dq-expand-head small{font-size:.54rem;}
      #homeScreen.dq-home-polished #duckieDashHomeCard .dq-token-pill img{width:21px;height:21px;}
      .dash-home-cost{font-size:.58rem;}
      #homeScreen.dq-home-polished #duckieDashHomeCard #openDuckieDash{min-height:39px;font-size:.66rem;flex-basis:62%;}
      #homeScreen.dq-home-polished .area-select-card{padding:9px;}
      #homeScreen.dq-home-polished .area-picker{gap:6px;}
      #homeScreen.dq-home-polished .area-option{min-height:66px;padding:6px;grid-template-columns:54px minmax(0,1fr);gap:6px;}
      #homeScreen.dq-home-polished .area-option-image{width:52px;height:50px;}
      #homeScreen.dq-home-polished .area-option strong{font-size:.64rem;}
      #homeScreen.dq-home-polished .area-option small{font-size:.47rem;}
      #homeScreen.dq-home-polished .area-select-card h2{font-size:.83rem;}
    }
    @media(max-width:390px){
      #homeScreen.dq-home-polished .hero-card{grid-template-columns:108px minmax(0,1fr);}
      #homeScreen.dq-home-polished .hero-quick-actions{grid-template-columns:repeat(3,minmax(0,1fr));gap:4px;}
      #homeScreen.dq-home-polished .hero-info-button{font-size:.48rem;padding:4px 2px;}
      #homeScreen.dq-home-polished .hearts{font-size:.82rem;}
      .dq-hero-tools #openHatchery .home-hatch-label{font-size:.66rem;}
    }
  `;
  ...
  function organizeHome(){
    ...
    const charm=document.querySelector('.charm-home-card');
    const dash=document.querySelector('#duckieDashHomeCard');
    const hatch=document.querySelector('#openHatchery');
    if(!home||!hero||!charm||!dash||!hatch) return;
    home.classList.add('dq-home-polished');

    let tools=hero.querySelector('.dq-hero-tools');
    if(!tools){tools=document.createElement('div');tools.className='dq-hero-tools';hero.appendChild(tools);}
    if(charm.parentElement!==tools) tools.appendChild(charm);
    const charmButton=document.querySelector('#openCharmScreen');
    if(charmButton) charmButton.textContent='Equip Charm';

    if(hatch.parentElement!==tools) tools.appendChild(hatch);
    ...
    if(!cost){
      ...
      const actions=dash.querySelector('.dq-expand-actions');
      actions?.before(cost);
    }
  }

  function fitHatcheryToViewport(){
    ...
    const available=Math.max(420,window.innerHeight-top-6);
  ...
  showScreen=function(which){
    previousShowScreen(which);
    const active=which==='hatchery';
    document.body.classList.toggle('hatchery-active',active);
    if(active) requestAnimationFrame(fitHatcheryToViewport);
  };
  window.addEventListener('resize',()=>requestAnimationFrame(fitHatcheryToViewport),{passive:true});
  window.addEventListener('orientationchange',()=>setTimeout(fitHatcheryToViewport,120));
})();
...
(function(){
  ...
  openPendingChest=async function(){
    ...
    // v24.185: once a chest has paid out, the same button becomes Continue.
    // Advance exactly once instead of reopening the same chest and rolling rewards again.
    if(pendingChest.opened){
      const finishedChest=pendingChest;
      pendingChest=null;
      ui.openChest.disabled=true;

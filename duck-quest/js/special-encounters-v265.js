(function(){
  ...
  function prepSpecial(encounter){
    ...
    ui.chestSprite?.classList.remove("gift-box-art-v262","event-scene-art","merchant-duck","luv-birdie-art-v262","mystery-chest-sprite");
  ...
  let giftTimer=null;
  let lastGiftGrantV264=null;
  function stopGiftAnimation(){if(giftTimer){clearInterval(giftTimer);giftTimer=null;}}
  function clearGiftRewardVisual(){
    ui.chestLayer?.querySelector("#giftRewardVisualV264")?.remove();
    ui.chestSprite?.classList.remove("gift-reward-hidden-v264");
  }
  function showGiftRewardVisual(granted,reward){
    if(!ui.chestLayer||!TC)return;
    clearGiftRewardVisual();
    ui.chestSprite?.classList.add("gift-reward-hidden-v264");
    const wrap=document.createElement("div");wrap.id="giftRewardVisualV264";wrap.className="gift-reward-visual-v264";
    const cards=Array.isArray(granted?.cardResults)?granted.cardResults:[];
    if(cards.length){
      cards.slice(0,3).forEach((result,index)=>{const holder=document.createElement("div");holder.className="gift-reward-card-v264";holder.style.setProperty("--gift-delay",`${index*85}ms`);holder.append(TC.createFace(result.card,{locked:false}));wrap.append(holder);});
    }else{
      const packs=Math.max(0,Math.floor(Number(reward?.packs)||0));
      if(packs){const holder=document.createElement("div");holder.className="gift-reward-pack-v264";const img=document.createElement("img");img.src=TC.packAsset("card-pack-unopened.png");img.alt=packs===1?"Card Pack":`${packs} Card Packs`;holder.append(img);if(packs>1){const badge=document.createElement("strong");badge.textContent=`×${packs}`;holder.append(badge);}wrap.append(holder);}
    }
    if(wrap.childElementCount){ui.chestCaption?.before(wrap);requestAnimationFrame(()=>wrap.classList.add("show"));}
  }
  function finishGiftReward(message){
    stopGiftAnimation();ui.openChest?.classList.add("hidden");setMessage(message);
    if(currentRun?.mode==="endless")markEndlessFloorComplete();
    ui.continueButton.textContent=currentRun?.mode==="endless"?"Next Floor":"Continue";
    ui.postFloorActions?.classList.remove("hidden");ui.leaveEndlessButton?.classList.toggle("hidden",currentRun?.mode!=="endless");
    setPostFloorLayout(true);actionLocked=false;persist();
  }
  function showGiftBox(){
    ...
    clearGiftRewardVisual();
    ui.chestLayer.classList.remove("hidden","merchant-active-v197","luv-birdie-active-v262");ui.chestLayer.classList.add("gift-box-active-v262");
  ...
  function grantCardPackReward(reward){
    ...
    persist(); return {notes,cardResults,packs};
  }
  ...
  async function openGiftBox(){
    ...
    showGiftRewardVisual(granted,info.giftReward);
    finishGiftReward(`Gift opened! ${granted.notes.join(" · ")}`.trim());
  }
  ...
  if(typeof applyRewards==="function"){
    ...
    applyRewards=function(rewards){oldApplyRewardsV262(rewards);if(!Array.isArray(rewards.items))rewards.items=[];if(!Array.isArray(rewards.tradingCardResults))rewards.tradingCardResults=[];
      ...
      if(s?.type==="dust"){const state=ensureCraftState(hubSave);if(state){state.powerUpDust+=s.amount;rewards.items.push({id:"power-up-dust-v265",name:"Power Up Dust",qty:s.amount});}}
      ...
      if(rewards.v262GiftReward){const g=grantCardPackReward(rewards.v262GiftReward);lastGiftGrantV264=g;rewards.v264GiftGranted=g;rewards.tradingCardResults.push(...g.cardResults);const packs=Math.max(0,Number(rewards.v262GiftReward.packs)||0);if(packs)rewards.items.push({id:"card-pack-v262",name:"Card Pack",qty:packs});}
  ...
  if(typeof openPendingChest==="function"){
    const oldOpenChestV264=openPendingChest;
    openPendingChest=async function(){
      if(pendingChest?.kind==="gift-box")return openGiftBox();
      const giftReward=pendingChest?.giftReward?{...pendingChest.giftReward}:null;
      lastGiftGrantV264=null;
      const result=await oldOpenChestV264();
      if(giftReward&&lastGiftGrantV264){
        showGiftRewardVisual(lastGiftGrantV264,giftReward);
        ui.chestCaption.textContent=lastGiftGrantV264.notes?.length?lastGiftGrantV264.notes.join(" · "):"Card reward collected!";
      }
      return result;
    };
  ...
  window.DUCKIE_SPECIAL_ENCOUNTERS_V264=V;
})();

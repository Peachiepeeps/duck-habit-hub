...
function makePaintableFurnitureItems() {
  ...
  for (const family of Object.values(PAINTABLE_FURNITURE_FAMILIES)) {
    for (const color of FURNITURE_COLOR_OPTIONS) {
      const itemId = `petbed-${color.id}`;
      entries.push([itemId, {
        name: color.id === "white" ? family.name : `${color.label} ${family.name}`,
        category: "furniture",
        image: `${family.assetFolder}${family.assetPrefix}${color.id}.PNG`,
        icon: "♡",
        sellValue: 12,
...
function furnitureFamilyItemId(familyId, colorId = "white") {
  if (familyId === "pet-bed") return `petbed-${colorId}`;
  return null;
}
...
function furnitureRecipeEntries() {
...
function renderFurnitureCraftTab() {
  ...
  const whiteBeds = inventoryQuantity("petbed-white");
  intro.innerHTML = `
    <div>
      <strong>Paint Furniture</strong>
      <p>Use a White furniture base + one Paint to create that color. Each color can be owned once.</p>
    </div>
    <span>${whiteBeds} white base${whiteBeds === 1 ? "" : "s"}</span>
  `;
  wrapper.append(intro);
  ...
  furnitureRecipeEntries().forEach(([itemId, entry]) => {
    ...
    grid.append(
      createCrafterCard(
        ...
        item.image,
        status,
        inventoryQuantity(itemId) > 0 ? "Color owned" : "Pet Bed + Paint"
      )
    );
  });
...
function renderCraftSheet() {
  ...
  if (target.type === "duck" && target.alreadyCrafted) {
    ...
    craftSheetNote.textContent = "Painting consumes one White Pet Bed and the selected Paint, then adds the new color to your Furniture Inventory.";
  } else {
...
function renderShopItemSheet() {
  ...
  if (item.category === "furniture" && isPaintableFurnitureItem(item)) {
    ...
    if (remaining <= 0) {
      shopAffordMessage.textContent = "You already own the maximum 21 Pet Beds — one for every color. ♡";
      shopBuyButton.disabled = true;
      ...
      shopAffordMessage.textContent = "New Pet Beds start White. Use Furniture Crafting + Paint to recolor them.";
      shopBuyButton.disabled = false;

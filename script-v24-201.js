// Hub v24.166 — Duckie Dash shop + max-happiness reward repair
const STORAGE_KEY = "duckHabitHubSave_v1";
const SAVE_VERSION = 64;
...
const CHARACTERS = {
  ...
  io: {
    ...
    duckHeadPlacement: {
      // v24.119: Io's curl covered too much of her head duck. Shift only her
      // perch left so the duck stays visible while still looking seated.
      left: 45.0,
      top: 37.0,
      width: 17.2
    },
    ...
    profile: {
      height: "5'2''",
      favoriteItem: "Parfait, Champagne",
      likes: "Sweets, eating, drinking magical juice, quiet spaces, and relaxing by herself.",
      dislikes: "Being a magical girl, cliches, spicy foods.",
      description: "Io became aware of herself being a stereotypical magical girl. Pink hair, annoying other cliches. She tries desperately to avoid them, and tends to drink... a lot."
    },
    giftPreferences: {
      ...
      "pizza": "okay",
      "pudding": "okay",
      "strawberry-shortcake": "like",
      "strawberry": "like",
      "water": "okay",
      "watermelon": "okay",
      "wedding-cake": "not-her-thing",
      "bouquet": "not-her-thing",
      "heart-locket": "not-her-thing",
      "love-letter": "not-her-thing",
      "rings": "not-her-thing",
      "teddy-bear": "okay"
    }
  },
  miho: {
    ...
    giftPreferences: {
      ...
      "pizza": "okay",
      "pudding": "okay",
      "strawberry-shortcake": "not-her-thing",
      "strawberry": "okay",
      "water": "favorite",
      "watermelon": "okay",
      "wedding-cake": "not-her-thing",
  ...
  annika: {
    ...
    profile: {
      height: "5'6''",
      favoriteItem: "Strawberries! Anything including strawberries!",
      likes: "Sweets, taking care of those she loves, acrobatics.",
      dislikes: "Mushrooms, sour things, Midori.",
      description: "Annika is an acrobat at her family's circus. She's flexible, and talented. As well as kind and motherly. Annika isn't afraid to scold others if they do wrong."
    },
    giftPreferences: {
      // Change any "okay" below to "favorite", "like", or "not-her-thing" when you are ready.
      "bread-loaf": "okay",
...
const ITEMS = {
  "jump-token": {
    "name": "Jump Token",
    "category": "special",
    "image": "duck-quest/assets/dash/Jump-Token.png",
    "icon": "✦",
    "sellValue": 0
  },
  "common-egg": {
    "name": "Common Egg",
    "category": "special",
    "image": "duck-quest/assets/eggs/Common-egg.png",
    "icon": "🥚",
    "sellValue": 0
  },
  "rare-egg": {
    "name": "Rare Egg",
    "category": "special",
    "image": "duck-quest/assets/eggs/Rare-egg.png",
    "icon": "🥚",
    "sellValue": 0
  },
  "dash-cart-pink": {
    "name": "Pink Duckie Dash Cart",
    "category": "dash-cart",
    "image": "duck-quest/assets/dash/carts/Duck-car-pink.png",
    "icon": "🦆",
    "sellValue": 0
  },
  "dash-cart-mint": {
    "name": "Mint Duckie Dash Cart",
    "category": "dash-cart",
    "image": "duck-quest/assets/dash/carts/Duck-car-mint.png",
    "icon": "🦆",
    "sellValue": 0
  },
  "dash-cart-purple": {
    "name": "Purple Duckie Dash Cart",
    "category": "dash-cart",
    "image": "duck-quest/assets/dash/carts/Duck-car-purple.png",
    "icon": "🦆",
    "sellValue": 0
  },
  "dash-cart-black": {
    "name": "Black Duckie Dash Cart",
    "category": "dash-cart",
    "image": "duck-quest/assets/dash/carts/Duck-car-black.png",
    "icon": "🦆",
    "sellValue": 0
  },
  "buddy-pon": {
...
const SHOP_CATEGORIES = {
  ...
  food: {
    label: "Food",
    subtitle: "Stock up on treats to gift your OCs. Every giftable food item is available here."
  },
  paint: {
    label: "Paint",
    subtitle: "Buy the base paint colors here. Mixed colors still come from Paint Crafting!"
  },
  battle: {
    label: "Battle Items",
...
// v24.119: Food is built from the full ITEMS food catalog so no food can be
// accidentally left out of the Shop again. Prices keep the existing tiers.
const FOOD_SHOP_ORDER = Object.freeze([
  "water", "milk", "apple", "lemon", "lime", "strawberry", "cookie",
  "gummy-bear", "lollipop", "bread-loaf", "fruit-cup", "milk-tea",
  "pudding", "watermelon", "mushroom", "burger", "pizza", "cake-slice",
  "croissant", "cupcake", "fruit-tart", "strawberry-shortcake",
  "fancy-milk-tea", "concha", "cake", "champagne", "chocolate-brioche",
  "parfait", "wedding-cake"
]);
const FOOD_SHOP_PRICES = Object.freeze({
  water:50, milk:50, apple:50, lemon:50, lime:50, strawberry:50, cookie:50,
  "gummy-bear":50, lollipop:50, "bread-loaf":60, "fruit-cup":60,
  "milk-tea":60, pudding:60, watermelon:60, mushroom:60, burger:75, pizza:75,
  "cake-slice":75, croissant:75, cupcake:75, "fruit-tart":75,
  "strawberry-shortcake":90, "fancy-milk-tea":90, concha:90, cake:100,
  champagne:125, "chocolate-brioche":125, parfait:125, "wedding-cake":150
});
const ALL_FOOD_SHOP_IDS = Object.freeze([
  ...FOOD_SHOP_ORDER.filter(itemId => ITEMS[itemId]?.category === "food"),
  ...Object.keys(ITEMS)
    .filter(itemId => ITEMS[itemId]?.category === "food" && !FOOD_SHOP_ORDER.includes(itemId))
    .sort((a, b) => String(ITEMS[a]?.name || a).localeCompare(String(ITEMS[b]?.name || b)))
]);
function foodShopPrice(itemId) {
  return FOOD_SHOP_PRICES[itemId] ?? 60;
}

// Base colors are the paints that are not produced by PAINT_RECIPES.
// All are repeat-buy and cost 60 coins.
const BASE_PAINT_SHOP_IDS = Object.freeze([
  "red-paint", "orange-paint", "gold-paint", "silver-paint", "bronze-paint",
  "green-paint", "blue-paint", "purple-paint", "white-paint", "black-paint"
]);

const SHOP_STOCK = {
  supplies: [
    { itemId: "standard-duck", price: 75 },
    { itemId: "dash-cart-pink", price: 500 },
    { itemId: "dash-cart-mint", price: 500 },
    { itemId: "dash-cart-purple", price: 600 },
    { itemId: "dash-cart-black", price: 600 },
  ...
  food: ALL_FOOD_SHOP_IDS.map(itemId => ({ itemId, price: foodShopPrice(itemId) })),
  paint: BASE_PAINT_SHOP_IDS.map(itemId => ({ itemId, price: 60 })),
  battle: [
  ...
  profiles: [
    ...
    { characterId: "miho", price: 1500 },
    { characterId: "annika", price: 1500 }
  ],
  rooms: [
    ...
    { wallpaperId: "pink-gingham-wallpaper", price: 125 },
    { wallpaperId: "strawberry-cream-wallpaper", price: 135 },
    { wallpaperId: "mint-checker-wallpaper", price: 135 },
    { wallpaperId: "tiny-hearts-wallpaper", price: 145 },
    { wallpaperId: "daisy-picnic-wallpaper", price: 150 },
    { wallpaperId: "cloudy-blue-wallpaper", price: 155 },
    { wallpaperId: "lavender-stripes-wallpaper", price: 155 },
    { wallpaperId: "peach-polka-wallpaper", price: 165 },
    { wallpaperId: "rose-ribbon-wallpaper", price: 170 },
    { wallpaperId: "berry-bows-wallpaper", price: 175 },
    { wallpaperId: "soft-cloud-check-wallpaper", price: 175 },
    { wallpaperId: "midnight-starfall-wallpaper", price: 185 },
    { wallpaperId: "crown-confetti-wallpaper", price: 185 }
  ],
  furniture: [
  ...
  hair: [
    ...
    { wardrobeId: "miho-hair-long", price: 150 },
    { wardrobeId: "annika-hair-ponytail", price: 150 }
  ],
  clothing: [
    ...
    { wardrobeId: "miho-top-shirt", price: 150 },
    { wardrobeId: "miho-top-button-shirt", price: 150 },
    { wardrobeId: "miho-jacket-red", price: 150 },
    { wardrobeId: "miho-bottom-skirt", price: 150 },
  ...
  shoes: [
    ...
    { wardrobeId: "miho-shoes-booties", price: 150 },
    { wardrobeId: "annika-shoes-booties", price: 150 },
    { wardrobeId: "annika-shoes-folded-booties", price: 150 }
  ],
  accessories: [
...
const DUCKS = {
  "westley-duck": {
    "name": "Westley Duck",
    "file": "duck-quest/assets/love-interests/westley/Westley-duck.png",
    "acquisition": "love-interest-westley"
  },
  "yuzuru-duck": {
    "name": "Yuzuru Duck",
    "file": "duck-quest/assets/love-interests/yuzuru/Yuzuru-duck.png",
    "acquisition": "love-interest-yuzuru"
  },
  "devlin-duck": {
    "name": "Devlin Duck",
  ...
  "cheryln-duck": {
    ...
    "acquisition": "love-interest-cheryln"
  },
  "shinobu-duck": {
    "name": "Shinobu Duck",
    "file": "duck-quest/assets/love-interests/shinobu/Shinobu-duck.png",
    "acquisition": "love-interest-shinobu"
  },
  "lukio-duck": {
    "name": "Lukio Duck",
  ...
  "miho-duck": {
    "name": "Miho Duck",
    "file": "assets/ducks/miho-duck.webp",
    "acquisition": "character-happiness-100",
    "characterId": "miho"
  },
  "annika-duck": {
    "name": "Annika Duck",
    "file": "assets/ducks/annika-duck.png",
    "acquisition": "character-happiness-100",
    "characterId": "annika"
  },
  "orange-duck": {
...
const FRIENDSHIP_DATE_ICONS = Object.freeze({
  ...
  miho: "assets/ducks/miho-duck.webp",
  annika: "assets/ducks/annika-duck.png"
});
...
const ASSETS = {
  ...
  "candy-hairclip": { label: "Candy Hairclip", file: "Candy-Hairclip.webp", z: 56 }
};
...
const THUMB_BOUNDS = {
  ...
  "Daisy-Crown.png": [202,788,859,1007],
  "Sunglasses.png": [281,761,789,978],
  "Halo.png": [436,575,765,755],
  "Candy-Hairclip.webp": [307,995,382,1071]
};
...
const IO_THUMB_BOUNDS = {
  ...
  "Daisy-Crown.png": [202,770,859,989],
  "Sunglasses.png": [281,747,789,964],
  "Halo.png": [370,525,699,705]
};

const MIHO_THUMB_BOUNDS = {
  "Miho-angry.webp": [333,920,733,1158],
  "Miho-bangs.webp": [296,772,790,1272],
  "Miho-base.webp": [319,798,761,1794],
  "Miho-belt.webp": [445,1331,632,1364],
  "Miho-booties.webp": [382,1727,690,1808],
  "Miho-boots.webp": [384,1613,688,1803],
  "Miho-bow-low.webp": [708,758,918,969],
  "Miho-bow.webp": [298,628,829,968],
  "Miho-button-shirt.webp": [393,1216,671,1476],
  "Miho-dress.webp": [339,1221,729,1497],
  "Miho-flustered.webp": [326,922,745,1228],
  "Miho-hair.webp": [220,752,859,1254],
  "Miho-jacket-shop.webp": [379,1218,694,1374],
  "Miho-jacket.webp": [379,1218,694,1374],
  "Miho-long-hair.webp": [139,747,941,1502],
  "Miho-neck-bow.webp": [477,1214,595,1315],
  "Miho-neutral.webp": [333,916,733,1129],
  "Miho-red-jacket-shop.webp": [327,1231,738,1458],
  "Miho-red-jacket.webp": [327,1232,738,1458],
  "Miho-shirt.webp": [428,1223,645,1385],
  "Miho-shorts.webp": [381,1379,684,1488],
  "Miho-skirt.webp": [334,1344,738,1531],
  "Miho-slit-skirt.webp": [349,1388,747,1533],
  "Miho-stockings-laceup.webp": [388,1414,683,1797],
  "Miho-stockings.webp": [388,1414,683,1797],
  "Daisy-Crown.png": [202,760,859,979],
  "Sunglasses.png": [281,743,789,960],
  "Halo.png": [370,548,699,728],
  "Candy-Hairclip.webp": [319,977,394,1053]
};
...
const IO_CLOSET = [
  ...
  { id: "extras", label: "Extras", type: "multi", options: ["ahoge", "heart-pin", "sheep-ears", "crown", "mew-garter", "mew-jacket", "mew-collar", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"] }
];
...
const MIHO_ASSETS = {
  ...
  "legwear-stockings-laceup": { label: "Lace-Up Stockings", file: "Miho-stockings-laceup.webp", z: 30 },
  "dress-black": { label: "Black Dress", file: "Miho-dress.webp", z: 33 },
  "top-shirt": { label: "White Shirt", file: "Miho-shirt.webp", z: 33 },
  ...
  "jacket-black": { label: "Black Jacket", file: "Miho-jacket.webp", previewFile: "Miho-jacket-shop.webp", z: 36 },
  "jacket-red": { label: "Red Jacket", file: "Miho-red-jacket.webp", previewFile: "Miho-red-jacket-shop.webp", z: 36 },
  "shoes-boots": { label: "Boots", file: "Miho-boots.webp", z: 31 },
  "shoes-booties": { label: "Booties", file: "Miho-booties.webp", z: 31 },
  "expression-neutral": { label: "Neutral", file: "Miho-neutral.webp", z: 46 },
  "expression-happy": { label: "Flustered", file: "Miho-flustered.webp", z: 46 },
  "expression-sad": { label: "Neutral", file: "Miho-neutral.webp", z: 46 },
  "expression-shocked": { label: "Flustered", file: "Miho-flustered.webp", z: 46 },
  "expression-mad": { label: "Angry", file: "Miho-angry.webp", z: 46 },
  "bow": { label: "Head Bow", file: "Miho-bow.webp", z: 11 },
  "bow-low": { label: "Low Bow", file: "Miho-bow-low.webp", z: 11 },
  "neck-bow": { label: "Neck Bow", file: "Miho-neck-bow.webp", z: 34 },
  "bangs": { label: "Bangs", file: "Miho-bangs.webp", z: 47 },
  "daisy-crown": { label: "Daisy Crown", file: "Daisy-Crown.png", z: 53 },
  "ocean-sunglasses": { label: "Sunglasses", file: "Sunglasses.png", z: 54 },
  "halo": { label: "Halo", file: "Halo.png", z: 55 },
  "candy-hairclip": { label: "Candy Hairclip", file: "Candy-Hairclip.webp", z: 56 }
...
const MIHO_CLOSET = [
  { id: "hair", label: "Hair", type: "hair", options: ["hair-main", "hair-long"] },
  { id: "headBow", label: "Head Bows", type: "single", allowNone: true, options: ["bow", "bow-low"] },
  { id: "top", label: "Tops", type: "single", allowNone: true, options: ["top-shirt", "top-button-shirt"] },
  { id: "outer", label: "Layers", type: "single", allowNone: true, options: ["jacket-black", "jacket-red"] },
  { id: "bottom", label: "Bottoms", type: "single", allowNone: true, options: ["bottom-skirt", "bottom-slit-skirt", "bottom-shorts"] },
  { id: "dress", label: "Dresses", type: "single", allowNone: true, options: ["dress-black"] },
  { id: "legwear", label: "Legwear", type: "single", allowNone: true, options: ["legwear-stockings", "legwear-stockings-laceup"] },
  { id: "shoes", label: "Shoes", type: "single", allowNone: true, options: ["shoes-boots", "shoes-booties"] },
  { id: "extras", label: "Extras", type: "multi", options: ["belt", "neck-bow", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"] }
];

const ANNIKA_THUMB_BOUNDS = {
  "Annika-angry.png": [335,934,728,1164],
  "Annika-bangs.png": [293,786,774,1376],
  "Annika-base-body.png": [319,798,761,1794],
  "Annika-black-stockings.png": [386,1402,685,1799],
  "Annika-booties.png": [381,1725,686,1805],
  "Annika-bow-stockings.png": [377,1512,692,1794],
  "Annika-bow-stockings-shop.png": [377,1512,692,1794],
  "Annika-bow-sweater.png": [346,1220,744,1509],
  "Annika-circus-back-bow.png": [190,1274,872,1769],
  "Annika-circus-stockings.png": [386,1401,687,1794],
  "Annika-collared-shirt.png": [404,1217,662,1455],
  "Annika-folded-booties.png": [382,1744,688,1806],
  "Annika-hair-bow-high.png": [321,679,753,868],
  "Annika-hair-ribbon.png": [184,728,904,1274],
  "Annika-happy.png": [280,976,776,1236],
  "Annika-leotard.png": [411,1254,655,1471],
  "Annika-long-hair.png": [193,767,875,1595],
  "Annika-neck-bow.png": [473,1247,591,1347],
  "Annika-neutral.png": [335,957,728,1144],
  "Annika-ponytail.png": [471,724,874,1477],
  "Annika-right-bow-stocking.png": [547,1512,692,1793],
  "Annika-left-bow-stocking.png": [377,1513,522,1794],
  "Annika-sad.png": [335,957,728,1150],
  "Annika-scarf.png": [425,1175,661,1370],
  "Annika-sheer-shirt.png": [406,1229,664,1400],
  "Annika-shorts.png": [383,1400,688,1485],
  "Annika-sneakers.png": [384,1729,688,1811],
  "Annika-sweater.png": [326,1246,735,1465],
  "Daisy-Crown.png": [202,760,859,979],
  "Sunglasses.png": [281,743,789,960],
  "Halo.png": [370,548,699,728],
  "Candy-Hairclip.webp": [307,987,382,1063]
};
...
const ANNIKA_ASSETS = {
  ...
  "neck-bow": { label: "Neck Bow", file: "Annika-neck-bow.png", z: 33 },
  "expression-neutral": { label: "Neutral", file: "Annika-neutral.png", z: 45 },
  "expression-happy": { label: "Happy", file: "Annika-happy.png", z: 45 },
  "expression-sad": { label: "Sad", file: "Annika-sad.png", z: 45 },
...
const ANNIKA_CLOSET = [
  ...
  { id: "extras", label: "Extras", type: "multi", options: ["neck-bow", "scarf", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"] }
];
...
const MIKO_CLOSET = [
  ...
  { id: "outer", label: "Layers", type: "single", allowNone: true, options: ["top-hoodie", "top-sweater", "top-big-shirt", "outer-black-blazer", "lukio-hoodie", "shino-sweater", "cheryln-sweater", "hibiki-coat", "devlin-vest", "yuzuru-shirt", "westley-jacket"] },
  { id: "bottom", label: "Bottoms", type: "single", options: ["bottom-capris", "bottom-jeans", "bottom-boxers", "bottom-shorts", "lukio-shorts", "shino-jeans", "cheryln-shorts", "hibiki-shorts", "devlin-pants", "yuzuru-shorts", "westley-pants"] },
  { id: "socks", label: "Socks", type: "single", allowNone: true, options: ["socks", "socks-garter", "lukio-socks", "cheryln-tights", "hibiki-stockings", "yuzuru-socks"] },
  { id: "shoes", label: "Shoes", type: "single", allowNone: true, options: ["shoes-loafer", "shoes-fancy-loafers", "lukio-booties", "shino-boots", "cheryln-boots", "hibiki-boots", "devlin-loafers", "yuzuru-shoes", "westley-boots"] },
  {
    id: "extras",
    label: "Extras",
    type: "multi",
...
const CLOSET = [
  ...
  { id: "tail", label: "Tails", type: "single", allowNone: true, options: ["tail-cow", "tail-bunny"] },
  {
    ...
    label: "Extras",
    type: "multi",
    options: ["jacket", "cardigan-white", "collar", "leg-bandage", "horns", "cheek-bandage", "left-bow", "right-bow", "bow-white", "hair-side-ribbon", "large-back-bow", "beret", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"]
  }

];

const WALLPAPERS = Object.freeze([
  { id: "pink-gingham-wallpaper", name: "Pink Gingham", source: "shop", price: 125, value: "linear-gradient(90deg,rgba(255,255,255,.42) 50%,transparent 50%),linear-gradient(rgba(255,255,255,.42) 50%,transparent 50%),#efb8ca", size: "26px 26px", opacity: .60 },
  { id: "strawberry-cream-wallpaper", name: "Strawberry Cream", source: "shop", price: 135, value: "radial-gradient(circle at 8px 8px,#fff6fa 0 3px,transparent 3.5px),linear-gradient(145deg,#f2bfd0,#f9d9e6)", size: "24px 24px,auto", opacity: .58 },
  { id: "mint-checker-wallpaper", name: "Mint Checker", source: "shop", price: 135, value: "conic-gradient(#dff4e7 25%,#bfe1ca 0 50%,#dff4e7 0 75%,#bfe1ca 0)", size: "24px 24px", opacity: .56 },
  { id: "tiny-hearts-wallpaper", name: "Tiny Hearts", source: "shop", price: 145, value: "radial-gradient(circle at 25% 35%,rgba(255,255,255,.95) 0 3px,transparent 3.5px),radial-gradient(circle at 75% 65%,rgba(255,255,255,.95) 0 3px,transparent 3.5px),linear-gradient(135deg,#f4b7cb,#e6c1ef)", size: "28px 28px", opacity: .58 },
  { id: "daisy-picnic-wallpaper", name: "Daisy Picnic", source: "shop", price: 150, value: "radial-gradient(circle at 9px 9px,#fffdf9 0 4px,transparent 4.6px),radial-gradient(circle at 9px 9px,#f5d67a 0 1.4px,transparent 1.7px),#f3b7c9", size: "30px 30px,30px 30px,auto", opacity: .60 },
  { id: "cloudy-blue-wallpaper", name: "Cloudy Blue", source: "shop", price: 155, value: "radial-gradient(ellipse at 20% 35%,rgba(255,255,255,.72) 0 10px,transparent 11px),radial-gradient(ellipse at 74% 62%,rgba(255,255,255,.62) 0 11px,transparent 12px),linear-gradient(145deg,#b8ddf2,#d8edf7)", size: "52px 36px,58px 40px,auto", opacity: .52 },
  { id: "lavender-stripes-wallpaper", name: "Lavender Stripes", source: "shop", price: 155, value: "repeating-linear-gradient(135deg,rgba(255,255,255,.42) 0 10px,transparent 10px 22px),linear-gradient(135deg,#d6c3ee,#c5b0e7)", size: "34px 34px,auto", opacity: .56 },
  { id: "peach-polka-wallpaper", name: "Peach Polka Dots", source: "shop", price: 165, value: "radial-gradient(circle at 8px 8px,#fff8f0 0 3px,transparent 3.6px),#efbea8", size: "24px 24px", opacity: .58 },
  { id: "rose-ribbon-wallpaper", name: "Rose Ribbon", source: "shop", price: 170, value: "repeating-linear-gradient(135deg,rgba(255,255,255,.38) 0 9px,transparent 9px 20px),linear-gradient(145deg,#efb1c8,#f7d6e3)", size: "28px 28px,auto", opacity: .57 },
  { id: "berry-bows-wallpaper", name: "Berry Bows", source: "shop", price: 175, value: "radial-gradient(circle at 8px 8px,rgba(255,255,255,.82) 0 3px,transparent 3.6px),radial-gradient(circle at 20px 20px,rgba(255,219,233,.88) 0 3px,transparent 3.8px),linear-gradient(135deg,#d989a8,#f0bfd2)", size: "28px 28px,28px 28px,auto", opacity: .56 },
  { id: "soft-cloud-check-wallpaper", name: "Soft Cloud Check", source: "shop", price: 175, value: "linear-gradient(90deg,rgba(255,255,255,.38) 50%,transparent 50%),linear-gradient(rgba(255,255,255,.38) 50%,transparent 50%),linear-gradient(145deg,#c8e4f7,#f3d5e5)", size: "26px 26px,26px 26px,auto", opacity: .55 },
  { id: "midnight-starfall-wallpaper", name: "Midnight Starfall", source: "shop", price: 185, value: "radial-gradient(circle at 16% 24%,#fff1b8 0 1.8px,transparent 2.4px),radial-gradient(circle at 74% 32%,#ffffff 0 1.3px,transparent 1.9px),radial-gradient(circle at 48% 78%,#cfd8ff 0 1.5px,transparent 2.1px),linear-gradient(150deg,#6670b7,#8b7cc6,#b9acd9)", size: "28px 28px,34px 34px,24px 24px,auto", opacity: .54 },
  { id: "crown-confetti-wallpaper", name: "Crown Confetti", source: "shop", price: 185, value: "radial-gradient(circle at 18% 28%,rgba(255,255,255,.92) 0 2px,transparent 2.6px),radial-gradient(circle at 72% 38%,rgba(255,239,181,.88) 0 2px,transparent 2.6px),linear-gradient(135deg,#eac6f1,#f2d6df,#c7d8f2)", size: "30px 30px,24px 24px,auto", opacity: .55 },

  { id: "starry-night-wallpaper", name: "Starry Night", source: "quest", rarity: "rare", weight: 1.8, value: "radial-gradient(circle at 18% 22%,#fff6bf 0 2px,transparent 2.6px),radial-gradient(circle at 72% 34%,#fff 0 1.5px,transparent 2px),radial-gradient(circle at 48% 78%,#fff6bf 0 1.5px,transparent 2px),linear-gradient(160deg,#8d97d7,#b9b1ea)", size: "30px 30px,26px 26px,34px 34px,auto", opacity: .54 },
  { id: "petal-dream-wallpaper", name: "Petal Dream", source: "quest", rarity: "rare", weight: 1.05, value: "radial-gradient(circle at 24% 30%,rgba(255,245,252,.92) 0 3px,transparent 3.7px),radial-gradient(circle at 70% 68%,rgba(255,229,239,.9) 0 3px,transparent 3.7px),linear-gradient(145deg,#f0bfd2,#edd7f6)", size: "32px 32px,28px 28px,auto", opacity: .57 },
  { id: "magical-sparkle-wallpaper", name: "Magical Sparkle", source: "quest", rarity: "rare", weight: 1.2, value: "radial-gradient(circle at 15% 20%,#fff 0 1.5px,transparent 2px),radial-gradient(circle at 72% 28%,#ffe9a8 0 1.3px,transparent 1.8px),radial-gradient(circle at 38% 75%,#fff 0 1px,transparent 1.7px),linear-gradient(135deg,#eda9c4,#d995c1,#c5b0ea)", size: "22px 22px,28px 28px,18px 18px,auto", opacity: .58 },
  { id: "rainbow-dream-wallpaper", name: "Rainbow Dream", source: "quest", rarity: "ultra", weight: .7, value: "linear-gradient(90deg,rgba(255,255,255,.36) 50%,transparent 50%),linear-gradient(rgba(255,255,255,.36) 50%,transparent 50%),linear-gradient(135deg,#f1adc4,#f6d299,#cfe4af,#b7ddf1,#cab6ef,#ecbddf)", size: "26px 26px,26px 26px,auto", opacity: .55 },
  { id: "moonlit-hearts-wallpaper", name: "Moonlit Hearts", source: "quest", rarity: "ultra", weight: .45, value: "radial-gradient(circle at 18% 26%,#f8bfdc 0 2.5px,transparent 3.1px),radial-gradient(circle at 72% 39%,#d5c2ff 0 2.4px,transparent 3px),radial-gradient(circle at 44% 79%,#fff3b9 0 1.7px,transparent 2.4px),linear-gradient(150deg,#53538c,#715d9a,#a693d4)", size: "29px 29px,36px 36px,24px 24px,auto", opacity: .50 },
  { id: "royal-celestial-wallpaper", name: "Royal Celestial", source: "quest", rarity: "ultra", weight: .38, value: "radial-gradient(circle at 16% 24%,#fff4bc 0 2px,transparent 2.7px),radial-gradient(circle at 76% 28%,#fff 0 1.6px,transparent 2.2px),radial-gradient(circle at 46% 76%,#ffd7ea 0 1.9px,transparent 2.6px),linear-gradient(150deg,#6a5e9f,#8a76bb,#c8b7e9)", size: "26px 26px,34px 34px,30px 30px,auto", opacity: .52 }
]);
...
function normalizeMihoOutfit(rawOutfit = {}) {
  ...
  normalized.extras = Array.isArray(normalized.extras)
    ? normalized.extras.filter(id => ["belt", "neck-bow", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"].includes(id))
    : [...DEFAULT_MIHO_OUTFIT.extras];
  return normalized;
}

function normalizeAnnikaOutfit(rawOutfit = {}) {
  const incoming = rawOutfit && typeof rawOutfit === "object" ? rawOutfit : {};
  ...
  normalized.extras = Array.isArray(normalized.extras)
    ? normalized.extras.filter(id => ["neck-bow", "scarf", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"].includes(id))
...
function normalizeIoOutfit(rawOutfit = {}) {
  ...
  normalized.extras = Array.isArray(normalized.extras)
    ? normalized.extras.filter(id => ["ahoge", "heart-pin", "sheep-ears", "crown", "mew-garter", "mew-jacket", "mew-collar", "daisy-crown", "ocean-sunglasses", "halo", "candy-hairclip"].includes(id))
    : ["heart-pin"];
  return normalized;
}
...
function normalizeMikoOutfit(rawOutfit = {}) {
  ...
  if (![null, "top-hoodie", "top-sweater", "top-big-shirt", "outer-black-blazer", "lukio-hoodie", "shino-sweater", "cheryln-sweater", "hibiki-coat", "devlin-vest", "yuzuru-shirt", "westley-jacket"].includes(outer)) outer = "top-hoodie";

  const bangsStyle = ["bangs", "bangs-pinned"].includes(incoming.bangsStyle)
    ? incoming.bangsStyle
    : "bangs";
  const bottom = ["bottom-capris", "bottom-jeans", "bottom-boxers", "bottom-shorts", "lukio-shorts", "shino-jeans", "cheryln-shorts", "hibiki-shorts", "devlin-pants", "yuzuru-shorts", "westley-pants"].includes(incoming.bottom)
    ? incoming.bottom
    : "bottom-capris";
  const socks = ["socks", "socks-garter", "lukio-socks", "cheryln-tights", "hibiki-stockings", "yuzuru-socks"].includes(incoming.socks) ? incoming.socks : null;
  const shoes = [null, "shoes-loafer", "shoes-fancy-loafers", "lukio-booties", "shino-boots", "cheryln-boots", "hibiki-boots", "devlin-loafers", "yuzuru-shoes", "westley-boots"].includes(incoming.shoes)
    ? incoming.shoes
    : "shoes-loafer";
...
const DEFAULT_SAVE = {
  ...
  characterOutfits: {
    ...
    miho: structuredClone(DEFAULT_MIHO_OUTFIT),
    annika: structuredClone(DEFAULT_ANNIKA_OUTFIT)
  },
  characterProgress: {
    peep: { happinessTotal: 0 },
    miko: { happinessTotal: 0 },
    io: { happinessTotal: 0 },
    miho: { happinessTotal: 0 },
    annika: { happinessTotal: 0 }
  },
  tasks: [],
  savedTaskTemplates: [],
  ...
  roomExpansion: {
    ...
    byCharacter: {
      ...
      miho: { accentRoom: null, shelfStyle: "cream", duckSlots: Array(24).fill(null) },
      annika: { accentRoom: null, shelfStyle: "cream", duckSlots: Array(24).fill(null) }
    }
  },
  buddies: {
    collection: {},
    equippedByCharacter: {
      ...
      io: emptyBuddySlots(),
      miho: emptyBuddySlots(),
      annika: emptyBuddySlots()
    },
    personalizationByCharacter: {
      ...
      miho: emptyBuddyPersonalizationSlots(),
      annika: emptyBuddyPersonalizationSlots()
    }
  },
  achievements: {
    completed: [],
    ...
    const merged = {
      ...
      roomsByCharacter: {
        ...
        io: ROOMS.some(room => room.id === saved.roomsByCharacter?.io)
          ...
          : "cream",
        miho: ROOMS.some(room => room.id === saved.roomsByCharacter?.miho)
          ? saved.roomsByCharacter.miho
          : "cream"
      },
      ...
      characterUnlockedItems: {
        ...
        miho: Array.isArray(saved.characterUnlockedItems?.miho)
          ? [...new Set(saved.characterUnlockedItems.miho)]
          : [...structuredClone(DEFAULT_SAVE.characterUnlockedItems.miho)],
        annika: Array.isArray(saved.characterUnlockedItems?.annika)
          ? [...new Set(saved.characterUnlockedItems.annika)]
          : [...structuredClone(DEFAULT_SAVE.characterUnlockedItems.annika)]
      },
      ...
      characterOutfits: {
        ...
        annika: normalizeAnnikaOutfit(saved.characterOutfits?.annika || {})
      },
      ...
      characterProgress: {
        ...
        miho: normalizeLoadedCharacterProgress(saved.characterProgress?.miho, 0),
        annika: normalizeLoadedCharacterProgress(saved.characterProgress?.annika, 0)
      },
...
const GACHA_EXCLUDED_DUCKS = new Set(["tiny-duck", "tiny-duck-stack", "pile-of-tiny-ducks", "lukio-duck", "shinobu-duck", "cheryln-duck", "hibiki-duck", "devlin-duck", "yuzuru-duck", "westley-duck"]);
...
function getCharacterAssetMap(characterId = save.selectedCharacter) {
  ...
  if (characterId === "miho") return MIHO_ASSETS;
  if (characterId === "annika") return ANNIKA_ASSETS;
  return ASSETS;
}
...
function getCharacterCloset(characterId = save.selectedCharacter) {
  ...
  if (characterId === "miho") return MIHO_CLOSET;
  if (characterId === "annika") return ANNIKA_CLOSET;
  return CLOSET;
}
...
function getCharacterThumbBounds(characterId = save.selectedCharacter) {
  ...
  if (characterId === "miho") return MIHO_THUMB_BOUNDS;
  if (characterId === "annika") return ANNIKA_THUMB_BOUNDS;
  return THUMB_BOUNDS;
}
...
function getCharacterDefaultOutfit(characterId = save.selectedCharacter) {
  ...
  if (characterId === "miho") return DEFAULT_MIHO_OUTFIT;
  if (characterId === "annika") return DEFAULT_ANNIKA_OUTFIT;
...
function normalizeCharacterState() {
  ...
  const mihoUnlocks = new Set([
    ...(Array.isArray(save.characterUnlockedItems.miho) ? save.characterUnlockedItems.miho : []),
    ...getCharacterStarterWardrobe("miho")
  ]);
  const annikaUnlocks = new Set([
  ...
  save.characterUnlockedItems.miho = [...mihoUnlocks];
  save.characterUnlockedItems.annika = [...annikaUnlocks];
  ...
  save.characterProgress.miho = normalizeLoadedCharacterProgress(save.characterProgress.miho, 0);
  save.characterProgress.annika = normalizeLoadedCharacterProgress(save.characterProgress.annika, 0);
...
function isCharacterUnlocked(characterId) {
  ...
  if (id === "peep") return true;
  return Array.isArray(save.unlockedCharacters) && save.unlockedCharacters.includes(id);
}
...
function addCharacterHappiness(amount, characterId = save.selectedCharacter) {
  ...
  if (gained > 0 || progress.happinessTotal >= MAX_CHARACTER_HAPPINESS_TOTAL) {
    // This runs during normal gameplay, after the UI has fully initialized.
    // The max check also repairs a missing Level-100 duck for an already-maxed OC.
...
const roomWallpaperOverlay = document.querySelector("#roomWallpaperOverlay");
...
const statusCandyRank = document.querySelector("#statusCandyRank");
const statusCloudRank = document.querySelector("#statusCloudRank");
...
function normalizeDuckQuestCharacterProgress(characterId = save.selectedCharacter) {
  ...
  const rawMeadowUnlocked = Number(existing.areas?.meadow?.unlockedRank ?? legacyAreas.meadow?.unlockedRank ?? (id === "peep" ? quest.unlockedRank : 1)) || 1;
  const rawMeadowLast = Number(existing.areas?.meadow?.lastRank ?? legacyAreas.meadow?.lastRank ?? (id === "peep" ? quest.lastRank : 1)) || 1;
  const rawOceanUnlocked = Number(existing.areas?.ocean?.unlockedRank ?? legacyAreas.ocean?.unlockedRank) || 1;
  const rawOceanLast = Number(existing.areas?.ocean?.lastRank ?? legacyAreas.ocean?.lastRank) || 1;
  const rawCandyUnlocked = Number(existing.areas?.candy?.unlockedRank ?? legacyAreas.candy?.unlockedRank) || 1;
  const rawCandyLast = Number(existing.areas?.candy?.lastRank ?? legacyAreas.candy?.lastRank) || 1;
  const rawCloudUnlocked = Number(existing.areas?.cloud?.unlockedRank ?? legacyAreas.cloud?.unlockedRank) || 1;
  const rawCloudLast = Number(existing.areas?.cloud?.lastRank ?? legacyAreas.cloud?.lastRank) || 1;

  const meadowUnlocked = Math.max(1, Math.min(40, rawMeadowUnlocked));
  const meadowLast = Math.max(1, Math.min(meadowUnlocked, rawMeadowLast));
  const oceanUnlocked = Math.max(1, Math.min(80, rawOceanUnlocked));
  const oceanLast = Math.max(1, Math.min(oceanUnlocked, rawOceanLast));
  const candyUnlocked = Math.max(1, Math.min(120, rawCandyUnlocked));
  const candyLast = Math.max(1, Math.min(candyUnlocked, rawCandyLast));
  const cloudUnlocked = Math.max(1, Math.min(160, rawCloudUnlocked));
  const cloudLast = Math.max(1, Math.min(cloudUnlocked, rawCloudLast));
  ...
  const requestedLastArea = existing.lastArea ?? (id === "peep" ? quest.lastArea : null);
  const validAreas = ["meadow", "ocean", "candy", "cloud"];
  ...
  quest[id] = {
    ...
    lastArea: validAreas.includes(requestedLastArea) ? requestedLastArea : "meadow",
    areas: {
      meadow: { unlockedRank: meadowUnlocked, lastRank: meadowLast },
      ocean: { unlockedRank: oceanUnlocked, lastRank: oceanLast },
      candy: { unlockedRank: candyUnlocked, lastRank: candyLast },
      cloud: { unlockedRank: cloudUnlocked, lastRank: cloudLast }
    },
    clearedAreas: {
      meadow: Boolean(existing.clearedAreas?.meadow || rawMeadowLast >= 40 || rawMeadowUnlocked >= 40),
      // Ocean/Cloud used larger legacy caps. Preserve only true old clears here;
      // new clears are written by Duck Quest itself at 80/160.
      ocean: Boolean(existing.clearedAreas?.ocean || rawOceanLast >= 100 || rawOceanUnlocked >= 100),
      candy: Boolean(existing.clearedAreas?.candy),
      cloud: Boolean(existing.clearedAreas?.cloud || rawCloudLast >= 150 || rawCloudUnlocked >= 150)
    },
    endless: {
      record: Math.max(0, Math.floor(Number(existing.endless?.record) || 0)),
      checkpoint: Math.max(1, Math.floor(Number(existing.endless?.checkpoint) || 1))
    }
  };
  ...
  // Keep the older Peep mirrors intact for old backups/older code without
  // stripping Candyland or Cloud Garden from the per-character save.
  if (id === "peep") {
...
function renderRoom() {
  ...
  updateRoomPickerButtonDisplay();
  applyCurrentRoomWallpaper();
...
function getMihoEquippedAssetIds() {
  const outfit = getCharacterOutfit("miho");
  const ids = [outfit.hair, "base"];
  if (outfit.legwear) ids.push(outfit.legwear);
  if (outfit.dress) ids.push(outfit.dress);
  else ids.push(outfit.bottom, outfit.top);
  ids.push(...(Array.isArray(outfit.extras) ? outfit.extras : []));
...
function getCharacterAssetIds(characterId = save.selectedCharacter) {
  ...
  if (character.id === "miho") return getMihoEquippedAssetIds();
  if (character.id === "annika") return getAnnikaEquippedAssetIds();
...
function renderRoomPicker() {
  ...
  const currentRoomId = currentDisplayedRoomId();
  const currentWallpaperId = currentDisplayedWallpaperId();

  const clearButton = document.createElement("button");
  clearButton.type = "button";
  clearButton.className = "room-choice wallpaper-choice base-wallpaper-choice";
  clearButton.setAttribute("aria-current", String(!currentWallpaperId));
  clearButton.setAttribute("aria-label", "No special wallpaper");
  clearButton.title = "No Special Wallpaper";
  const clearSwatch = document.createElement("span");
  clearSwatch.className = "room-swatch wallpaper-swatch";
  applyWallpaperPreviewStyle(clearSwatch, null);
  clearButton.append(clearSwatch);
  clearButton.addEventListener("click", () => {
    setCharacterWallpaperId(null);
    persist();
    renderRoom();
    renderRoomPicker();
    closeRoomPicker();
  });
  roomPicker.append(clearButton);
  ...
  for (const room of ROOMS) {
    ...
    button.setAttribute("aria-current", String(!currentWallpaperId && currentRoomId === room.id));
    ...
    swatch.style.background = room.swatch;
  ...
  for (const wallpaper of WALLPAPERS) {
    const unlocked = isWallpaperUnlocked(wallpaper.id);
    const button = document.createElement("button");
    button.type = "button";
    button.className = `room-choice wallpaper-choice${unlocked ? "" : " locked"}`;
    button.setAttribute("aria-current", String(currentWallpaperId === wallpaper.id));
    button.setAttribute("aria-label", `${wallpaper.name}${unlocked ? "" : ", locked"}`);
    button.title = `${wallpaper.name}${wallpaper.source === "quest" ? " • Duck Quest" : ""}`;

    const swatch = document.createElement("span");
    swatch.className = "room-swatch wallpaper-swatch";
    applyWallpaperPreviewStyle(swatch, wallpaper);
    button.append(swatch);
    ...
    button.addEventListener("click", () => {
      if (!unlocked) {
        showToast(wallpaper.source === "quest"
          ? "Locked — find this special wallpaper in Duck Quest!"
          : `Locked — unlock this wallpaper in the Shop for ${wallpaper.price} Pink Coins!`);
        return;
      }
      setCharacterWallpaperId(wallpaper.id);
      persist();
      renderRoom();
      renderRoomPicker();
      closeRoomPicker();
...
const CHARACTER_PREVIEW_THUMB_FILES = {
  ...
  miho: new Set(["Miho-angry.webp", "Miho-bangs.webp", "Miho-base.webp", "Miho-belt.webp", "Miho-booties.webp", "Miho-boots.webp", "Miho-bow-low.webp", "Miho-bow.webp", "Miho-button-shirt.webp", "Miho-dress.webp", "Miho-flustered.webp", "Miho-hair.webp", "Miho-jacket-shop.webp", "Miho-jacket.webp", "Miho-long-hair.webp", "Miho-neck-bow.webp", "Miho-neutral.webp", "Miho-red-jacket-shop.webp", "Miho-red-jacket.webp", "Miho-shirt.webp", "Miho-shorts.webp", "Miho-skirt.webp", "Miho-slit-skirt.webp", "Miho-stockings-laceup.webp", "Miho-stockings.webp"])
};
...
function chooseSingle(group, id) {
  ...
  if (save.selectedCharacter === "peep" || save.selectedCharacter === "io" || save.selectedCharacter === "miho" || save.selectedCharacter === "annika") {
    ...
    if ((save.selectedCharacter === "peep" || save.selectedCharacter === "miho" || save.selectedCharacter === "annika") && group.id === "legwear" && id) {
...
const WARDROBE_SHOP_META = {
  ...
  "miho-hair-long": { characterId: "miho", label: "Miho · Long Hair", assetIds: ["hair-long"], unlockIds: ["hair-long"] },
  "miho-top-shirt": { characterId: "miho", label: "Miho · White Shirt", assetIds: ["top-shirt"], unlockIds: ["top-shirt"] },
  "miho-top-button-shirt": { characterId: "miho", label: "Miho · Button Shirt", assetIds: ["top-button-shirt"], unlockIds: ["top-button-shirt"] },
  "miho-jacket-red": { characterId: "miho", label: "Miho · Red Jacket", assetIds: ["jacket-red"], unlockIds: ["jacket-red"] },
  "miho-bottom-skirt": { characterId: "miho", label: "Miho · Skirt", assetIds: ["bottom-skirt"], unlockIds: ["bottom-skirt"] },
  "miho-bottom-slit-skirt": { characterId: "miho", label: "Miho · Slit Skirt", assetIds: ["bottom-slit-skirt"], unlockIds: ["bottom-slit-skirt"] },
  "miho-bottom-shorts": { characterId: "miho", label: "Miho · Shorts", assetIds: ["bottom-shorts"], unlockIds: ["bottom-shorts"] },
  "miho-legwear-laceup": { characterId: "miho", label: "Miho · Lace-Up Stockings", assetIds: ["legwear-stockings-laceup"], unlockIds: ["legwear-stockings-laceup"] },
  "miho-shoes-booties": { characterId: "miho", label: "Miho · Booties", assetIds: ["shoes-booties"], unlockIds: ["shoes-booties"] },
  "miho-bow-low": { characterId: "miho", label: "Miho · Low Bow", assetIds: ["bow-low"], unlockIds: ["bow-low"] },
...
function ensureStarterWardrobeUnlocked() {
  ...
  const miho = new Set(getCharacterUnlockedItems("miho"));
  getCharacterStarterWardrobe("miho").forEach(id => miho.add(id));
  save.characterUnlockedItems.miho = [...miho];
...
function launchDuckQuestGame() {
  window.location.href = "duck-quest/?v=31";
}
...
function duckDiscoveryHint(duck) {
  ...
  switch (duck.acquisition) {
    case "love-interest-westley":
      return "Meet Westley while exploring Duck Quest as Miko to unlock this special duck.";
    case "love-interest-yuzuru":
      return "Meet Yuzuru while exploring Duck Quest as Miko to unlock this special duck.";
    ...
    case "love-interest-shinobu":
      return "Meet Shinobu while exploring Duck Quest as Miko to unlock this special duck.";
...
function normalizeRoomWallpaperState() {
  if (!Array.isArray(save.unlockedWallpapers)) save.unlockedWallpapers = [];
  save.unlockedWallpapers = [...new Set(save.unlockedWallpapers)]
    .filter(id => WALLPAPERS.some(wallpaper => wallpaper.id === id));

  if (!save.roomWallpapersByCharacter || typeof save.roomWallpapersByCharacter !== "object") {
    save.roomWallpapersByCharacter = {};
  }

  for (const characterId of Object.keys(CHARACTERS)) {
    let selected = save.roomWallpapersByCharacter[characterId] ?? null;
    if (!wallpaperById(selected) || !save.unlockedWallpapers.includes(selected)) selected = null;
    save.roomWallpapersByCharacter[characterId] = selected;
  }

  return save.roomWallpapersByCharacter;
}

function isWallpaperUnlocked(wallpaperId) {
  if (!wallpaperId) return false;
  normalizeRoomWallpaperState();
  return save.unlockedWallpapers.includes(wallpaperId);
}
...
function unlockWallpaper(wallpaperId) {
  ...
  normalizeRoomWallpaperState();
  if (save.unlockedWallpapers.includes(wallpaperId)) return null;
  save.unlockedWallpapers.push(wallpaperId);
  return wallpaper;
}

function currentCharacterWallpaperId(characterId = save.selectedCharacter) {
  normalizeRoomWallpaperState();
  return save.roomWallpapersByCharacter[characterId] || null;
}

function currentDisplayedWallpaperId() {
  return currentCharacterWallpaperId(save.selectedCharacter);
}

function currentDisplayedWallpaper() {
  return wallpaperById(currentDisplayedWallpaperId());
}

function setCharacterWallpaperId(wallpaperId = null, characterId = save.selectedCharacter) {
  normalizeRoomWallpaperState();
  save.roomWallpapersByCharacter[characterId] = isWallpaperUnlocked(wallpaperId) ? wallpaperId : null;
}

function applyWallpaperPreviewStyle(element, wallpaper) {
  if (!element) return;
  if (!wallpaper) {
    element.style.background = "linear-gradient(135deg,#fff9f5,#f2dde7)";
...
function renderShopWallpaperArtwork(container, wallpaper) {
  if (!container || !wallpaper) return;
  container.innerHTML = "";
  const preview = document.createElement("span");
  preview.className = "shop-sheet-wallpaper-preview";
  applyWallpaperPreviewStyle(preview, wallpaper);
  container.append(preview);
}

function updateRoomPickerButtonDisplay() {
  const room = ROOMS.find(r => r.id === currentDisplayedRoomId()) || ROOMS[0];
  const wallpaper = currentDisplayedWallpaper();

  roomPickerSwatch.classList.toggle("wallpaper-active", Boolean(wallpaper));
  roomPickerSwatch.classList.toggle("wallpaper-swatch", Boolean(wallpaper));
  if (wallpaper) {
    applyWallpaperPreviewStyle(roomPickerSwatch, wallpaper);
    roomPickerText.textContent = wallpaper.name;
    roomPickerButton.setAttribute("aria-label", `Change room or wallpaper. Current wallpaper: ${wallpaper.name}. Base room: ${room.name}.`);
  } else {
    roomPickerSwatch.style.setProperty("--swatch", room.swatch);
    roomPickerSwatch.style.background = room.swatch;
    roomPickerSwatch.style.backgroundSize = "auto";
    roomPickerSwatch.style.backgroundPosition = "0 0";
    roomPickerText.textContent = room.name;
    roomPickerButton.setAttribute("aria-label", `Change room or wallpaper. Current room: ${room.name}.`);
  }
}

function applyCurrentRoomWallpaper() {
  if (!roomWallpaperOverlay) return;
  const wallpaper = currentDisplayedWallpaper();
  if (!wallpaper) {
    roomWallpaperOverlay.classList.add("hidden");
    roomWallpaperOverlay.style.background = "none";
    roomWallpaperOverlay.style.backgroundSize = "auto";
    roomWallpaperOverlay.style.opacity = "0";
    return;
  }
  applyWallpaperPreviewStyle(roomWallpaperOverlay, wallpaper);
  roomWallpaperOverlay.style.opacity = String(wallpaper.opacity ?? .58);
  roomWallpaperOverlay.classList.remove("hidden");
}

function getShopWallpaper(wallpaperId) {
  return wallpaperById(wallpaperId);
}
...
function renderShop() {
  ...
  stock.forEach(listing => {
    ...
    const isWallpaper = Boolean(listing.wallpaperId);
    ...
    const wallpaper = isWallpaper ? getShopWallpaper(listing.wallpaperId) : null;
    ...
    const item = (!isRoom && !isWallpaper && !isWardrobe && !isCharacter) ? ITEMS[listing.itemId] : null;

    if (isRoom && !room) return;
    if (isWallpaper && !wallpaper) return;
    if (isCharacter && !characterUnlock) return;
    if (isWardrobe && !wardrobe) return;
    if (!isRoom && !isWallpaper && !isWardrobe && !isCharacter && !item) return;

    const button = document.createElement("button");
    button.type = "button";
    button.className = `shop-item-card${(isRoom || isWallpaper) ? " shop-room-card" : ""}${isWardrobe ? " shop-wardrobe-card" : ""}`;
    button.dataset.itemId = shopListingKey(listing);

    const art = document.createElement("span");
    art.className = `shop-item-art${(isRoom || isWallpaper) ? " shop-room-art" : ""}`;

    if (isRoom) renderShopRoomArtwork(art, room);
    else if (isWallpaper) renderShopWallpaperArtwork(art, wallpaper);
    ...
    const name = document.createElement("strong");
    name.textContent = isRoom ? room.name : isWallpaper ? wallpaper.name : isCharacter ? characterUnlock.name : isWardrobe ? wardrobe.label : item.name;
...
function openShopItem(listingId) {
  ...
  } else if (listing.wallpaperId) {
    if (!getShopWallpaper(listing.wallpaperId)) return;
...
function renderShopItemSheet() {
  ...
  if (!isRoom && !isWallpaper && !isWardrobe && !isCharacter && !item) {
    closeShopItem();
    return;
  }
  ...
  if (isWallpaper) {
    ...
    renderShopWallpaperArtwork(shopSheetIcon, wallpaper);
    shopSheetCategory.textContent = category.label;
    shopSheetName.textContent = wallpaper.name;
    shopPriceLabel.textContent = "Unlock price";
    shopSheetUnitPrice.textContent = `${listing.price} Pink Coins`;
    shopSheetOwned.textContent = unlocked ? "Unlocked ✓" : "Locked";
    shopQuantityBlock.classList.add("hidden");
    ...
    if (unlocked) {
      shopAffordMessage.textContent = "This wallpaper is already permanently unlocked. ♡";
      shopBuyButton.disabled = true;
      shopBuyButton.textContent = "Already Unlocked ✓";
    } else if (shortfall > 0) {
      shopAffordMessage.textContent = `You need ${shortfall} more Pink Coin${shortfall === 1 ? "" : "s"} to unlock this wallpaper.`;
      shopBuyButton.disabled = true;
      shopBuyButton.textContent = "Not Enough Pink Coins";
    } else {
      shopAffordMessage.textContent = "Unlock it once, then use it on any OC's room from the wallpaper picker.";
      shopBuyButton.disabled = false;
      shopBuyButton.innerHTML = `<span>Unlock Wallpaper</span><span class="shop-buy-total"><img src="assets/ui/pink-coin.webp" alt=""> ${total}</span>`;
    }
    return;
  }
...
function buyShopItem() {
  ...
  if (listing.wallpaperId) {
    const wallpaper = getShopWallpaper(listing.wallpaperId);
    if (!wallpaper) return;

    if (isWallpaperUnlocked(wallpaper.id)) {
      showToast(`${wallpaper.name} is already unlocked!`);
      renderShopItemSheet();
      return;
    }
    ...
    renderShop();
    closeShopItem();
    showToast(`${wallpaper.name} unlocked! You can select it from the room picker.`);
    return;
  }
...
function normalizeProfileIconBackgroundSelections() {
  if (!save.profileIconBackgrounds || typeof save.profileIconBackgrounds !== "object") {
    save.profileIconBackgrounds = {};
  }

  const unlocked = unlockedProfileIconBackgroundIds();
  for (const characterId of Object.keys(CHARACTERS)) {
    let selected = save.profileIconBackgrounds[characterId];

    // One-time migration: start with that OC's current Duck Quest background,
    // then let Profile and Duck Quest choices diverge independently afterward.
    if (!PROFILE_ICON_BACKGROUNDS.some(background => background.id === selected)) {
      const questProgress = normalizeDuckQuestCharacterProgress(characterId);
      selected = unlocked.includes(questProgress.iconBackground)
        ? questProgress.iconBackground
        : "white";
    }

    if (!unlocked.includes(selected)) selected = "white";
    save.profileIconBackgrounds[characterId] = selected;
  }

  return save.profileIconBackgrounds;
}

function profileIconBackgroundForCharacter(characterId) {
  const selections = normalizeProfileIconBackgroundSelections();
  return profileIconBackgroundById(selections[characterId] || "white");
}
...
function renderProfileIconPicker(characterId = selectedProfileCharacterId) {
  ...
  const currentId = selections[characterId] || "white";
  const current = profileIconBackgroundById(currentId);
  ...
  for (const background of PROFILE_ICON_BACKGROUNDS) {
    ...
    button.type = "button";
    button.className = `profile-icon-choice${isUnlocked ? "" : " locked"}`;
    button.setAttribute("aria-current", String(currentId === background.id));
    ...
    button.addEventListener("click", () => {
      ...
      selections[characterId] = background.id;
      save.profileIconBackgrounds = selections;
      persist();
...
function renderStatus() {
  ...
  const meadowRank = Math.max(1, Math.min(40, Number(questProgress.areas?.meadow?.unlockedRank) || 1));
  const oceanRank = Math.max(1, Math.min(80, Number(questProgress.areas?.ocean?.unlockedRank) || 1));
  const candyRank = Math.max(1, Math.min(120, Number(questProgress.areas?.candy?.unlockedRank) || 1));
  const cloudRank = Math.max(1, Math.min(160, Number(questProgress.areas?.cloud?.unlockedRank) || 1));
  ...
  if (statusCandyRank) statusCandyRank.textContent = candyRank;
  if (statusCloudRank) statusCloudRank.textContent = cloudRank;
...
// v24.186 Duckie Dash layout tweaks build.

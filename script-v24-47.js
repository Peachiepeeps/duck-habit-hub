...
const DEFAULT_SAVE = {
  ...
  dailies: {
    ...
    paintMixing: { dayKey: "", usedToday: 0, lastReward: null },
    friendshipDate: { dayKey: "", usedToday: 0, lastCharacterId: null, lastGain: 0 }
  },
  stats: {
    tasksCompleted: 0,
    taskCoinsEarned: 0,
...
function launchDuckQuestGame() {
  window.location.href = "duck-quest/battle-v07.html";
}
...
function getSimpleDaily(key) {
  ...
  if (daily.dayKey !== today) {
    daily.dayKey = today;
    daily.usedToday = 0;
    if (key === "paintMixing") daily.lastReward = null;
...
function renderPaintMixingDaily() {
  ...
  if (daily.lastReward) {
    const reward = ITEMS[daily.lastReward];
    renderSimpleDailyState(
      paintMixState,
      daily.lastReward,
      reward ? `${reward.name} mixed!` : "Paint mixed!",
      remaining > 0 ? `${remaining} mix left today.` : "That was your last mix for today."
    );
  } else if (remaining > 0) {
    renderSimpleDailyState(paintMixState, "🎨", `${remaining} mix${remaining === 1 ? "" : "es"} left today`, "Each mix gives exactly one Paint.");
  } else {
    renderSimpleDailyState(paintMixState, "♡", "All mixed for today", "Two fresh mixes will be ready tomorrow.");
  }

  paintMixActionButton.disabled = remaining <= 0;
  paintMixActionButton.textContent = remaining > 0 ? "Mix Paint" : "Come back tomorrow ♡";
}

function mixDailyPaint() {
  const daily = getSimpleDaily("paintMixing");
  if (Number(daily.usedToday || 0) >= PAINT_MIXING_DAILY_LIMIT) return;

  const rewardId = choosePaintMixingReward();
  ...
  persist();
  renderDailies();
  showToast(`Paint Mixing: ${reward.name} added to Inventory! 🎨`);
}
...
function renderFriendshipDateDaily() {
  ...
  if (used > 0 && daily.lastCharacterId) {
    const character = CHARACTERS[daily.lastCharacterId];
    renderSimpleDailyState(friendshipState, "♡", `${character?.name || "Your OC"} had a lovely date!`, `+${Number(daily.lastGain || FRIENDSHIP_DATE_HAPPINESS)} Happiness today.`);
  } else {
    renderSimpleDailyState(friendshipState, "♡", "Choose who gets the Happiness", "It does not have to be the OC currently standing in the room.");
  }

  friendshipActionButton.disabled = used >= FRIENDSHIP_DATE_DAILY_LIMIT;
  friendshipActionButton.textContent = used < FRIENDSHIP_DATE_DAILY_LIMIT ? "Choose OC" : "Date complete ♡";
}
...
function openFriendshipChooser() {
  ...
  const unlockedIds = (Array.isArray(save.unlockedCharacters) ? save.unlockedCharacters : ["peep"]).filter(id => CHARACTERS[id]);
  ...
  unlockedIds.forEach(characterId => {
    ...
    button.innerHTML = `
      <span class="daily-character-heart">♡</span>
      <span>
        <strong>${character.name}</strong>
        <small>Happiness Lv. ${happiness.level} · +${FRIENDSHIP_DATE_HAPPINESS}</small>
      </span>
    `;
    button.addEventListener("click", () => completeFriendshipDate(characterId));
...
function completeFriendshipDate(characterId) {
  const daily = getSimpleDaily("friendshipDate");
  if (Number(daily.usedToday || 0) >= FRIENDSHIP_DATE_DAILY_LIMIT) return;
...
paintMixActionButton.addEventListener("click", mixDailyPaint);
friendshipActionButton.addEventListener("click", openFriendshipChooser);

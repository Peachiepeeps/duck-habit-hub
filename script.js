...
const SHOP_CATEGORIES = {
  ...
  rooms: {
    label: "Room Styles",
    subtitle: "Unlock a new room color permanently for 60 Pink Coins."
  },
  furniture: {
    label: "Furniture",
...
// Standard Duck is fixed at 40 coins and always stays first.
// All shop-available base paints are 30 coins.
// Recipe paints are intentionally excluded so mixing colors stays meaningful.
const SHOP_STOCK = {
  supplies: [
    { itemId: "standard-duck", price: 40 },
    { itemId: "sunglasses", price: 25 },
    { itemId: "yarn", price: 25 },
    { itemId: "thread", price: 25 },
    { itemId: "party-hat", price: 25 },
    { itemId: "knife", price: 30 },
    { itemId: "game-controller", price: 30 },
    { itemId: "pill", price: 25 },
    { itemId: "ufo", price: 35 },
    { itemId: "pocket-monster-ball", price: 35 },
    { itemId: "jester-bell", price: 30 },
    { itemId: "crown", price: 35 },
    { itemId: "magical-girl-pendant", price: 35 },
    { itemId: "bunny-ears", price: 30 },
    { itemId: "flower", price: 22 },
    { itemId: "sleep-mask", price: 25 },
    { itemId: "paintbrush", price: 25 },
    { itemId: "ghost", price: 30 },
    { itemId: "demon-wing", price: 35 },
    { itemId: "angel-wing", price: 35 },
    { itemId: "glitter", price: 30 },
    { itemId: "cat-ears", price: 30 },
    { itemId: "moon", price: 30 },
    { itemId: "mustache", price: 25 },
    { itemId: "top-hat", price: 30 },
    { itemId: "bow", price: 25 }
  ],
  paint: [
    { itemId: "red-paint", price: 30 },
    { itemId: "orange-paint", price: 30 },
    { itemId: "gold-paint", price: 30 },
    { itemId: "silver-paint", price: 30 },
    { itemId: "bronze-paint", price: 30 },
    { itemId: "green-paint", price: 30 },
    { itemId: "blue-paint", price: 30 },
    { itemId: "purple-paint", price: 30 },
    { itemId: "white-paint", price: 30 },
    { itemId: "black-paint", price: 30 }
  ],
  rooms: [
    { roomId: "green", price: 60 },
    { roomId: "blue", price: 60 },
    { roomId: "purple", price: 60 },
    { roomId: "peach", price: 60 },
    { roomId: "pink", price: 60 },
    { roomId: "dark-purple", price: 60 },
    { roomId: "cotton-candy", price: 60 },
    { roomId: "chocolate-brown", price: 60 }
  ],
  furniture: [
    { itemId: "petbed-gray", price: 40 },
    { itemId: "petbed-pink", price: 40 },
    { itemId: "petbed-cream", price: 40 },
    { itemId: "petbed-red", price: 40 },
    { itemId: "petbed-blue", price: 40 },
    { itemId: "shelf-white", price: 45 },
    { itemId: "shelf-dark-brown", price: 45 },
    { itemId: "shelf-brown", price: 45 },
    { itemId: "dresser-brown", price: 45 },
    { itemId: "dresser-dark-brown", price: 45 },
    { itemId: "rug-cloud-dark", price: 35 },
    { itemId: "rug-cloud-white", price: 35 },
    { itemId: "rug-heart-white", price: 35 },
    { itemId: "rug-heart-dark", price: 35 },
    { itemId: "rug-heart-pink", price: 35 },
    { itemId: "rug-heart-blue", price: 35 }
  ],
  hair: [
    { wardrobeId: "hair-low-pigtails", price: 50 },
    { wardrobeId: "hair-ponytail", price: 50 },
    { wardrobeId: "hair-long-pigtails", price: 50 },
    { wardrobeId: "hair-jellyfish", price: 50 }
  ],
  clothing: [
    { wardrobeId: "top-shirt", price: 50 },
    { wardrobeId: "bottom-fluffy", price: 50 },
    { wardrobeId: "socks-blue", price: 50 },
    { wardrobeId: "jacket", price: 50 }
  ],
  shoes: [
    { wardrobeId: "shoes-sneaker", price: 50 }
  ],
  accessories: [
    { wardrobeId: "cow-ears", price: 50 },
    { wardrobeId: "tail-cow", price: 50 },
    { wardrobeId: "collar", price: 50 },
    { wardrobeId: "leg-bandage", price: 50 },
    { wardrobeId: "horns", price: 50 },
    { wardrobeId: "hair-side-ribbon", price: 50 },
    { wardrobeId: "large-back-bow", price: 50 },
    { wardrobeId: "beret", price: 50 }
  ]
};
  ...
  try {
    ...
    const merged = {
      ...
      roomExpansion: {
        ...
        ...(saved.roomExpansion || {}),
        duckSlots: Array.from({ length: 12 }, (_, index) => {
...
function renderWingDuckPicker() {
  ...
  unlocked.forEach(([id, duck]) => {
    ...
    button.setAttribute("aria-label", `Place ${duck.name}`);
...
function getCharacterAssetIds(characterId = save.selectedCharacter) {
  ...
  // equipped-item function here without changing Profiles or Status.
...
function renderRoomPicker() {
  ...
  for (const room of ROOMS) {
    ...
    button.addEventListener("click", () => {
      if (!unlocked) {
        showToast("Locked — unlock this room from the Shop for 60 Pink Coins!");
        return;
      }
      if (currentRoomView === "wing" && save.roomExpansion?.unlocked) {
...
function renderHairOptions(group) {
  ...
  for (const id of group.options) {
    ...
    closetOptions.append(makeOptionCard({
      ...
      onClick: () => {
        if (!unlocked) {
          showToast(`${asset.label} is locked — unlock it from Shop → Hair for 50 Pink Coins!`);
          return;
        }
        save.outfit.hair = id;
        persist();
...
function isClosetAssetUnlocked(assetId) {
  return save.unlockedItems.includes(assetId);
}

function isWardrobeShopUnlocked(wardrobeId) {
  const meta = getWardrobeShopMeta(wardrobeId);
  return Boolean(meta) && meta.unlockIds.every(isClosetAssetUnlocked);
}
...
function unlockWardrobeShopItem(wardrobeId) {
  ...
  const next = new Set(Array.isArray(save.unlockedItems) ? save.unlockedItems : []);
  meta.unlockIds.forEach(id => next.add(id));
  save.unlockedItems = [...next];
  return true;
}
...
function ensureStarterWardrobeUnlocked() {
  const next = new Set(Array.isArray(save.unlockedItems) ? save.unlockedItems : []);
  PEEP_STARTER_WARDROBE.forEach(id => next.add(id));
  save.unlockedItems = [...next];
}

function applyLostProgressRecoveryOnce() {
  if (save.progressRecoveryV1431) return false;

  // Restore the progress the player specifically remembered losing.
  // The old regular duck collection, task history, and Bakery count are
  // deliberately NOT guessed.
  save.coins = 225;
  save.tinyDuckSightings = 4;

  // All currently available room colors + Room to Grow.
  save.unlockedRooms = ROOMS.map(room => room.id);
  save.roomExpansion = {
    ...structuredClone(DEFAULT_SAVE.roomExpansion),
    ...(save.roomExpansion || {}),
    unlocked: true,
    accentRoom: save.room || "cream",
    shelfStyle: save.roomExpansion?.shelfStyle || "cream",
    duckSlots: Array.from({ length: 24 }, (_, index) =>
      save.roomExpansion?.duckSlots?.[index] || null
    )
  };

  // Restore every current Peep Closet option that is either a starter/free
  // piece or sold in one of the wardrobe Shop categories.
  const wardrobeUnlocks = new Set(PEEP_STARTER_WARDROBE);
  ["hair", "clothing", "shoes", "accessories"].forEach(category => {
    (SHOP_STOCK[category] || []).forEach(listing => {
      const meta = getWardrobeShopMeta(listing.wardrobeId);
      (meta?.unlockIds || []).forEach(id => wardrobeUnlocks.add(id));
    });
  });
  save.unlockedItems = [...wardrobeUnlocks];

  // Restore every current furniture item as owned once.
  (SHOP_STOCK.furniture || []).forEach(listing => {
    if (!listing.itemId) return;
    if (inventoryQuantity(listing.itemId) < 1) {
      addInventoryItem(listing.itemId, 1);
    }
  });

  // This recovery happens after the old wardrobe cleanup, so mark that
  // migration complete to ensure paid cosmetics are not stripped again.
  save.wardrobeResetV1261 = true;
  save.wardrobeResetV1262 = true;
  save.progressRecoveryV1431 = true;
...
function resetAccidentalWardrobeUnlocksOnce() {
  ...
  save.unlockedItems = [...PEEP_STARTER_WARDROBE];
...
function renderSockStyle(style) {
  ...
  card.addEventListener("click", () => {
    if (!unlocked) {
      showToast(`${style.label} socks are locked — unlock them from Shop → Clothing for 50 Pink Coins!`);
      return;
    }
  ...
  for (const id of group.options) {
    ...
    if (group.type === "multi") {
      ...
      closetOptions.append(makeOptionCard({
        ...
        onClick: () => {
          if (!unlocked) {
            showToast(`${asset.label} is locked — unlock it from the Shop for 50 Pink Coins!`);
            return;
          }
...
function renderWardrobeShopArtwork(container, wardrobeId, large = false) {
  ...
  const thumb = meta.assetIds.length > 1
    ? makeCombinedThumb(meta.assetIds, { boxW, boxH, targetW, targetH })
    : makeThumb(meta.assetIds[0], { boxW, boxH, targetW, targetH });
...
function renderShop() {
  ...
  if (!stock.length) {
    ...
    message.textContent = "You've unlocked all of this category for now! Congrats!";
  ...
  stock.forEach(listing => {
    ...
    const item = (!isRoom && !isWardrobe) ? ITEMS[listing.itemId] : null;

    if (isRoom && !room) return;
    if (isWardrobe && !wardrobe) return;
    if (!isRoom && !isWardrobe && !item) return;
    ...
    const name = document.createElement("strong");
    name.textContent = isRoom ? room.name : isWardrobe ? wardrobe.label : item.name;
...
function submitTaskForm(event) {
  ...
  const task = normalizeTask({
    ...
    reward,
    repeat,
    nextDue: initialDueForRepeat(repeat),
    createdAt: Date.now(),
    completions: 0
  });
...
const lostProgressRecoveryApplied = applyLostProgressRecoveryOnce();
...
if (lostProgressRecoveryApplied) {
  setTimeout(() => {
    showToast("💕 Lost progress restored! Rooms, wardrobe, furniture, Tiny sightings, and coins are back.");
  }, 250);

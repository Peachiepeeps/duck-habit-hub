...
const DEFAULT_SAVE = {
  ...
  unlockedRooms: ["cream"],
  unlockedItems: ["hair-short", "cat-ears", "left-bow", "right-bow", "top-sweater", "bottom-pleated", "sock-left-rainbow", "sock-right-rainbow", "shoes-loafer", "collar"],
...
function resetAccidentalWardrobeUnlocksOnce() {
  if (save.wardrobeResetV1261) return false;

  // V12.4 accidentally treated Peep's currently equipped pieces as purchased.
  // Reset the Closet to the explicit free/starter set once so old accidental
  // unlocks such as Jacket do not linger forever in localStorage.
  save.unlockedItems = [...PEEP_STARTER_WARDROBE];
  save.wardrobeResetV1261 = true;

  // Also remove equipped pieces that are no longer unlocked, so the visible
  // outfit and Closet state agree immediately after the migration.
  const unlocked = new Set(save.unlockedItems);
  const outfit = save.outfit || {};

  if (!unlocked.has(outfit.hair)) outfit.hair = "hair-short";
  if (!unlocked.has(outfit.top)) outfit.top = "top-sweater";
  if (!unlocked.has(outfit.bottom)) outfit.bottom = "bottom-pleated";
  if (!unlocked.has(outfit.leftSock)) outfit.leftSock = "sock-left-rainbow";
  if (!unlocked.has(outfit.rightSock)) outfit.rightSock = "sock-right-rainbow";
  if (!unlocked.has(outfit.shoes)) outfit.shoes = "shoes-loafer";
  if (!unlocked.has(outfit.ears)) outfit.ears = "cat-ears";
  if (outfit.tail && !unlocked.has(outfit.tail)) outfit.tail = null;

  outfit.extras = (Array.isArray(outfit.extras) ? outfit.extras : [])
    .filter(id => unlocked.has(id));

  // Guarantee the two bow freebies and Collar are worn on the clean default
  // if they were removed by a prior custom outfit.
  ["left-bow", "right-bow", "collar"].forEach(id => {
...
function isRepeatBuyShopListing(listing) {
  ...
  // Crafting supplies, paints, food-style consumables, and Standard Duck
  // can be bought repeatedly. Furniture is a one-time room-style purchase.
...
function shouldHideShopListing(listing) {
  ...
  // Permanent unlocks disappear once owned:
  // room styles, wardrobe cosmetics, and furniture.

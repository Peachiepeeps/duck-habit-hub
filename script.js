...
function loadSave() {
  try {
    ...
    const merged = {
      ...
      craftingInventory: saved.craftingInventory && typeof saved.craftingInventory === "object"
...
function totalCraftingItems() {
  return Object.values(save.craftingInventory || {}).reduce(
    (sum, quantity) => sum + Math.max(0, Number(quantity) || 0),
    0
  );
}

function addCraftingItem(name, quantity = 1) {
  if (!save.craftingInventory) save.craftingInventory = {};
  save.craftingInventory[name] = (Number(save.craftingInventory[name]) || 0) + quantity;
}
...
function renderDailies() {
  ...
  dailyInventoryCount.textContent = `${totalCraftingItems()} item${totalCraftingItems() === 1 ? "" : "s"}`;
  ...
  if (daily.pendingReward) {
    ...
    p.textContent = "There’s a crafting item waiting in the shopping bag.";
...
function claimShoppingReward() {
  ...
  const reward = daily.pendingReward;
  addCraftingItem(reward, 1);
  ...
  persist();
  renderDailies();
  showToast(`Found: ${reward}! Added to crafting inventory. ✨`);
}
...
document.querySelector("#dailyInventoryHint").addEventListener("click", () => {
  showToast("We’ll build the full Inventory page next! ♡");
});

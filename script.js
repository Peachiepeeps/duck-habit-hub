...
function shopStockForTab(tab = currentShopTab) {
  ...
  return stock
    .map((listing, index) => ({ listing, index, owned: shopListingIsOwned(listing) }))
    .sort((a, b) => Number(a.owned) - Number(b.owned) || a.index - b.index)
...
function renderProfiles() {
  ...
  characters.forEach(character => {
    ...
    if (unlocked) {
      button.addEventListener("click", () => selectProfileCharacter(character.id));
    }
...
document.querySelector("#profilesBack").addEventListener("click", closeProfilesToBook);
document.querySelector("#closeProfiles").addEventListener("click", closeProfilesAll);

...
function pokePeep() {
  ...
  peepWrap.classList.remove("poked");
  void peepWrap.offsetWidth;
  peepWrap.classList.add("poked");
...
function makeThumb(assetId) {
  ...
  if (bbox) {
    ...
    const targetW = 76;
    const targetH = 68;
    const scale = Math.min(.80, targetW / bw, targetH / bh);
    const cx = (x0 + x1) / 2;
    const cy = (y0 + y1) / 2;
    const boxW = 92;
    const boxH = 82;
...
function renderSockSide(side) {
  const isLeft = side === "left";
  const saveKey = isLeft ? "leftSock" : "rightSock";
  const options = isLeft
    ? ["sock-left-blue", "sock-left-rainbow"]
    : ["sock-right-blue", "sock-right-rainbow"];

  addSectionLabel(isLeft ? "Left sock" : "Right sock");
  closetOptions.append(makeOptionCard({
    label: "No sock",
    none: true,
    selected: !save.outfit[saveKey],
    onClick: () => {
      save.outfit[saveKey] = null;
      persist();
      renderPeep();
      renderClosetOptions();
    }
  ...
  for (const id of options) {
    closetOptions.append(makeOptionCard({
      assetId: id,
      label: ASSETS[id].label.replace(/ · (Left|Right)$/, ""),
      selected: save.outfit[saveKey] === id,
      onClick: () => {
        save.outfit[saveKey] = id;
        persist();
        renderPeep();
        renderClosetOptions();
      }

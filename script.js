...
function currentOutfitUnlockIds() {
  const ids = new Set(["hair-short", "hair-low-pigtails"]);
...
function ensureCurrentOutfitUnlocked() {
  const next = new Set(Array.isArray(save.unlockedItems) ? save.unlockedItems : []);
  currentOutfitUnlockIds().forEach(id => next.add(id));
  save.unlockedItems = [...next];
}

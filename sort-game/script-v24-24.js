(() => {
  ...
  function randDuck(){let p=DUCKS.filter(d=>d.name!==lastDuckName);if(!p.length)p=DUCKS;const d=p[Math.floor(Math.random()*p.length)];lastDuckName=d.name;return d}
  ...
  function basketFromThrow(projectedX,releaseX,w,v){
    ...
    const mostlyStraightDown=horizontal<Math.max(.30,downward*.58);
    // The Food basket intentionally gets a wider catch area. A mostly straight
    // swipe released anywhere near the middle should feel like a Food throw.
    if(mostlyStraightDown&&releaseX>w*.25&&releaseX<w*.75)return "food";
    if(projectedX<w*.29)return "color";
    if(projectedX>w*.71)return "special";
    return "food";
  }

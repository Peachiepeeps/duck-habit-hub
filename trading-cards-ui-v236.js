(function(){
  ...
  const panel=document.createElement("section");panel.className="tc-panel hidden";panel.innerHTML=`
    ...
    <div class="tc-toolbar"><strong id="tcCollectionCount">0 / ${TC.cards.length} Cards</strong><label>Rarity <select id="tcRarity"><option value="all">All</option><option value="common">Common</option><option value="uncommon">Uncommon</option><option value="rare">Rare</option><option value="ultra">Ultra Rare</option><option value="fabled">Fabled</option></select></label><label class="tc-toggle"><input id="tcMissing" type="checkbox"> Show Missing</label></div>
  ...
  const packLayer=document.createElement("div");packLayer.className="tc-pack-layer hidden";packLayer.innerHTML=`<section class="tc-pack-stage"><button class="tc-pack-close" type="button" aria-label="Close">×</button><p class="tc-pack-kicker">TRADING CARDS</p><h2 class="tc-pack-title">Open a Pack!</h2><div class="tc-pack-rip" role="button" tabindex="0" aria-label="Swipe or click to rip open this Card Pack"><img class="tc-pack-body" src="${TC.packAsset("card-pack-unopened.png")}" alt="Card Pack"><img class="tc-pack-top" src="${TC.packAsset("card-pack-torn-top.png")}" alt=""><span class="tc-swipe-hint" aria-hidden="true">↔</span></div><p class="tc-pack-instruction">Swipe across or click the pack to rip it!</p><div class="tc-reveal-grid" aria-live="polite"></div></section>`;document.body.append(packLayer);
  ...
  function renderCards(){
    TC.ensureState(save);const ownedCount=TC.cards.filter(card=>Number(state.owned[card.id]||0)>0).length;
  ...
  function ripPack(){
    ...
    packRipping=true;const sequence=packSequence;state.unopenedPacks-=1;state.stats.packsOpened+=1;const results=TC.rollPack();results.forEach(card=>TC.grantCard(save,card.id,1));persist();renderCards();
  ...
  window.DUCKIE_TRADING_CARD_UI_BUILD="24.236";
})();

(function(){
  ...
  function ensureDustLine(){
    ...
    if(!line){line=document.createElement("div");line.className="tc-dust-line-v262";line.innerHTML='Card Dust <strong id="tcDustCountV262">0</strong>';info.append(line);}
    const state=TC.cardCraft.ensure(dataRef());
    const count=line.querySelector("#tcDustCountV262");
    const next=Number(state.cardDust||0).toLocaleString();
  ...
  function render(cardId){
    ...
    const copy=document.createElement("div");copy.innerHTML=`<p class="tc-workshop-kicker-v264">CARD WORKSHOP</p><h2>${card.name}</h2><p>${TC.labels[effective]||effective} · Owned ×${qty}</p><p class="tc-workshop-dust-v264">Card Dust <strong>${Number(state.cardDust||0).toLocaleString()}</strong></p>`;
    ...
    duplicate.innerHTML=`<div><strong>Extra Copies</strong><small>Your first copy is always protected. Turn extras into Card Dust.</small><span class="tc-craft-status-v262">${extraCount} extra · ${per} Dust each</span></div>`;
    const dupActions=document.createElement("div");dupActions.className="tc-craft-actions-v262";
    dupActions.append(
      button(`Convert 1 → ${per} Dust`,extraCount<1,()=>{const d=dataRef(),s=TC.cardCraft.ensure(d);if(extras(s,card.id)<1)return;s.owned[card.id]-=1;s.cardDust+=per;commit(d);rerender(card.id,`+${per} Card Dust!`);}),
      button(`Convert All → ${extraCount*per} Dust`,extraCount<1,()=>{const d=dataRef(),s=TC.cardCraft.ensure(d),count=extras(s,card.id);if(count<1)return;s.owned[card.id]-=count;s.cardDust+=count*per;commit(d);rerender(card.id,`Converted ${count} extras into ${count*per} Card Dust!`);},"secondary")
    ...
    if(card.rarity==="fabled"){
      ...
      if(target)rarity.append(button(`Upgrade to ${TC.labels[target]} · ${cost} Dust`,state.cardDust<cost,()=>{const d=dataRef(),s=TC.cardCraft.ensure(d);if(s.cardDust<cost)return;s.cardDust-=cost;s.crafting.rarity[card.id]=target;commit(d);rerender(card.id,`${card.name} is now ${TC.labels[target]}!`);}));
    ...
    if(craft.holo){holo.innerHTML='<div><strong>Holo Finish ✨</strong><small>This card has its permanent Holo finish.</small></div><span class="tc-craft-max-v262">HOLO</span>';}
      ...
      holo.innerHTML=`<div><strong>Holo Finish</strong><small>Uses ${HOLO_EXTRAS} extra copies plus ${HOLO_DUST} Card Dust. Your first copy stays protected.</small></div>`;
      const can=extras(state,card.id)>=HOLO_EXTRAS&&state.cardDust>=HOLO_DUST;
      holo.append(button(`Apply Holo · ${HOLO_DUST} Dust + ${HOLO_EXTRAS} Extras`,!can,()=>{const d=dataRef(),s=TC.cardCraft.ensure(d);if(extras(s,card.id)<HOLO_EXTRAS||s.cardDust<HOLO_DUST)return;s.owned[card.id]-=HOLO_EXTRAS;s.cardDust-=HOLO_DUST;s.crafting.holo[card.id]=true;commit(d);rerender(card.id,`${card.name} is now Holo! ✨`);}));
    }panel.append(holo);

    const variants=TC.variantOptions(card);
    if(variants.length>1){
      const variant=document.createElement("div");variant.className="tc-craft-section-v262 tc-variant-section-v262";variant.innerHTML='<div><strong>Color Variant</strong><small>Unlock normal color variants with Card Dust. Shinies stay encounter-only.</small></div>';
      ...
      for(const option of variants){
        ...
        const b=button(label,!unlocked&&state.cardDust<VARIANT_DUST,()=>{const d=dataRef(),s=TC.cardCraft.ensure(d),c=s.crafting;if(!Array.isArray(c.variantsUnlocked[card.id]))c.variantsUnlocked[card.id]=["default"];if(!c.variantsUnlocked[card.id].includes(option.id)){if(s.cardDust<VARIANT_DUST)return;s.cardDust-=VARIANT_DUST;c.variantsUnlocked[card.id].push(option.id);}c.displayVariant[card.id]=option.id;commit(d);rerender(card.id,`${option.label} ${card.name} is now displayed!`);},selected?"selected":"");
        grid.append(b);
      }
  ...
  window.DuckieCardWorkshopV264={open,close,render:()=>activeCardId&&render(activeCardId)};
  window.DUCKIE_TRADING_CARD_WORKSHOP_UI="24.264";
})();
